...
$sql_mail_dtl = "SELECT `emailjobID`, `jobIndateTime`, `sender`, `receivers_to`, `subject`, `body`, `receivers_cc` 
                 FROM `mail_queue` WHERE `stat`= 0 and `sendDateTime` = '0000-00-00 00:00:00' limit 1;";
$que_mail_dtl = mysqli_query($conn , $sql_mail_dtl) or die(mysqli_error($conn));
while($rec_mail_dtl = mysqli_fetch_assoc($que_mail_dtl)){
    
    $sql_update_sendOn = "UPDATE `mail_queue` SET stat = 1 
                           WHERE `emailjobID` = '".$rec_mail_dtl['emailjobID']."';";
...
if(!$mail->Send()) {
  ...
  $sql_update_sendOn = "UPDATE `mail_queue` SET `stat`=2,  `sendDateTime`= NOW() WHERE `emailjobID` = '".$rec_mail_dtl['emailjobID']."';";
  $que_update_sendOn = mysqli_query($conn,$sql_update_sendOn) or die(mysqli_error($conn));
