...
$dbstr1 ="(DESCRIPTION = (ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = 192.168.3.71)(PORT = 1521))) (CONNECT_DATA =(SERVICE_NAME = uclprod)))";
$dbConn = oci_connect('uclproddb','uclproddb',$dbstr1); 
...
,ESP.ASSET_TYPE,ESP.EQUIP_CAT FROM UCLPRODDB.CDB_ERP_SECPRN_PO ESP,UCLPRODDB.PRODUCTS PR
WHERE ESP.ERP_READ = 0 
AND PR.PRODUCT_CODE = ESP.PRODUCT_CODE
AND PR.PRODUCT_LEASING = '1' AND PR.PRODUCT_CODE IN (505,510)");
...
while (($row = oci_fetch_array($stid, OCI_BOTH)) != false) {	//Read Oracle Source table
    ...
    $v_sql_mysql_insert = "INSERT INTO `ucl_sps_po_gen`(`fac_no`, `cbd`, `system_no`, `product_code`, `server_date`, `accpunt_open_date`, `charges_rec_date`, `client_name`, `client_full_name`, `cc_address_01`, `cc_address_02`, `cc_address_03`, `make_desc`, `model_desc`, `serial_number`, `vehicle_number`, `engine_number`, `chassis_number`, `yom`, `asset_price`, `lease_amount`, `vat`, `period`, `gross_rental`, `margin`, `client_nic`, `supplier_code`, `supplier_name`, `supplier_address_01`, `supplier_address_02`, `supplier_address_03`, `po_printed_date`, `po_printed_date_time`, `po_auth_by`, `erp_read`, `print_stats`, `TYPE_CODE`,`asset_type`,`EQUIP_CAT`) 
                                            VALUES ('".$row[34]."','".$row[0]."','".$row[1]."','".$row[2]."','".$row[3]."','".$row[4]."','".$row[5]."','".$row[6]."','".$row[7]."','".$row[8]."','".$row[9]."','".$row[10]."','".$row[11]."','".$row[12]."','".$row[13]."','".$row[14]."','".$row[15]."','".$row[16]."','".$row[17]."','".$row[18]."','".$row[19]."','".$row[20]."','".$row[21]."','".$row[22]."','".$row[23]."','".$row[24]."','".$row[25]."','".$row[26]."','".$row[27]."','".$row[28]."','".$row[29]."','".$row[30]."','".$row[31]."','".$row[32]."','".$row[33]."','0','UCLEPO','".$row[35]."','".$row[36]."')";                                             
    //echo  $v_sql_mysql_insert."<br/>";
    echo  $row[34]." - LEPO3W"."<br/>";
    mysqli_query($conn,$v_sql_mysql_insert) or die(mysqli_error());
    $v_sql = oci_parse($dbConn, "UPDATE uclproddb.CDB_ERP_SECPRN_PO CCD SET CCD.ERP_READ = '1' WHERE CCD.FACNO = '".$row[34]."' AND CCD.SYSTEM_NO = ".$row[1]." AND CCD.ERP_READ = '0'");
