...
Page Name		: CBL exceeding 05 Years Letter
Purpose			: Request for CBL exceeding 05 Years Letter
Author			: Nilanka Chameera
Date & Time		: 10.53 A.M 09/08/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="sps/e/011";
                ...
                if(val_txt_u_defiend == ""){
                    ...
                    if (r==true){
                       // alert('B');
            			...
            			$.ajax({ 
            				type:'POST', 
            				data: {get_LetterTypeCode_cbl : get_let_types ,get_User_cbl : val_user , U_type_cbl : val_U_type ,txt_u_defiend_cbl : val_txt_u_defiend }, 
...
<table>
    <tr>
        ...
        <td>
        	 <?php
        		$sql_sps_let_types = "SELECT `TYPE_CODE`, `TYPE_DESC` FROM `sps_let_types` WHERE `TYPE_CODE` = 'CBL05' ;";
    ...
    <tr>
        ...
        <td>
            <select class="box_decaretion" name="txt_U_Type" id="txt_U_Type" onkeyup="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" onchange="getAcceessLeval();">
                <option value="U">User Define</option>
