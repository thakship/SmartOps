...
Page Name		: Receive Courier Files - New
Purpose			: To Receive Courier Files
Author			: Madushan Wikramaarachchi
Date & Time		: 11.26 A.M 27/09/2016 (Modified)
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
//	$_SESSION['page']="cou/e/011";
...
<script type="text/javascript">
    ...
    function isCheckBox(title){
        if(document.getElementById('chkDoc'+title).checked == true){
            document.getElementById('txt3'+title).value = "OK";
            document.getElementById('txt3'+title).readOnly = true;
        }else{
            document.getElementById('txt3'+title).value = "";
            document.getElementById('txt3'+title).readOnly = false;
        }
    }
       ...
       // alert(title);
        ...
        for(var i = 1 ; i < rowCount ; i++){
            ...
            if(document.getElementById('chkDoc'+i).checked == true){
                ...
                PDRString = PDRString+document.getElementById('txt1'+i).value+"|";
                PDRDis = PDRDis+document.getElementById('txt3'+i).value+"|";
                if(document.getElementById('txt3'+i).value == ""){
                    PDRX = 1;
                    document.getElementById('txt3'+i).focus();
                    document.getElementById('txt3'+i).style.backgroundColor = "lightblue";
...
if(PDRX == 1){ //-- Status PDRX If------------------------------------
    alert("Fill the reason column for unchecked documents ");
}else{
    //alert("OK Function");
    //alert(PDRString);
    //alert(PDRDis);
        ...
        if (r == true){
    		$.ajax({ 
    			type:'POST', 
    			data: {isFileNumber : fileNum , isDocString : docString , isLogUser : logUser , isPDRString : PDRString , isPDRDis : PDRDis}, 
        ...
        if(preSts == 1){
            ...
            window.open('http://cdberp:8080/cdb/pages/Courier%20Management%20System/Entry/ReceiveCourierFiles-New.php?DispName=Receive%20Courier%20Files','conectpage');
        }else{
             alert('Another courier file(s) Remaining');
