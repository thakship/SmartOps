...
Page Name		: Entering Courier Item - ALL Branch
Purpose			: To Create Courier Files - All Branch
Author			: Madushan Wikramaarachchi
Date & Time		: 11.51 A.M 13/09/2016
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->

<?php
	session_start();
	$_SESSION['page']="cou/e/010";
...
<title>ntering Courier Item - ALL Branch</title>
...
<script type="text/javascript">
    function pageRef(){
        window.open('http://cdberp:8080/cdb/pages/Courier%20Management%20System/Entry/enteringCourierItemAllBranch.php?DispName=Entering%20Courier%20Item%20-%20All%20Branch','conectpage');
    }
</script>
...
<script type="text/javascript">
    ...
    function checkbox_selector(){
        //alert("a");
        var txtnumbranch = document.getElementById('txtnumbranch').value;
        if(document.getElementById("chk_all_item").checked == true){
            for(var br_chk = 1 ; br_chk <= txtnumbranch ; br_chk++){
                document.getElementById("checkboxBrn"+br_chk).checked = true;
            }
        }else{
             for(var br_chk = 1 ; br_chk <= txtnumbranch ; br_chk++){
                document.getElementById("checkboxBrn"+br_chk).checked = false;
            }
        }
...
<input type="text" name="txtufileAll" id="txtufileAll" value=""  onkeypress="return disableEnterKey(event)"/>
</div>
<table>
    <tr>
...
<table>
  <tr>
    ...
    <td>
         <input type="button" onclick="popupb(1);" value="..." id="groupDOC" name="groupDOC"/>
    </td>
  </tr>
  <tr>
    <td style=" width:110px;"><label class="linetop">Receiving Officer :</label></td>
    <td>
...
<table>
 ...
 <input class="buttonManage" type="button" onclick="popupa(1)" value="Insert group doc" id="rowNew1" name="rowNew1" /><br/><br/>
    ...
    <div id="conten2">
        ...
        <input style="margin-left:200px; font-size: 12px;" type="button" onclick="popupb(0)" value="Colse" id="popupclose" name="popupclose" />
        </p>
        <div style='display:none;'>
        <input style="width:100px;" type="text" name="tx" id="tx" value=""  onkeypress="return disableEnterKey(event)" readonly="readonly" required="required"/>
        <input style="width:100px;" type="text" name="ty" id="ty" value=""   onkeypress="return disableEnterKey(event)" readonly="readonly" required="required"/>
        ...
        <div style="text-align: right; margin-right: 50px;">
             <input class="buttonManage" type="button" value="SELECT" id="popupSearchBTN" name="popupSearchBTN" onclick="popupb(0);" style="width: 100px;" />
             <label style="margin-left: 20px;">Check All Branch</label> 
             <input type="checkbox" id="chk_all_item" name="chk_all_item" onclick="checkbox_selector();"/>
             
        </div>
        
        <div id="getNewtblPopup">
		<?php
            $viewBranch = "SELECT b.branchNumber, b.branchName , d.deparmentNumber
                            FROM branch AS b , deparment AS d
                            WHERE 
                            b.branchNumber = d.branchNumber AND
                            b.branchNumber != '".$_SESSION['userBranch']."'";
            $sql_viewBranch = mysqli_query($conn,$viewBranch);
        ?>
        
        <table class="tbl1" border="1">
              <tr>
                <td style="text-align:center; padding-top:5px; padding-bottom:5; width:150px;">Branch Number</td>
                <td style="text-align:center; padding-top:5px; padding-bottom:5;width:200px;">Branch Name</td>
                <td style="text-align:center; padding-top:5px; padding-bottom:5;width:100px;">Access</td>
              </tr>
				<?php
                    $a = 1 ;
                    while ($add_sql_viewBranch = mysqli_fetch_array($sql_viewBranch)){
                        echo "<tr style='background-color:#FFFFFF;'>";
                        echo "<td style='width:150px;'>
                              <div id='divb".$a."' style='display:none;'>
                              <input class='txt' type='text' name='txtDepNumber".$a."' id='txtDepNumber".$a."' value='".$add_sql_viewBranch[2]."'/></div>
                              <input style='width:150px;' type='text' name='txtbranchNumber".$a."' id='txtbranchNumber".$a."'  value='".$add_sql_viewBranch[0]."' readonly='readonly'/></td>";
                        echo "<td style='width:200px;'>
                              <input style='width:200px;' type='text' name='txtbranchName".$a."' id='txtbranchName".$a."' value='".$add_sql_viewBranch[1]."' readonly='readonly'/></td>";
                        echo "<td style='width:100px;'>";
                        echo "<input type='checkbox' id='checkboxBrn".$a."' name='checkboxBrn".$a."'/>"; //  checked='checked'
                        echo "</td>"; 
                        echo "</tr>";
                        $a++;
                    }
             ...
             <div style="display: none;"><input type="text" name="txtnumbranch" id="txtnumbranch" value="<?php echo $a-1; ?>"/></div>
            </div>
    </div>
 <div id="newdoctb">
...
<script>
    function pageSAVE(){
        ...
        if(txtResiveOfficer == ""){
            ...
            for(var br_chk = 1 ; br_chk <= txtnumbranch ; br_chk++){
                //alert(br_chk);
                if(document.getElementById("checkboxBrn"+br_chk).checked == true){
                    //alert(document.getElementById('txtbranchNumber'+br_chk).value);
                    //var selBranchNumber = document.getElementById('txtbranchNumber'+br_chk).value
                    fname1 = fname1+fname+" "+document.getElementById('txtbranchNumber'+br_chk).value+"|";
                    branchString = branchString+document.getElementById('txtbranchNumber'+br_chk).value+"|";
                    //alert(fname1);
                    
                    //alert(document.getElementById('txtDepNumber'+br_chk).value);
                    //var selDepartmentNumber = document.getElementById('txtDepNumber'+br_chk).value
                    //departmentString = branchString+document.getElementById('txtDepNumber'+br_chk).value+"|";
                        ...
                        for(var tr = 1; tr <= txtrow; tr++){
                            ...
                            selectDocument = selectDocument+document.getElementById('txtb'+ty).value+"|";
                        }
                        if(getDocData ==  "A"){
                             if(fileName == ""){
                            ...
                            /*}else if(selBranchNumber == ""){
                                ...
                                alert("Department is missing.");*/
                                  ...
                                  //alert(selBranchNumber);
                              //  var r = confirm('Are you sure you send to '+document.getElementById('txtbranchName'+br_chk).value+' ?')
                                //var r = true;
                                //if (r == true){
                        			$.ajax({ 
                        				type:'POST', 
                        				data: {get_fname : fname1 , get_selFileType : selFileType , get_selBranchNumber : branchString , get_txtResiveOfficer : txtResiveOfficer , get_txtareSN : txtareSN , get_txtPreFNum : txtPreFNum , get_txtrow : txtrow , get_txtUser : txtUser , get_txtubranch : txtubranch , get_txtuDepartment : txtuDepartment , get_selectDocument : selectDocument}, 
                        				url: '../CMS_DEVELOPMENT/PHP_FUNCTION/document_save_function_all.php', 
                        				success: function(val_retn) { 
                        				   // alert(val_retn); 
                                            //pageCloseDefineLetterTypes();
                                           // document.getElementById('txtufileAll').value = val_retn;
                                             document.getElementById('qwert').innerHTML = val_retn;
