...
include('../../../php_con/includes/db.ini.php');
	$filefull = "SELECT @rownum := @rownum + 1 AS Line,tmp.brn_num, tmp.brn_name, tmp.sen_Date FROM 
(SELECT DISTINCT`courier_files`.`branchNumber` brn_num, `branch`.`branchName` brn_name,`courier_files`.`sendDateTime` sen_Date FROM `courier_files`,`branch` WHERE `courier_files`.`branchNumber` = `branch`.`branchNumber` AND DATE(`courier_files`.`sendDateTime`)='$sendDate' AND (`stats`='FDR' OR `stats`='PDR') ORDER BY `courier_files`.`branchNumber` )tmp,(SELECT @rownum := 0) r";
   // echo $filefull;
