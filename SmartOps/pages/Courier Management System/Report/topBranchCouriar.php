...
Page Name		: Courier Top Send Branches
Purpose			: To get Courier Top Send Branches
Author			: Madushan Wikramaarachchi
Date & Time		: 01.09 P.M 25/08/2014 (Modified)
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="cou/r/002";
...
<title>Courier Top Send Branches</title>
...
<script type="text/javascript">
	function TypeSelect(){
		...
		mydata.open("GET","ajaxTopBranch1.php"+"?date1="+type1+"&date2="+type2,true);
		mydata.send();
	}
    function Typeprint(){
...
/*if(isset($_POST['getPDF']) && $_POST['getPDF']=='PDF'){
	$title = "Courier Top Send Branches";//report name
	$psD1 = $_POST['empappodate1'];
	$psD2 = $_POST['empappodate2'];
	$d1 = "Date Range                : ".$psD1." to ".$psD2;//Other information
	$tableget = "rank,15,Rank|brn1,75,Send Branch|nof,30,Num of Files";//must want to table

	$sql = "SELECT @rownum := @rownum %PLUS% 1 AS rank, temp.branch1 AS brn1, temp.Num_of_Files AS nof FROM(SELECT `branch`.`branchName` AS branch1, COUNT(`fileNumber`) AS Num_of_Files FROM `branch`,`courier_files` WHERE `branch`.`branchNumber`=`courier_files`.`branchNumber` AND DATE(`sendDateTime`) BETWEEN '$psD1' AND '$psD2' group by `courier_files`.`branchNumber`)temp,(SELECT @rownum := 0) r ORDER BY temp.Num_of_Files DESC LIMIT 100";
