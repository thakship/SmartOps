<?php
	...
	$sqlQur = "SELECT @rownum := @rownum + 1 AS rank, temp1.`branchNumber` AS brn1,temp1.Num_of_Files AS nof1  FROM (SELECT `branchNumber`,COUNT(`fileNumber`) AS Num_of_Files FROM `courier_files` WHERE DATE(`sendDateTime`) BETWEEN '$sDate1' AND '$sDate2' group by `branchNumber`)temp1 ,(SELECT @rownum := 0) r  ORDER BY nof1 DESC LIMIT 100";
...
<table class="tbl1" border="1" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td style="text-align:left; padding-left:5px; width:300px;">Send Branch</td>
