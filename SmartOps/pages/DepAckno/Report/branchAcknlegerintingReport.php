...
<!------------------------------------------------------------------------------------------------------------------------
Module Code		: Dep Ack
Page Name		: Branch Acknowledgement
Purpose			: get report for Branch Acknowledgement
Author			: Madushan Wikramaarachchi
Date & Time		: 12.00 PM 25/03/2015
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="depac/r/002";
...
<script type="text/javascript">
    ...
    function getGriedViwe(){
        ...
        var selBranch = document.getElementById('selBranchNumber').value;
...
<table>
     <tr>
      ...
      <tr>
        <td style="text-align: right; width:150px;"><label class="linetop" style="margin-right: 5px;">Branch :</label></td>
        <td style="text-align:left; padding-top:5px; padding-bottom:5;width:200px;">
        	<?php
                $sql_select_branch="SELECT `branchNumber`,`branchName` FROM `branch`";
                $quary_select_branch = mysqli_query($conn,$sql_select_branch);
            ?>
            <select class="box_decaretion" name="selBranchNumber" id="selBranchNumber" onKeyPress="return disableEnterKey(event)">
                <option value="">All Branch</option>
      			<?php
                    while ($rec_select_branch = mysqli_fetch_array($quary_select_branch)){
                        echo "<option value='".$rec_select_branch[0]."'>".$rec_select_branch[1]."</option>";
	 				}
                ?>
    		</select>
        </td>
