        ...
        try{
            ...
            if(trim($_POST['sel_catagory']) != "" && trim($_POST['sel_scat01']) != "" && trim($_POST['sel_scat02']) != "" && trim($_POST['txt_issue']) != "" && trim($_POST['txt_Description']) != "" && trim($_POST['sel_States']) != "" && trim($_POST['sel_Urgency']) != "" && trim($_POST['sel_Priority'])!= ""){
				...
				while ($rec = mysqli_fetch_array($add)){
                 ...
                 $sqlFunction ="SELECT GetNextSerial('m11',".$Current_Year.") FROM DUAL";    //Create the Unique Bokking Reference [Year wise]
                 $quary_Function = mysqli_query($conn,$sqlFunction);
			     while ($rec_Function = mysqli_fetch_array($quary_Function)){
			         $batch_num = $rec_Function[0]; 
			     }
                $TableID = "loan_".$Current_Year.str_pad($batch_num, 4, '0', STR_PAD_LEFT); //This will return 20140001 as first number for 2014 and 20150001 in 2015
                // Attachemt Uploaded..............................................................................
                ...
                if($getUploadStates == 1){
                    ...
                    if($getUploadStates == 2){
                        ...
                        if($getUploadStates == 3){
                            ...
                            $v_getSQL_insert = "INSERT INTO `loan_cdb_helpdesk`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_type`,`ssb_cycle`,`ssb_facility_amount`,`attachment_namesub`) 
                                                VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".trim($_POST['txt_issue'])."', '".trim($_POST['txt_Description'])."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','Initial Submission',1,'".trim($_POST['txt_facility_amount'])."','".$getUploadStatessub."');";
                            //echo $v_getSQL_insert;
                            $que_getSQL_insert = mysqli_query($conn,$v_getSQL_insert) or die(mysqli_error($conn));
                            $v_getSQL_insert_1 = "INSERT INTO `loan_cdb_helpdesk_his`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_type`,`ssb_cycle`,`ssb_facility_amount`,`attachment_namesub`)
                                                 VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".trim($_POST['txt_issue'])."', '".trim($_POST['txt_Description'])."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','Initial Submission',1,'".trim($_POST['txt_facility_amount'])."','".$getUploadStatessub."');";
                            //echo $v_getSQL_insert_1;
                            $que_getSQL_insert_1 = mysqli_query($conn,$v_getSQL_insert_1) or die(mysqli_error($conn));
                             $sql_insert = "INSERT INTO `loan_cdb_ssb`(`helpid`, `applicant_info`, `clear_valuation`, `guarantor_1`, `guarantor_2`, `cr_copy_and_invoce`, `supplar_info`) 
                            ...
                            for($y = 1 ; $y <= trim($_POST['row_COUNT']) ; $y++){
                                if(trim($_POST['txtb'.$y])!= ""){
                                    $v_getSQL_Note =  "INSERT INTO `loan_cdb_help_note`(`helpid`, `note_code`, `note_discr`, `enterBy`, `enterDateTime`) VALUES ('".$TableID."','".$y."',CONCAT('".trim($_POST['txtb'.$y])." on ', now()),'".$_SESSION['user']."',now());";
...
sendMailNuw($getmail,$title,$mail,$headers);
                ...
                /*if($getUploadStates == 0){
                    ...
                    echo "Facility amount is invalied.";  
                }       
