...
Page Name		: My Branch Request
Purpose			: CMy Branch Request
Author			: Madushan Wikramaarachchi
Date & Time		: 02:28 P.M 27/04/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="lon/e/011";
...
<title>My Branch Request - Loan Approval</title> 
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/LoanApproval/Entry/completedFile.php?DispName=Completed%20File','conectpage');
}
function pageRef(){
...
function popup(x,title){
		...
		if(x==1){ 
			...
			mydataGried.open("GET","ajax_Gried.php"+"?get_Approve_new="+x+"&get_function_new="+title,true);
			mydataGried.send();
			/*document.getElementById('outer').style.visibility = "visible";
			document.getElementById('conten').style.visibility = "visible";*/
    ...
    function clientValidate(title,cat_cod){
        ...
        var helpClose = document.getElementById('txt_helpdesk_close').value;
         var stats = "NO";
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreqmybranch="+getHI+"&gettxt_helpdesk_closemybranch="+helpClose+"&get_c_statsmybranch="+stats,true);
            mydata.send();
...
function is_kiosk_ok(title){
    //alert('OK button');
    if(document.getElementById('txt_pn').value != ""){
             var chk_1 = 0;
        var chk_2 = 0;
        var chk_3 = 0;
        var chk_4 = 0;
        ...
        var ksrl_pn = "Completed File - "+document.getElementById('txt_pn').value;
        ...
        alert('User ID is missing.');
         ...
         var r = confirm('Confirm to proceed ?')
            if (r==true){
    			...
    			$.ajax({ 
    				type:'POST', 
    				data: {get_reInitate_helpid : ksrl_helpid , get_reInitate_user : ksrl_user  , get_reInitate_pn : ksrl_pn}, 
    				url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				    alert(val_retn);
...
function is_submit(){
    ...
    var ksrl_user = document.getElementById('txt_USERMY').value;
    var ksrl_pn = document.getElementById('txt_pn').value;
    var app_number = document.getElementById('appNumber').value;
    //alert(appNUmber+'--'+ksrl_helpid+'--'+ksrl_user);
    var status = "YES";
    if(ksrl_helpid == ""){
        alert('Help ID is missing.');
        ...
        alert('Missing Applicetion Number.');
    }else{
         var r = confirm('Confirm to proceed ?')
            if (r==true){
                //alert('function ok');
    			$.ajax({ 
    				type:'POST', 
    				data: {get_submit_helpid : ksrl_helpid , get_submit_user : ksrl_user , get_submit_pn : ksrl_pn , get_status : status , get_app_number : app_number}, 
    				url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				    alert(val_retn);
...
function getNICGrid(){
    ...
    var txtuserBranch = document.getElementById('txtuserBranch').value;
    var txtuserDepartment = document.getElementById('txtuserDepartment').value;
	...
	mydata.open('GET','../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php'+'?get_cnicmybranch='+cnic+"&get_c_cstatusmybranch="+status+"&txtuserBranch="+txtuserBranch+"&txtuserDepartment="+txtuserDepartment,true);
	mydata.send();
}
...
<table>
  <tr>
    <td style="width: 100px;"><label class="linetop">Client Information  :</label></td>
    <td>
...
<?php
    $v_sql_det_detelsCNT = "SELECT COUNT(1) ,SUM(`ssb_facility_amount`) , 
SUM(CASE WHEN DATE(`loan_cdb_helpdesk`.`caloser_dateTime`)>='".date('Y-m-01')."' THEN `ssb_facility_amount` ELSE 0 END),
SUM(CASE WHEN DATE(`loan_cdb_helpdesk`.`caloser_dateTime`)>='".date('Y-m-01')."' THEN 1 ELSE 0 END)
FROM `loan_cdb_helpdesk`, `urgency_states`, `priority_states`, `cmb_states`, `branch`, `deparment`
 WHERE `loan_cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
	...
	echo "<h3>Total record(s) : ". $rec_det_detelsCNT[0]. "<span style='padding-left:5em'> Total of Rs. ".number_format($rec_det_detelsCNT[1], 2)."</span><span style='padding-left:5em'> For the month : ".number_format($rec_det_detelsCNT[2], 2) . " [". $rec_det_detelsCNT[3] ."]</span></h3>";
    }
...
<script type="text/javascript"><!--
        var pager = new Pager('myTable', 25); 
        pager.init(); 
...
<div style="display: none;">
    ...
    <input type="text" name="txtuserDepartment" id="txtuserDepartment" value="<?php echo  $_SESSION['userDepartment']; ?>" />
</div>
