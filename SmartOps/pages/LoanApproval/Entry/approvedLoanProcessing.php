...
Page Name		: Approved Loan Processing 
Purpose			: View list for Approved Loan Processing Credit and ligele Department 
Author			: Madushan Wikramaarachchi
Date & Time		: 10.17 A.M 27/05/2016
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="lon/e/008";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/LoanApproval/Entry/approvedLoanProcessing.php?DispName=Approved%20Loan%20Processing','conectpage');
}
function pageRef(){
...
function is_Legal_Approve(){
    ...
    if (r==true){
		...
		$.ajax({ 		 
			type:'POST', 
			data: {get_Legal_Approve_helpid : ksrl_helpid  , get_Legal_Approve_user : ksrl_user , get_Legal_Approve_pn : ksrl_pn , get_Legal_Approve_stat : stat}, 
			url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
			success: function(val_retn) { 
			  // document.getElementById('maneSpan').innerHTML = val_retn;
			   alert(val_retn);
...
function is_Credit_Approve(){
    ...
    if(ksrl_pn != ""){
        ...
        if (r==true){
    		...
    		$.ajax({ 		 
    			type:'POST', 
    			data: {get_Credit_Approve_helpid : ksrl_helpid  , get_Credit_Approve_user : ksrl_user , get_Credit_Approve_pn : ksrl_pn , get_Credit_Approve_stat : stat}, 
    			url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    			success: function(val_retn) { 
    			  // document.getElementById('maneSpan').innerHTML = val_retn;
    			   alert(val_retn);
...
<?php
 ...
 WHERE `loan_cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `loan_cdb_helpdesk`.`submit_on` != '0000-00-00 00:00:00' AND
       `loan_cdb_helpdesk`.`cro_chk_on` = '0000-00-00 00:00:00' AND
    ...
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
                ...
                echo "<h4>Total record(s) : ". $rec_det_detelsCNT[0]."</h4>";
