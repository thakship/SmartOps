...
Page Name		: Completed File – Loan Approval
Purpose			: Completed File – Loan Approval
Author			: Madushan Wikramaarachchi
Date & Time		: 04.06 A.M 16/09/2015
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="lon/e/004";
    ...
    function clientValidate(title,cat_cod){
        ...
        var helpClose = document.getElementById('txt_helpdesk_close').value;
         var stats = "NO";
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreq="+getHI+"&gettxt_helpdesk_close="+helpClose+"&get_c_stats="+stats,true);
            mydata.send();
...
function getNICGrid(){
	...
	mydata.open('GET','../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php'+'?get_cnic='+cnic+"&get_c_cstatus="+status,true);
	mydata.send();
}
...
FROM `loan_cdb_helpdesk`, `urgency_states`, `priority_states`, `cmb_states`, `branch`, `deparment`
 WHERE `loan_cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `loan_cdb_helpdesk`.`file_final_stat` = ''
