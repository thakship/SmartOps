...
Purpose			: View Request List
Author			: Madushan Wikramaarachchi
Date & Time		: 10.31 A.M 07/01/2016
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="lon/e/002";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/LoanApproval/Entry/serviceRequsetList.php?DispName=Service%20Request%20List','conectpage');
}
function pageRef(){
   ...
   window.open('http://cdberp:8080/cdb/pages/LoanApproval/Entry/serviceRequsetList.php?DispName=Service%20Request%20List','conectpage');
    ...
    function clientValidate(title,cat_cod){
       ...
       //alert(cat_cod);
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreq1="+getHI+"&gettxt_helpdesk_close1="+helpClose+"&getuserID="+userID,true);
            mydata.send();
...
function is_kiosk_ok(title){
    ...
    if(ksrl_user == ""){
        ...
        var r = confirm('Confirm to Notify Comment?')
        if(r==true){
            //alert('function ok');
			$.ajax({ 
				type:'POST', 
...
<?php
 ...
 WHERE `loan_cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `loan_cdb_helpdesk`.`cmb_code` = '5001'
       AND `loan_cdb_helpdesk`.`ssb_type` NOT LIKE 'Pending Notified%'
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:70px;text-align: right;"><span style="margin-right: 5px;">Zone</span></td>
...
<?php
 $v_sql_det_detels = "SELECT `loan_cdb_helpdesk`.`helpid` , 
                              DATE(`loan_cdb_helpdesk`.`enterDateTime`), 
                              ...
                              `loan_cdb_helpdesk`.`cat_code` , 
                              `loan_cdb_helpdesk`.`ssb_facility_amount` , 
                              `loan_cdb_helpdesk`.`ssb_type`,
                              `loan_cdb_helpdesk`.`help_discr`,
                              IF(`loan_cdb_helpdesk`.`ssb_app_number` = '', 4, `loan_cdb_helpdesk`.`IsAppValid`) as validapp,
                              `loan_cdb_helpdesk`.`ssb_app_number`,
                              `loan_cdb_helpdesk`.`facno`,
                              datediff(date(now()),DATE(`loan_cdb_helpdesk`.`enterDateTime`)) as agedays,
                              `loan_cdb_helpdesk`.`curr_stage`, 
                              `loan_cdb_helpdesk`.`assign_to`,
                              `loan_cdb_helpdesk`.`taken_by` , 
                              DATE(`loan_cdb_helpdesk`.`recommend_datetime`), 
                              `branch`.CDBZONE
 FROM `loan_cdb_helpdesk`, `urgency_states`, `priority_states`, `loan_product` , `branch`, `deparment`
 WHERE `loan_cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `loan_cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       ...
       `loan_cdb_helpdesk`.`cmb_code` = '5001'
        AND `loan_cdb_helpdesk`.`ssb_type` NOT LIKE 'Pending Notified%'
       ORDER BY `loan_cdb_helpdesk`.`enterDateTime` DESC;";
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
      ...
      $sql_count_noteP = "SELECT COUNT(*) FROM `loan_cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."' AND note_discr like 'Pending Notified%';";
      $v_que_count_noteP = mysqli_query($conn,$sql_count_noteP);
      while($rec_count_noteP =  mysqli_fetch_array($v_que_count_noteP)){
            $f_ColP = "background-color:#FFFFFF;";
            if($rec_count_noteP[0] == 0){
                $f_ColP = "background-color:#FFFFFF;";
            }else{
                $f_ColP = "background-color:#FFFF00;";
            }  
      }
      
      $sql_count_noteF = "SELECT COUNT(*) FROM loan_cdb_help_note AS l  
                           WHERE l.helpid = '".$rec_det_detels[0]."' 
                             AND note_discr like 'File forwarded by%';";
      $v_que_count_noteF = mysqli_query($conn,$sql_count_noteF);
      while($rec_count_noteF =  mysqli_fetch_array($v_que_count_noteF)){
            $f_ColF = "background-color:#FFFFFF;";
            if($rec_count_noteF[0] > 0){
                $f_ColF = "background-color:#d4a0e8;";
            }else{
                $f_ColF = "background-color:#FFFFFF;";
            }  
      }
        ...
        echo "<td style='width:190px;text-align: left;".$MyRow .$f_ColP."'><span style='margin-left: 2px;'>".$rec_det_detels[12]."</span></td>";
        ...
        echo "<td style='width:70px;text-align: right;".$MyRow .$f_ColF."' title = '".$asingBy."'><span style='margin-right: 2px;'>".$rec_det_detels[9]."</span></td>";
