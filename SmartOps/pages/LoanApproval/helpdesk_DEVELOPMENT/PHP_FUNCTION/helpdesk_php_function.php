            ...
            if($_FILES["fileAttachmentsub"]["name"] != ""){ 
                ...
                if(($_FILES["fileAttachmentsub"]["size"] < 12000000)){
                    if($_FILES["fileAttachmentsub"]["error"] > 0){
                        ...
                        if(file_exists("C:/wamp64/www/CDB/uploadHelpdesk/" .$_SESSION['fileAttachmentsub'])){
                            //echo "<script> alert('already exists');script>";
                            return 3;
                            $_SESSION['fileAttachmentsub']="";
...
//.........................................................Service Request Delete..................................................................................
    function isServiceRequstDelete($helpid,$Entry_User){
        $conn = DatabaseConnection();
        date_default_timezone_set('Asia/Colombo');
        $v_Delete_sql = "UPDATE `loan_cdb_helpdesk` SET `cmb_code`='5003', `caloser_by`= '".$Entry_User."', `caloser_dateTime`= now(), `resulution`='Cenacle by User',`ssb_type`='Cenacle by User', `solution`='Cenacle by User' WHERE `helpid` = '".$helpid."';";
...
//................................................. Remove Attachment.....................................................................................
    function isRemoveAttachment($getAttach){
        $conn = DatabaseConnection();
        $sql_att = "SELECT `attachment_name` FROM `loan_cdb_helpdesk` WHERE `helpid` = '".$getAttach."';";
        $que_att = mysqli_query($conn,$sql_att);
        while($RES_att = mysqli_fetch_assoc($que_att)){
            $files = $_SERVER['DOCUMENT_ROOT'] . "bkperp/cdberptest/uploadHelpdesk/".$RES_att['attachment_name'];
            //$mke = "C:/www/CDB/uploadHelpdesk/20150003EOD Error 27112014.png";
            //$files = glob($mke); // get all file names
      		if(file_exists($files)){
            	unlink($files); // delete file
                $V_update_att = "UPDATE `loan_cdb_helpdesk` SET `attachment_name`='' WHERE `helpid` = '".$getAttach."'";
    ...
    function isRemoveAttachmentsub($getAttach){
        $conn = DatabaseConnection();
        $sql_att = "SELECT `attachment_namesub` FROM `loan_cdb_helpdesk` WHERE `helpid` = '".$getAttach."';";
        $que_att = mysqli_query($conn,$sql_att);
        while($RES_att = mysqli_fetch_assoc($que_att)){
            ...
            //$files = glob($mke); // get all file names
      		if(file_exists($files)){
            	unlink($files); // delete file
                $V_update_att = "UPDATE `loan_cdb_helpdesk` SET `attachment_namesub`='' WHERE `helpid` = '".$getAttach."'";
                $que_update_att = mysqli_query($conn,$V_update_att) or die(GetFormattedException(mysqli_errno($conn),mysqli_error($conn)));
        	}else{
        	   /*$file = 'people.txt';
               $current = file_get_contents($file); // Open the file to get existing content
...
//..........................................................Helpdesk ID.....................................................................................
    ...
    function isHelpDeskHistory($helpID){
        $conn = DatabaseConnection();
        $sql_his = "SELECT `helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`, `attachment_name`, `caloser_by`, `caloser_dateTime`, `resulution`, `solution`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`, `entry_branch`, `entry_department`, `solved_by`, `solved_on`, `s_type`, `scat_code_3`,`asing_by`,`act_by`,`act_on`,`reOpen`,`ipAddress` FROM `loan_cdb_helpdesk` WHERE `helpid` = '".$helpID."';";
        $que_his = mysqli_query($conn,$sql_his);
        while($RES_his = mysqli_fetch_assoc($que_his)){
             $v_getSQL_insert_2 = "INSERT INTO `loan_cdb_helpdesk_his`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`, `attachment_name`, `caloser_by`, `caloser_dateTime`, `resulution`, `solution`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`, `entry_branch`, `entry_department`,`solved_by`,`solved_on`,`s_type`,`scat_code_3`,`asing_by`,`act_by`,`act_on`,`reOpen`,`ipAddress`)
