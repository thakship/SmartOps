...
<?php
$sql_file_count = "SELECT COUNT(`doc_number`) FROM `doc_line_file_stack` WHERE `box_number`='$boxNumber'";
$quary_file_count = mysqli_query($conn,$sql_file_count);
while ($rec_file_count = mysqli_fetch_array($quary_file_count)){
	if($rec_file_count[0] == 0){
?>
		<table border="1" id="myTable"  style="width:840px;background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
        	<tr class="tbl1">
             ...
             <tr style="background:#FFFFFF;">
                ...
                <td style="width:150px;"><input style="width:150px;" type="text" name="txtb1" id="txtb1" value=""  onKeyPress="return disableEnterKey(event)" required/></td>
...
<?php			
		...
		$sql_file_list = "SELECT MIS.`doc_number` AS DOCUMENT_NO,
							...
							(SELECT 'checked' FROM `doc_line_file_stack` WHERE `doc_type` = 'SD' AND `box_number` = '$boxNumber' AND `doc_line_file_stack`.`doc_number` =  MIS.`doc_number`)  AS SD
								...
								WHERE `box_number` = '$boxNumber') MIS";

		//$sql_file_list = "SELECT `box_number`, `doc_line_file_stack`.`doc_number`,`doc_name`, `doc_type`, `action_stat` FROM `doc_line_file_stack`,`doc_line_doc_mast` WHERE `doc_line_doc_mast`.`doc_number` = `doc_line_file_stack`.`doc_number` AND `box_number`='$boxNumber' order by `doc_line_file_stack`.doc_number";
        ...
        <tr class="tbl1">
                ...
                <td style="width:350px;">Document Name</td>
		...
		while ($rec_file_st_count = mysqli_fetch_array($sql_file_list_ex )){
			...
			<tr style="background:#FFFFFF;">
				...
				<td style="width:150px;"><input style="width:150px;" type="text" name="<?php echo "txtb".$index; ?>" id="<?php echo "txtb".$index; ?>" value="<?php echo $rec_file_st_count[0]; ?>"  onKeyPress="return disableEnterKey(event)" required/></td>
				...
				<td style="width:100px;"><input type="checkbox" name="<?php echo "txtd".$index; ?>" id="<?php echo "txtd".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)" onclick="checkValidate(this.id)" <?php if($rec_file_st_count[2]=='checked') echo "checked='checked'"; ?>/></td>
				<td style="width:100px;"><input type="checkbox" name="<?php echo "txte".$index; ?>" id="<?php echo "txte".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)" onclick="checkValidate(this.id)" <?php if($rec_file_st_count[3]=='checked') echo "checked='checked'"; ?>/></td>
                <td style="width:90px;"><img src="../../../img/dele.png" style=" width:15px;" onclick="deleteRow(this)"/></td>
             </tr>	
                 	<?php
	} ?>
  	   <tr style="background:#FFFFFF;">
                <td style="width:50px;"><input style="width:50px;" type="text" name="<?php $index = $index + 1; echo "txta".$index; ?>" id="<?php echo "txta".$index; ?>" value="<?php echo $index; ?>"  onKeyPress="return disableEnterKey(event)" readonly required/></td>
                <td style="width:150px;"><input style="width:150px;" type="text" name="<?php echo "txtb".$index; ?>" id="<?php echo "txtb".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)" required/></td>
                <td style="width:350px;"><div id="<?php echo "gettext".$index; ?>"><input style="width:350px; color:#999999;" type="text" name="<?php echo "txtc".$index; ?>" id="<?php echo "txtc".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)" required readonly="readonly"/></div></td>
                <td style="width:100px;"><input type="checkbox" name="<?php echo "txtd".$index; ?>" id="<?php echo "txtd".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)" onclick="checkValidate(this.id)" checked="checked"/></td>
                <td style="width:100px;"><input type="checkbox" name="<?php echo "txte".$index; ?>" id="<?php echo "txte".$index; ?>" value=""  onKeyPress="return disableEnterKey(event)"  onclick="checkValidate(this.id)" checked="checked"/></td>
                <td style="width:90px;"><img src="../../../img/dele.png" style=" width:15px;" onclick="deleteRow(this)"/></td>
             </tr>
	</table>
	<table>
