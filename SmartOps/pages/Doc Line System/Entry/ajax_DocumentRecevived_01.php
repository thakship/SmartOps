<?php
	...
	$docType = $_REQUEST['txt2'];
	//echo $docNumber." -- ".$docType;
	include('../../../php_con/includes/db.ini.php');
	if($docType != "BO"){
		$sql_Doc_get = "SELECT `box_number`,`doc_number`,`doc_type` FROM `doc_line_file_stack` WHERE `doc_number` = '$docNumber' AND `doc_type`='$docType' AND `action_stat`='RF'";
	}else{
		$sql_Doc_get = "SELECT `box_number`,`doc_number`,`doc_type` FROM `doc_line_file_stack` WHERE `doc_number` = '$docNumber' AND `action_stat`='RF'";
	}
...
?>
        ...
        <td style="width:100px;"><input style="width:200px; color:#999999;" type="text" name="<?php echo "txtd".$index; ?>" id="<?php echo "txtd".$index; ?>" value="<?php echo $rec_Doc_get[2]; ?>"  onKeyPress="return disableEnterKey(event)" required readonly="readonly"/></td>
