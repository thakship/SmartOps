...
Author			: Rizvi Kareem (Cloned from the ocument Request - Forward)
Date & Time		: 4:08 PM 26/01/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="doc/e/004/b";
...
<select name="selBoxMast" style=" font-family:Arial, Helvetica, sans-serif; border: 1px solid #000000;" id="selBoxMast" onKeyPress="return disableEnterKey(event)" onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)">
 ...
 <label>The grid will show  only the 10 boxes data at a time.  </label>
 <br/><br/>
 <div id ="tblgried">
...
<?php
	$sql_Doc_get = "SELECT BR.`branchName`, dh.`box_number`, dh.`doc_number`, dh.`doc_type`, MAX(dh.`action_date_time`), PR.`parpas_value`,dh.`action_user`
...
WHERE dh.`box_number` = ds.`box_number`
  ...
  AND dh.`branch_Numbar` = BR.`branchNumber`
  AND dh.`branch_Numbar` = '".$_SESSION['userBranch']."'
  AND (ds.APPR_BY = '' OR ds.APPR_BY = 'N')
  AND dh.`perpas_code` = PR.`perpas_code`
  /*AND dh.`box_number`  IN  (select mis.* from  (select mid.* from (SELECT DISTINCT `box_number` FROM `doc_line_file_stack` WHERE `action_stat`='RQ' order by box_number) mid limit 10) mis)  */
  AND dh.`action_date_time` >=   (SELECT MIN(dhs.`action_date_time`) FROM `doc_line_stat_history` dhs,`doc_line_file_stack` dsh,`branch` BRh, `doc_line_perpase_requst` PRs
  /*17-10-2017 comm : AND dh.`action_date_time` =   (SELECT max(dhs.`action_date_time`) FROM `doc_line_stat_history` dhs,`doc_line_file_stack` dsh,`branch` BRh, `doc_line_perpase_requst` PRs*/ 
WHERE dhs.`box_number` = dsh.`box_number`
  ...
  AND dhs.`branch_Numbar` = '".$_SESSION['userBranch']."'
  AND (dsh.APPR_BY = '' OR dsh.APPR_BY = 'N')
  AND dhs.`perpas_code` = PRs.`perpas_code`)  
GROUP BY ds.`doc_number`
   LIMIT 20;";
  //echo $sql_Doc_get;
...
?>
        ...
        <td style="width:200px;">
           <!-- <img src="../../../img/dele.png" title="Row Delete" style=" width:15px;" onclick="deleteRow(this)"/>-->
            <img src="../../../img/dele.png" title="<?php echo $index; ?>" style=" width:15px;" onclick="prossessUndo(title)"/>
        </td>
        </tr>
<?php
	}
...
<input type="button" style="font-size:12px;" id="btnClose" name="btnClose" value="CloseX" onclick="pageClose()"/>
<?php
		...
		try{
			if(trim($_POST['selBoxMast']) == "NULL"){
				if(trim($_POST['txtrow'])!=0){
					$DocCouNum = "";
					 ...
					 for ($i = 1; $_POST['txtrow']>= $i; $i++){		//For each box
						if(isset($_POST['txte'.$i])){
							...
							$sql_FileStacUpdate = "UPDATE `doc_line_file_stack` SET `APPR_BY`='".$_SESSION['user']."',`APPR_ON`=NOW() WHERE `box_number`='".trim($_POST['txtc'.$i])."' AND `doc_number`='".trim($_POST['txtd'.$i])."' AND `doc_type`='".trim($_POST['txtf'.$i])."' AND `action_stat`='RQ'";
							...
							$sql_InsertStackHistory ="INSERT INTO `doc_line_stat_history`(`box_number`,`doc_number`, `doc_type`, `action_date_time`, `action_stat`, `action_user`) VALUES ('".trim($_POST['txtc'.$i])."','".trim($_POST['txtd'.$i])."','".trim($_POST['txtf'.$i])."',now(),'RQ','".$_SESSION['user']."')";						
					...
					mysqli_commit($conn);
					$getmail = "upul@cdb.lk"; //GetParamValue(2,$conn);
					if($getmail != ""){
						...
						for ($i = 1; $_POST['txtrow']>= $i; $i++){		//For each box
							if(isset($_POST['txte'.$i])){
								...
								$mess2 = $mess2."<tr>		<td style='width:40px;text-align:left;padding-left:5px'>".$i."</td>
						...
						$headers .= 'Cc: rizvi.kareem@cdb.lk' . "\r\n";
						sendMailNuw($getmail,$title,$message,$headers);
						//mail($emailaddress, $emailsubject, $msg, $headers); 
					}else{
						echo "<script> alert('Null'); </script>";
					}
					echo "<script> alert('Request completed successfully !.');pageClose();</script>";
				...
				date_default_timezone_set('Asia/Colombo');
				$sql_InsertStackHistory1 ="INSERT INTO `doc_line_stat_history`(`box_number`,`doc_number`, `doc_type`, `action_date_time`, `action_stat`, `action_user`) SELECT `box_number`,`doc_number`,`doc_type`,now(),`action_stat`,'".$_SESSION['user']."' FROM `doc_line_file_stack` WHERE `box_number`='".trim($_POST['selBoxMast'])."'  AND  (`action_stat`='RQ' OR `action_stat`='ST')";
				//echo sql_InsertStackHistory1;
				...
				// Update the box master
				$sql_FileStacUpdate1 = "UPDATE `doc_line_file_stack` SET `APPR_BY`='".$_SESSION['user']."',`APPR_ON`=NOW() WHERE `box_number`='".trim($_POST['selBoxMast'])."' AND (`action_stat`='RQ' OR `action_stat`='ST')";
				...
				mysqli_commit($conn);
				$getmail = 'upul.cdb.lk'; //Must defined in the Parameter
				if($getmail != ""){
				
					$title = "CITIZENS DEVELOPMENT BUSINESS FINANCE PLC - First Level Document Request";
					$message = "<html>
								<head>
								<title>HTML email</title>
								</head>
