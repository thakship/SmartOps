<?php
	...
	if($docType != "BO"){
		$sql_Doc_get = "SELECT `box_number`,`doc_number`,`doc_type` FROM `doc_line_file_stack` WHERE `doc_number` = '$docNumber' AND `doc_type`='$docType' AND `action_stat`='ST'";
	}else{
		$sql_Doc_get = "SELECT `box_number`,`doc_number`,`doc_type` FROM `doc_line_file_stack` WHERE `doc_number` = '$docNumber' AND `action_stat`='ST'";
	}
