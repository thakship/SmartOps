<?php
    ...
    function is_update_ok_function($get_ksrl_helpid,$get_chk_1,$get_chk_2,$get_chk_3,$get_chk_4,$get_chk_5,$get_chk_6,$get_ksrl_user,$get_ksrl_pn){
        ...
        try{
            // Validation rule engine
            date_default_timezone_set('Asia/Colombo');
            $sql_s = "SELECT s.ssb_app_number FROM cdb_helpdesk AS s WHERE s.helpid = '".$get_ksrl_helpid."';";
            ...
            if($fac != ""){
                     $sql_update = "UPDATE cdb_helpdesk AS chd
                                    SET chd.ssb_type = 'CDBiNet Account is Created', 
                                        chd.lastactivityon = now() ,
                                        chd.facno = '".$fac."'
                    ...
                    while($rec_select_note_count = mysqli_fetch_array($query_select_note_count)){
                        ...
                        $sql_note_update = "INSERT INTO `cdb_help_note`(`helpid`, `note_code`, `note_discr`, `enterBy`, `enterDateTime`,`NoteTag`) 
                                        VALUES (".$get_ksrl_helpid.",'".$cou."',CONCAT('CDBiNet Account is Created on ', now()),'".$get_ksrl_user."',now(),'FILE_VERIFIED');";
                        $query_note_update = mysqli_query($conn,$sql_note_update) or die(mysqli_error($conn));
                    }
                    
                    /*$sql_cou_cycal = "SELECT `ssb_cycle`,`GSMNO`,`issue`,`ssb_facility_amount` 
                                        FROM `cdb_helpdesk` ,`user` 
                                       WHERE `cdb_helpdesk`.`helpid` = '".$get_ksrl_helpid."' AND 
                                              `user`.`userName` = `cdb_helpdesk`.`enterBy`;"; */                   //
                    
                     $sql_cou_cycal =   "SELECT h.ssb_cycle, 
                                                u.GSMNO, 
                                                h.issue, 
                                                h.ssb_facility_amount , 
                                                s.C_Moble , 
                                                s.C_Client_name
                                        FROM cdb_helpdesk AS h ,user AS u  ,cdb_ssb AS s
                                        WHERE h.helpid = '".$get_ksrl_helpid."' AND 
                                              h.enterBy = u.userName AND
                                        		h.helpid = s.helpid;";
                    ...
                    while($res_cou_cycal = mysqli_fetch_array($query_cou_cycal)){
                        $mail_address = $res_cou_cycal[2];
                        $Cname = $res_cou_cycal[5];
                        if ( substr($res_cou_cycal[1],0,2) =='07'){
                            
                            $sql_sms = "INSERT INTO `CDB_SMS_INBOX`(`mobileno`, `message`, `idatetime`, `module`, `uevent`, `sent`) 
                                                        VALUES ('".$res_cou_cycal[1]."',CONCAT('CDBiNet Account is Created..".chr(10)."Ref :".$get_ksrl_helpid." ".chr(10).$get_ksrl_pn.chr(10)."File : ".$res_cou_cycal[2].chr(10)."Amount : ".$res_cou_cycal[3].chr(10)."', now()),now(),'HELPDESK','PENDINGNOTE',0);";
                    ...
                    if($mail_address != ""){
                         ...
                         $to = $mail_address;
                         $subject = "CDB : CDBiNet Account is Created!";
...
<body>
<span>Dear ".$Cname.",</span><br /><br />
<span>We warmly welcome you as a valued customer of Citizens Development Business Finance PLC (CDB).</span> <br /><br />
<span>Thank you for choosing us as your financial institution and we are committed to provide our 
customers with the highest level of services and most innovative financial service solutions. 
Our revolutionized e-finance platform – CDBiNet offers the following facilities;</span> <br />

<ul>
  <li>Open Online Savings accounts</li>
  <li>Open Online Fixed Deposits</li>
  <li>Facebook Friends fund transfers</li>
  <li>Online Utility bill payment</li>
  <li>Online Fund transfers</li>
  <li>Online Lending services</li> 
</ul>


<p>In order to serve you better with an upgraded 
    services please visit the nearest CDB branch to avail yourself for below facilities. </p>
    
    <ul>
  <li>Deposit and withdraw money over the counter from any CDB branch</li>
  <li>Access to an island wide branch and ATM network </li>
  <li>Ability to perform transactions at any VISA merchant or withdraw cash at all ATMs displaying the VISA trademark</li>
</ul>

<p>We take our responsibility of being 'Your Friend' very seriously and strive to meet your high expectations and aspirations always.  
Please see below for your savings accounts number;</p>

<span style='font-weight: bold;'>CDB Savings Account No: ".$fac." </span> <br /><br />
<span>Please do not hesitate to contact us on our 24X7 Hotline 011-7388388 for any inquiry. </span><br />
<span>Looking forward to a mutually beneficial relationship.</span>
<br /><br />

<span>Thank you.</span><br />
<span>Yours faithfully,</span><br />
<span style='font-weight: bold;'>CITIZENS DEVELOPMENT BUSINESS FINANCE PLC</span>
                        ...
                        $r = 1;
                         while($res_cou_cycal = mysqli_fetch_array($query_cou_cycal)){
                            if ( substr($res_cou_cycal[1],0,2) =='07'){
                                    ...
                                    $sql_sms = "INSERT INTO `CDB_SMS_INBOX`(`mobileno`, `message`, `idatetime`, `module`, `uevent`, `sent`) 
                                                    VALUES ('".$res_cou_cycal[1]."',CONCAT('CDPU alert!".chr(10)."Account Created".chr(10)."Ref : ".$get_app_ksrl_helpid.chr(10)."Applicetion : ".$get_appNUmber.chr(10)."File : ".$res_cou_cycal[2].chr(10)."Amount : ".$res_cou_cycal[3].chr(10)."', now()),now(),'HELPDESK','PENDINGNOTE',0);";
                                    //$query_sms =  mysqli_query($conn,$sql_sms) or die(GetFormattedException(mysqli_errno($conn),mysqli_error($conn)));                                                    
                            }
                         }
                    }
    ...
    <tr>
        ...
        <td style='width:400px; text-align:left; padding-left:5px'>".$get_appNUmber."</td>
                        ...
                        $sql_cou_cycal =   "SELECT h.ssb_cycle, 
                                                ...
                                                s.supacc
                                        FROM cdb_helpdesk AS h ,user AS u  ,cdb_ssb AS s
                                        WHERE h.helpid = '".$get_app_ksrl_helpid."' AND 
                                              h.enterBy = u.userName AND
                                        		h.helpid = s.helpid;";
...
<span style='font-weight: bold;'>CDB Savings Account No: ".$get_appNUmber." </span> <br />
<span style='font-weight: bold;'>Postal Code: ".$postalCd."</span> <br /><br />
<span>Please do not hesitate to contact us on our 24X7 Hotline 011-7388388 for any inquiry. </span><br />
<span>Looking forward to a mutually beneficial relationship.</span>
<br /><br />
