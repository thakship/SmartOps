...
Page Name		: User Authorization
Purpose			: To authorization for Acting Role and Transfer Request 
Author			: Madushan Wikramaarachchi
Date & Time		: 10.19 A.M 18/03/2016
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/018";
...
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/user_authorization.php?DispName=User%20Authorization','conectpage');
}
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable" style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
    ...
    <?php
     ...
     car.done_by, 
     car.done_on,
     (select brn.branchName from branch brn where brn.branchNumber = car.br_code limit 1) as curbrnname
FROM cdb_acting_user_role AS car , user AS u , cdb_acting_roles AS cr 
WHERE car.user_id = u.userName AND
    car.req_user_role = cr.role_id AND
    car.req_user_role NOT IN ('PWO','CERUSE','FDACEO','SAIOFF')  AND
    car.auth_by = '' AND
    car.auth_on = '0000-00-00 00:00:00' AND
    car.status = 0;";
    ...
    <?php
        ...
        if(mysqli_num_rows($que_get_gried) != 0){
            ...
            while($rec_get_gried = mysqli_fetch_array($que_get_gried)){
               ...
               echo "<td style='width:80px;'>
               <img src='../../../img/ok.png' style='border: solid 1px #000000; margin-top:3px;' title='".$rec_get_gried[9]."' onclick='isApprove_tr(title);' />&nbsp;&nbsp;
               <img src='../../../img/dele.png' style='border: solid 1px #000000; margin-top:3px;' title='".$rec_get_gried[9]."' onclick='isReject_tr(title);' />&nbsp;&nbsp;
               </td>";
               echo "</tr>"; 
            }
        }else{
