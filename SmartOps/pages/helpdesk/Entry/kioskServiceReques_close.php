...
Page Name		: Kiosk - Completed Files
Purpose			: To viwe Request kiosk list for Service
Author			: Madushan Wikramaarachchi
Date & Time		: 03.04 A.M 16/09/2015
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/009";
...
<title>Kiosk - Completed Files</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/kioskServiceReques_close.php?DispName=Kiosk%20-%20Completed%20Files','conectpage');
}
function pageRef(){
...
function getNICGrid(){
    ...
    var scat2 = document.getElementById('sel_scat02').value;
    var scat3 = document.getElementById('sel_scat03').value;
	...
	mydata.open('GET','../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php'+'?get_cnic='+cnic+'&getscat2='+scat2+'&getscat3='+scat3,true);
	mydata.send();
}

function getScat3Dtl(){
   
    var getCat = document.getElementById('sel_scat02').value;
    //alert(getCat);
    var getTitle = 4;
     var mydata;
        mydata= new XMLHttpRequest();
        mydata.onreadystatechange=function(){
            if(mydata.readyState==4){
                document.getElementById('sp_3').innerHTML = mydata.responseText;
...
<table>
  <tr>
    ...
    <td>
        ...
        <select class="box_decaretion"  style="width: 200px;" name="sel_scat02" id="sel_scat02" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" onchange="getScat3Dtl()">
                <option value="">--Select Sub Catagory 2--</option>
            
        <?php
            $sql_load = "SELECT s2.scat_code_2 , s2.scat_discr_2 FROM scat_02 AS s2 WHERE s2.scat_code_1 = '101401';";
             $que_load = mysqli_query($conn , $sql_load) or die(mysqli_error($conn));
             while($rec_load = mysqli_fetch_array($que_load)){
                echo "<option value='".$rec_load[0]."'>".$rec_load[1]."</option>";   
             }
...
<?php
 $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` ,
                             ...
                             DATE(`cdb_helpdesk`.`caloser_dateTime`) = DATE(NOW()) AS is_old ,
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`cat_code` = '1014' AND
       DATE(`cdb_helpdesk`.`caloser_dateTime`) = DATE(NOW()) AND
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
       ORDER BY `cdb_helpdesk`.`caloser_dateTime` desc, `cdb_helpdesk`.`helpid`;";
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        echo "<td style='width:100px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[19]."</span></td>";
