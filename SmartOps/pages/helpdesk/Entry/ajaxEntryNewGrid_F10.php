<?php
    ...
    $FromTables = "FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`";       
     ...
     WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
		   ...
		   `cdb_helpdesk`.`helpid` NOT IN (select cdb_ssb_his.helpid from cdb_ssb_his where cdb_ssb_his.P_V = 'V') AND
           ...
           `cdb_helpdesk`.`ssb_app_number` = ''  AND  
           date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 30 DAY)  AND ". $vsqlAux_1.$vsqlAux_2.$vsqlAux_4." ORDER BY `cdb_helpdesk`.`helpid`;";       
	...
	echo "<h3>Total record(s) - QC Pending : ". $rec_det_detelsCNT[0]. "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font style='color:red; font-size:14px;'>[ Not Assigned : ".$rec_det_detelsCNT[1]." ]</font></h3>";
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cat_code` = '1014' AND `scat_02`.scat_code_1 = '101401' AND 
       `cdb_helpdesk`.`ssb_app_number` = '' AND  
       date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 30 DAY)  
	   AND `cdb_helpdesk`.`helpid` NOT IN (select cdb_ssb_his.helpid from cdb_ssb_his where cdb_ssb_his.P_V = 'V')
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
			...
			<td style="width:70px;text-align: right;"><span style="margin-right: 5px;">Approval</span></td>
            ...
            <td style="width:80px;text-align: right;"><span style="margin-right: 5px;">Int Rate</span></td>			
			...
			<td style="width:70px;text-align: right;"><span style="margin-right: 5px;">Minutes Elapsed(F8 Logic)</span></td>
...
<?php
 ...
 ,(if(`cdb_helpdesk`.`ssb_type` like 'Pending Notified%' , 0, ROUND(time_to_sec((TIMEDIFF(NOW(), `cdb_helpdesk`.lastactivityon))) / 60))),`cdb_helpdesk`.`mainFileType`,(select scat_03.scat_discr_3 from scat_03 where scat_03.cat_code = cdb_helpdesk.cat_code and scat_03.scat_code_1 = cdb_helpdesk.scat_code_1 and scat_03.scat_code_2 = cdb_helpdesk.scat_code_2 and scat_03.scat_code_3 = cdb_helpdesk.scat_code_3),`cdb_helpdesk`.`Intrest_Rate`".
 $FromTables."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       `cdb_helpdesk`.`scat_code_2` = `scat_02`.`scat_code_2` AND
       ...
       date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 30 DAY)  
	   AND `cdb_helpdesk`.`helpid` NOT IN (select cdb_ssb_his.helpid from cdb_ssb_his where cdb_ssb_his.P_V = 'V')
	   AND ". $vsqlAux_1.$vsqlAux_2.$vsqlAux_4." ORDER BY 18 desc;";
    ...
    $index = 0;
     ...
     while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
		...
		echo "<td style='width:70px;text-align: right;".$MyRow ."'><span style='margin-right: 2px;'>".$rec_det_detels[19]."</span></td>";
        echo "<td style='width:100px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[20]."</span></td>";
        echo "<td style='width:120px;text-align: right;".$MyRow ."'><span style='margin-right: 2px;'>".number_format($rec_det_detels[11], 2)."</span></td>";
		echo "<td style='width:80px;text-align: right;".$MyRow ."'><span style='margin-right: 2px;'>".$rec_det_detels[21]."</span></td>";
