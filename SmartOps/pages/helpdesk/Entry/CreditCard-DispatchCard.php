...
Page Name		: Credit Card - Dispatch Card 
Purpose			: To Dispatch Credit Card 
Author			: Madushan Wikramaarachchi
Date & Time		: 09.04 A.M 13/11/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/063";
...
<title>Credit Card - Dispatch Card</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/CreditCard-DispatchCard.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20Dispatch','conectpage');
}
function pageRef(){
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
        ...
        `cdb_helpdesk`.`COD_FILE_VERIFIED_BY` IS NOT NULL AND
         ...
         `cdb_helpdesk`.`COD_CHKLIST_BY` IS NULL AND
...
<table border="0" >
    <tr>
        <td style="text-align: left; vertical-align: bottom;">
            Search Courier Provider
        </td>
        <td style="vertical-align: bottom;">
            <select class='box_decaretion' name='sel_COU' id='sel_COU' onkeypress='return disableEnterKey(event)'  onfocus='hilightColoyr(this.id)' onblur='colourLeave(this.id)'>
                   <option value=''>--Select Courier --</option>
                  <?php
$v_sql_getCategory = "SELECT d.id , d.location_code , d.location , d.courier_company
                                           FROM credit_card_courier_dtl As d;";
$que_getCategory = mysqli_query($conn,$v_sql_getCategory);
while($RES_getCategory = mysqli_fetch_array($que_getCategory)){
    echo "<option value=".$RES_getCategory[3].">".$RES_getCategory[2]." - ".$RES_getCategory[3]."</option>";
}
?>
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11px; font-weight: bold;">
            ...
            <td style="width:190px;text-align: left;"><span style="margin-left: 5px;">Mail City</span></td>
            <td style="width:190px;text-align: left;"><span style="margin-left: 5px;">Card Number</span></td>
             <td style="width:190px;text-align: left;"><span style="margin-left: 5px;">Embossing Name</span></td>
            ...
            <td style="width:150px;text-align: right;"><span style="margin-right: 5px;">Courier Provider</span></td>
...
<?php
 $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` ,
                              ...
                              (SELECT w.mailing_city FROM world_line_dtl AS w WHERE w.application_sequence_no_1 = `cdb_helpdesk`.ssb_app_number GROUP BY w.application_sequence_no_1) AS MAIL_CITY,
			                  (SELECT w1.card_number FROM world_line_dtl AS w1 WHERE w1.application_sequence_no_1 = `cdb_helpdesk`.ssb_app_number GROUP BY w1.application_sequence_no_1) AS CARD_NUMBER,
			                  (SELECT  w2.embossing_name FROM world_line_dtl AS w2 WHERE w2.application_sequence_no_1 = `cdb_helpdesk`.ssb_app_number GROUP BY w2.application_sequence_no_1) AS EBN  
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        //----------------------------------------------------------------------------------------------------------------
            ...
            echo "<td style='width:190px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[22]."</span></td>";
            echo "<td style='width:190px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[23]."</span></td>";
            ...
            echo "<td style='width:150px;text-align: right;".$MyRow ."'>";
                   echo " <select class='box_decaretion'  style='width: 150px;' name='sel_catagory".$index."' id='sel_catagory".$index."' onkeypress='return disableEnterKey(event)'  onfocus='hilightColoyr(this.id)' onblur='colourLeave(this.id)'>
                            <option value=''>--Select Courier --</option>";
                   $v_sql_getCategory = "SELECT d.courier_company 
                                           FROM courier_coms As d
                                           WHERE d.stat = '1'
                                            ORDER BY d.courier_company";
                   $que_getCategory = mysqli_query($conn,$v_sql_getCategory);
                    while($RES_getCategory = mysqli_fetch_array($que_getCategory)){
                        echo "<option value=".$RES_getCategory[0].">".$RES_getCategory[0]."</option>";
                    }

            echo "</select></td>";

            echo "<td style='width:150px;'>
                    <input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='txta".$index."' onclick='clientValidate(title,".$rec_det_detels[10].");'/>
                    <input type='checkbox' class='buttonManage' id='chkbtn_Validate".$index."' name='chkbtn_Validate".$index."'/>
...
<br /><hr />
<input type='submit' class='buttonManage' id='btn_Submit' name='btn_Submit' value='Submit' />
...
if(isset($_POST['btn_Submit']) && $_POST['btn_Submit']=='Submit'){
    ...
    try{
        ...
        //echo $count_gried;
        for ($i = 1; $i <= $count_gried; $i++){ 
            if(isset($_POST['chkbtn_Validate'.$i])){
				//echo $_POST['txta'.$i];
                $noteDis = 'Card Center - Dispatch and Send Courier : '.$_POST['sel_catagory'.$i];
                $sql_update = "UPDATE cdb_helpdesk AS a
                                SET a.COD_CHKLIST_BY = '".$_POST['txt_USERMY']."' ,
                                    a.COD_CHKLIST_ON = NOW(),
                                    a.ssb_type = 'Card Center - Dispatch and Send Courier : ".$_POST['sel_catagory'.$i]."'
                                WHERE a.helpid = '".$_POST['txta'.$i]."';";
                $query_update = mysqli_query($conn,$sql_update) or  die(mysqli_error($conn));

                $sql_courier = "INSERT INTO credit_card_send_courier (helpid,courier_company,send_by)
                                      VALUES('".$_POST['txta'.$i]."','".$_POST['sel_catagory'.$i]."','".$_POST['txt_USERMY']."');";
                $query_courier = mysqli_query($conn,$sql_courier) or die(mysqli_error($conn));

                setNoteCreditCard($conn,trim($_POST['txta'.$i]),$noteDis,$_SESSION['user']);
                
                noteTbl($conn, $_POST['txta'.$i], $noteDis, $_SESSION['user']);
                 // ----------------------------------------------------------------------------------------------------------------------//              
                mysqli_commit($conn); 
        ...
        /*if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
           // echo "<script> alert('C');</script>";
                    //$getmail = 0;
           $StrCallAnswered = "Credit Evalution - File Approve";
            date_default_timezone_set('Asia/Colombo');
            $sql_update = "UPDATE cdb_helpdesk AS a
                            SET a.COD_CHG_REC_BY = '".$_POST['txt_USERMY']."' ,
                                a.COD_CHG_REC_ON = NOW(),
                                a.ssb_type = 'Credit Evalution - File Approve',
                                a.cmb_code ='5002', 
                                a.caloser_by = '".$_SESSION['user']."', 
                                a.caloser_dateTime = now(), 
                                a.resulution = '".$StrCallAnswered."', 
                                a.solution = '".$StrCallAnswered."', 
                                a.solved_by = '".$_SESSION['user']."',
                                a.solved_on = now()
                            WHERE a.helpid = '".$_POST['txt_helpdeskCredit']."';";
            ...
            $msg = "Credit Card Application Approved by Credit Evalution";
            noteTbl($conn, $_POST['txt_helpdeskCredit'], $msg, $_SESSION['user']);
             // ----------------------------------------------------------------------------------------------------------------------//              
            mysqli_commit($conn); 
            echo "<script> alert('Record Updated.');pageClose();</script>";      
...
if(isset($_POST['btn_Reject']) && $_POST['btn_Reject']=='Reject'){
   // echo "<script> alert('A');</script>";
    /*mysqli_autocommit($conn,FALSE);
    try{
            // Validation rule engine
