  ...
  var key = event.keyCode || event.which;
   ...
   // return;
     ...
     if(key == 112||key == 113){
		...
		if(key == 119 || key == 120){
        //alert("Key pressed credit_card_call_level01_F1.php " + key);
        var mydataNew;
		mydataNew= new XMLHttpRequest();
		mydataNew.onreadystatechange=function(){
			if(mydataNew.readyState==4){
        ...
        var userg = document.getElementById('txt_USERgROUP').value; 
		mydataNew.open('GET','credit_card_call_level01_Q1.php'+'?userg_CRditCC='+userg+"&FKey="+key+"&DispName=Q1",true);
		mydataNew.send();
			...
			/*	var mydataNew;
				...
				var userg = document.getElementById('txt_USERgROUP').value; 
				mydataNew.open('GET','credit_card_call_level01_InitSubmission.php'+'?userg_CRditCC='+userg+"&FKey="+key+"&DispName=Q1",true);
				mydataNew.send();
				*/
			}
		}
    ...
    if(key == 115){
        ...
        var userg = document.getElementById('txt_USERgROUP').value; 
		mydataNew.open('GET','credit_card_call_level01_Pending_NOT.php'+'?userg_CRditCC='+userg+"&FKey="+key,true);
		mydataNew.send();
    }
}
</script>
           ...
           //while($r_ex = mysqli_fetch_array($que_ex)){
                ...
                if($_POST['selCallAnswered'] == "NO"){ 
                    if(isset($_POST['chk_C'])){
                        ...
                        $StrCallAnswered = "Customer Call Answered : No ";
                        $strCallFeedback = "Feedback Inquiry : ".trim($_POST['selCallFeedbackNO']);
                        $DIS = "Call center  - ".$StrCallAnswered." : ".$strCallFeedback ." - ".trim($_POST['txtarea_Solution']);
                        setNoteCreditCard($conn,trim($_POST['get_help_ID']),$DIS,$_SESSION['user']);
                        CreateNoteTable($conn ,trim($_POST['get_help_ID']),$DIS,$_SESSION['user'],'CREDITC');
                   ...
                   // $StrCallAnswered = "Customer Call Answered : No ";
                    ...
                    $DIS = "Call center  - ".$StrCallAnswered." / ".$strCallFeedback;
                    setNoteCreditCard($conn,trim($_POST['get_help_ID']),$DIS,$_SESSION['user']);
                    CreateNoteTable($conn ,trim($_POST['get_help_ID']),$DIS,$_SESSION['user'],'CREDITC');
                }else{
        ...
        if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
           ...
           //if($rowcount == 3){
            if($rowcount == 2){
               ...
               $que_update_dis = mysqli_query($conn,$sql_upadte_dis) or die(mysqli_error($conn));
               $DIS = "Call center  - ".$StrCallAnswered." - ".$_POST['txtCommentCredit'] ;
            ...
            $NOTE = "Call center  - ".$Q." Customer Call Answered". " - ". $_POST['txtCommentCredit'] ;
