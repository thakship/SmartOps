...
Page Name		: Service Request List - PATPAT Branch
Purpose			: To viwe Request list for Service - PATPAT Branch
Author			: Madushan Wikramaarachchi
Date & Time		: 09.09 A.M 28/03/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/024";
...
<title>Service Request List - Patpat (Branch)</title>
...
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/serviceRequestList-PATPAT.php?DispName=Service%20Request%20List%20-%20PATPAT','conectpage');
    ...
    function clientValidate(title,cat_cod){
        ...
        var uGroup = document.getElementById('txt_USERgROUP').value;
        if(uGroup != 'ug00015'){
              helpClose = 1;
        }else{
             helpClose = 0;
        ...
        if(cat_cod != '1014' ){
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreqpatpat="+getHI+"&gettxt_helpdesk_closepatpat="+helpClose,true);
            mydata.send();
        }else{
            //alert(cat_cod);
            var mydata;
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `scat_02`.`scat_discr_2` = '".$_SESSION['userBranchName']."'  AND
 ...
 //echo $v_sql_det_detels;
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        //----- Madushan 2018-11-15 12:00 PM get Patpat User  ------------------------------
         if($rec_det_detels[13] == 102401){
               $sql_get_usernam = "SELECT u.userID FROM user AS u WHERE u.userName = '".$rec_det_detels[14]."';";
               ...
               if($rowcount != 0){
                    ...
                    $v_sel_scat03 = getScat_discr_3_Descreption($rec_det_detels[10],$rec_det_detels[13],$rec_det_detels[11],$rec_det_detels[14],$conn); // Get SCat Descrepion...........
               }
        ...
        echo "<td style='width:80px;text-align: right;".$MyRow ."' title = '".$asingBy."' onclick='columFunction(this)'><span style='margin-right: 2px;'>".$v_sel_scat03."</span></td>";
...
<?php
    ...
    function getScat_discr_3_Descreption($catCode,$sCode1,$sCode2,$sCode3,$conn){
         ...
         $v_sql_getscat_code_3 = "SELECT `scat_discr_3` FROM `scat_03` 
                                   WHERE `cat_code` = '".$catCode."' 
                                   AND `scat_code_1` = '".$sCode1."' 
                                   AND `scat_code_2` = '".$sCode2."' 
                                   AND `scat_code_3` = '".$sCode3."';";
