<?php
	...
	if(isset($_REQUEST['n_keyCode'])){
		$CALLCOUNT = $_REQUEST['n_keyCode'];
	}
	
    
    $v_sql_det_detelsCNT = "SELECT COUNT(distinct `cdb_helpdesk`.`helpid`),
	...
	FROM `cdb_helpdesk`,`callcenterinquiry`
    WHERE `cdb_helpdesk`.`cat_code` = '1014' AND `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
		...
		AND callcenterinquiry.helpid = `cdb_helpdesk`.`helpid` 						
		AND (SELECT  COUNT(*) FROM callcenterinquiry AS c WHERE c.callstatus = 0 AND c.call_answered = 'NO' AND c.helpid = `cdb_helpdesk`.`helpid`) = ".$CALLCOUNT."		
    ...
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
        echo "<h3>Total record(s) - CDB  : ". $rec_det_detelsCNT[0]. "&nbsp;&nbsp;<font style='color:red; font-size:14px;'>[For the Day : ".$rec_det_detelsCNT[1]."]</font>&nbsp;&nbsp;<font style='color:blue; font-size:14px;'>[Last 30 Days : ".$rec_det_detelsCNT[2]." ]</font><img src='../../../images/cdblogo.png' height='20' width='50'></h3>";
		echo "<font style='color:Green; font-size:14px;'>Follow-up - ".$CALLCOUNT." [F1 - 1st Follow-up] [F2 - 2nd Follow-up] [F3 - 3rd Follow-up]</font>";
    }
...
<?php
 ...
 $v_sql_det_detels = "SELECT distinct `cdb_helpdesk`.`helpid` , 
 ...
 FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`callcenterinquiry`
