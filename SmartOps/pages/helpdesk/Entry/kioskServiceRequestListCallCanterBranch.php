...
Page Name		: kiosk Service Request List - Call Canter (Branch)
Purpose			: To viwe comlited app request list Branch
Author			: Madushan Wikramaarachchi
Date & Time		: 09:56 A.M 26/02/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/050";
...
function isSubmit(title){
    ...
    if (r==true){
		...
		$.ajax({ 
			type:'POST', 
			data: {get_helpID_pro_update : title , get_user_pro_update : ksrl_user }, 
			url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function_callCenter.php', 
			success: function(val_retn) { 
			   //document.getElementById('maneSpan').innerHTML = val_retn;
			    alert(val_retn);
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:50px;">Priority Request</td>
        </tr>
<?php
      ...
      $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
                           ...
                           `cdb_helpdesk`.`priorCallRecOn` AS PCR
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`ssb_app_number` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
       `cdb_helpdesk`.`facno` <> '' AND
       `cdb_helpdesk`.`helpid` NOT IN (SELECT c.helpid
											FROM callcenterinquiry AS c
											WHERE c.callstatus = 1) AND
    ...
    //-------------------------------------
        ...
        */
        ...
        //echo "<td style='width:40px;text-align: left;' title ='".$rec_det_detels[16]."'><span style='margin-left: 5px;'>".str_pad($rec_det_detels[17], 2, '0', STR_PAD_LEFT)."</span></td>";
            ...
            if($rec_det_detels[27] == 0 ){
                echo "<td style='width:50px;'><input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='".$rec_det_detels[0]."' onclick='isSubmit(title);'/></td>";
            }else if($rec_det_detels[30] == ''){
                echo "<td style='width:50px;'><input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='".$rec_det_detels[0]."' onclick='isSubmit(title);'/></td>";
            } 
            else{
                echo "<td style='width:50px;'><input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='txta".$index."' onclick='' disabled='disabled'/></td>";
