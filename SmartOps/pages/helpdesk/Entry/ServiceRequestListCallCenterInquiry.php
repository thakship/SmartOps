...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/ServiceRequestListCallCenterInquiry.php?DispName=Service%20Request%20List%20-%20Call%20Center%20Inquiry','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/ServiceRequestListCallCenterInquiry.php?DispName=Service%20Request%20List%20-%20Call%20Center%20Inquiry','conectpage');
}
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`scat_code_1` = '102402' AND
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND
       `scat_02`.scat_discr_2  = '".$_SESSION['USERBRANCHNAME']."'
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `scat_02`.scat_discr_2  = '".$_SESSION['USERBRANCHNAME']."'
       ORDER BY `cdb_helpdesk`.`enterDateTime`;";
 // $_SESSION['userBranchName'] echo $v_sql_det_detels; 
