...
Page Name		: Credit Card - Card center List 
Purpose			: To viwe Credit Card Request for Card center List 
...
<title>Credit Card - Card center List </title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/credit_card_card_center_list.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20Card%20Center','conectpage');
}
function pageRef(){
...
function isEvelualelist(title){
   // alert(title);
      //var getsubHI = document.getElementById('txt_help_ID').value;
      var resTitle = title.split("|");
      var v_srting =  "R|"+title+"|Are you sure you want to Recomented this?";
      popup1(1,v_srting);
}
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5001' AND
        ...
        `cdb_helpdesk`.`COD_FILE_VERIFIED_BY` IS NULL AND
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       ...
       /* echo "<td style='width:40px;text-align: left;' title ='".$rec_det_detels[15].chr(10).$rec_det_detels[18].chr(10).$rec_det_detels[19].chr(10).$rec_det_detels[20]."'><span style='margin-left: 5px;'><img src='../../../img/".$rec_det_detels[14].".png'></span></td>";
        ...
        echo "<input type='button' style='width: 100px;' class='buttonManage' value='Approve' title='".$rec_det_detels[0]."' onclick='isEvelualelist(title);'/>";

        echo "<input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='txta".$index."' onclick='clientValidate(title,".$rec_det_detels[10].");'/></td>";
        echo "</tr>";
    }
?>
</table>
    ...
    try{
        ...
        if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
           // echo "<script> alert('C');</script>";
                    //$getmail = 0;
           $StrCallAnswered = "Card Center - File Approve";
            date_default_timezone_set('Asia/Colombo');
            $sql_update = "UPDATE cdb_helpdesk AS a
                            SET a.COD_FILE_VERIFIED_BY = '".$_POST['txt_USERMY']."' ,
                                a.COD_FILE_VERIFIED_ON = NOW(),
                                a.ssb_type = 'Card Center - File Approve'
            ...
            $msg = "Credit Card Application Approved by Card Center";
   ...
   // echo "<script> alert('A');</script>";
    ...
    try{
        ...
        if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
           // echo "<script> alert('C');</script>";
                    //$getmail = 0;
           $StrCallAnswered = "Card Center - File Reject";
            date_default_timezone_set('Asia/Colombo');
