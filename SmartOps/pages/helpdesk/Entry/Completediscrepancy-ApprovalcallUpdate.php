...
<?php
 ...
 WHERE `cdb_helpdesk`.`cat_code` = '1014' AND `cdb_helpdesk`.`cmb_code` = '5002' AND
       ...
       `cdb_helpdesk`.`helpid` = c.helpid AND
        ...
        c.d_call_chk = 1 ";
 	  // echo $v_sql_det_detelsCNT; d_call_ans_entryBy
...
<?php
      ...
      $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
                           ...
                           (SELECT cq.brn_reply_remark FROM callcenterinquiry AS cq WHERE ((cq.callstatus = 1 AND cq.call_feedback = 2) OR (cq.callstatus = 1  AND cq.call_answered = 'NO' and cq.callFeedback in ('Invalid Number','No such person','No contact number'))) AND cq.helpid = `cdb_helpdesk`.`helpid`) AS CALLEN_ID ,
                           (SELECT f.inquiry_id
                                    	FROM callcenterinquiry AS f
                                    	WHERE ((f.callstatus = 1 AND f.call_feedback = 2 AND f.brn_reply_by <> '') OR (f.callstatus = 1  AND f.call_answered = 'NO' and f.callFeedback in ('Invalid Number','No such person','No contact number'))) AND f.helpid = `cdb_helpdesk`.`helpid`) AS inquiry_id ,
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       (( c.callstatus = 1  AND c.call_feedback = 2 AND brn_reply_by <> '') OR (c.callstatus = 1  AND c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number'))) and 
        c.d_call_chk = 1  AND
	    date(`cdb_helpdesk`.`callRecOn`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND  
