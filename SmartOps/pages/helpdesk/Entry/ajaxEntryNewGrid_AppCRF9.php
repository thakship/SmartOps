<?php
    ...
    $FromTables = "FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`";       
     ...
     // Change Request by Arshan 2019-01-23 Develop by madushan
    ...
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
		echo "<h3>Sanction Stage - Pending | Total record(s) : ". $rec_det_detelsCNT[0]. " </h3>";
    }
?>	
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
			...
			<td style="width:70px;text-align: right;"><span style="margin-right: 5px;">FACILITY AMOUNT</span></td>
...
<?php	// Print the group wise pending info
	...
	$v_sql_det_detelsGR =  "SELECT `cdb_helpdesk`.`assign_to`,
	 ...
	 while($rec_det_detelsGR = mysqli_fetch_array($v_que_det_detelsGR)){
		 ...
		 echo "<td style='width:70px;text-align: right;'><span style='margin-right: 2px;'>".$rec_det_detelsGR[3]."</span></td></TR>";
		 //echo "<td style='width:70px;text-align: right;'><span style='margin-right: 2px;'>".$rec_det_detelsGR[4]."</span></td>"; 
		 $COUNT_1 = $COUNT_1 + $rec_det_detelsGR[3];
	...
	echo "<TR><td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>Total</span></td>";
		 ...
		 echo "<td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>".$COUNT_2."</span></td></TR>";
		 //echo "<td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>".$COUNT_3."</span></td>";	 
?>
</table>
</div>
...
<?php	// Print the group wise pending info
	$COUNT_1 = 0; $COUNT_2 = 0; $COUNT_3 = 0;
    $TOTAL = 0; $TOTAL_BRANCH = 0; $TOTAL_HO = 0; $TOTAL_UCL = 0;
	$v_sql_det_detelsGR =  "SELECT branch.CDBZONE,
	 ...
	 while($rec_det_detelsGR = mysqli_fetch_array($v_que_det_detelsGR)){
		 ...
		 $TOTAL = $TOTAL + $rec_det_detelsGR[2];
		 $TOTAL_BRANCH = $TOTAL_BRANCH + $rec_det_detelsGR[5];
		 $TOTAL_HO = $TOTAL_HO + $rec_det_detelsGR[6];
		 $TOTAL_UCL = $TOTAL_UCL + $rec_det_detelsGR[7];
		 $COUNT_2 = $COUNT_2 + $rec_det_detelsGR[3];
		 $COUNT_3 = $COUNT_3 + $rec_det_detelsGR[4];
	 } 
	echo "<TR><td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>Total</span></td>";
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
			...
			<td style="width:150px;text-align: left;"><span style="margin-left: 5px;">LOS In</span></td>
            <td style="width:70px;text-align: right;"><span style="margin-right: 5px;">LOS Group</span></td>
...
<?php 
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
	   ...
	   `cdb_helpdesk`.`taken_by` ='  - Pending' AND
	    ...
	    date(`cdb_helpdesk`.`appcrdate`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
       ORDER BY 27"; /*`cdb_helpdesk`.`ssb_type`, cdb_helpdesk.lastactivityon desc ,`cdb_helpdesk`.`appcrdate` desc;*/
    ...
    $index = 0;
     ...
     while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
        ...
        */
        ...
        echo "<td style='width:80px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[28]."</span></td>";		
		...
		echo "<td style='width:80px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[19]."</span></td>";		
