...
Page Name		: Online Account Request List
Purpose			: To viwe Request Online Account Request List 
Author			: Madushan Wikramaarachchi
Date & Time		: 5:26 PM 21/12/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/031";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/OnlineAccountRequestList.php?DispName=Online%20Account%20Request%20List','conectpage');
}
function pageRef(){
   ...
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/OnlineAccountRequestList.php?DispName=Online%20Account%20Request%20List','conectpage');
    ...
    function clientValidate(title,cat_cod){
        ...
        /*if(cat_cod != '1032' ){
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_ONLINE.php"+"?getHelpIDreq1="+getHI+"&gettxt_helpdesk_close1="+helpClose,true);
            mydata.send();
        //}
    ...
    function getSolution(){
        var getsubHI = document.getElementById('txt_help_ID').value;
            ...
            mydata1.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_ONLINE.php"+"?getsubHelpIDreq="+getsubHI,true);
            mydata1.send();
    }
       ...
       if(document.getElementById('chk_1').checked == true && document.getElementById('chk_2').checked == true && document.getElementById('chk_3').checked == true && document.getElementById('chk_4').checked == true && document.getElementById('chk_5').checked == true && document.getElementById('chk_6').checked == true){
        //alert('all check');
            $.ajax({ 
    				...
    				url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function_ONL.php', 
...
function isAppGenerater(companyname){
    ...
    }else if(companyname == "CDB" && appNUmber.length != 18){
        alert('Invalid Account Number - length : 18.');
    }else if(companyname == "UCL" && appNUmber.length != 18){
        alert('Invalid Account Number - length : 18.');
    }else if(appNUmber == ""){
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5001' AND
        `cdb_helpdesk`.`cat_code` = '1032' AND
...
<table style="width: 100%;" border="0" >
    <tr>
        ...
        <td style="text-align: right; vertical-align: bottom;">
            <?php
                $query_Count_authPending = mysqli_query($conn , "select COUNT(*) FROM cdb_helpdesk AS c WHERE c.cmb_code = '5000' AND c.cat_code = '1032';");
