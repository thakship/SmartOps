...
Page Name		: LOS Queue
Purpose			: To viwe LOS Queue
Author			: Madushan Wikramaarachchi
Date & Time		: 13.30 P.M 03/07/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/052";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/LOSQueue.php?DispName=LOS%20Queue','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/LOSQueue.php?DispName=LOS%20Queue','conectpage');
}
    ...
    function clientValidate(title,cat_cod){
        ...
        var helpClose = document.getElementById('txt_helpdesk_close').value;
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreq1los="+getHI+"&gettxt_helpdesk_close1los="+helpClose,true);
            mydata.send();
    ...
    var r = confirm('Confirm to Update ?')
            if (r==true){
    			...
    			$.ajax({ 
    				...
    				type:'POST', 
    				data: {get_assuserlos : assuser  , get_aass_ksrl_helpidlos : ksrl_helpid}, 
    				url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				    //alert()
    				   document.getElementById('maneSpan').innerHTML = val_retn;
...
<?php
    ...
    FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `cmb_states`, `branch`, `deparment`,`cdb_help_user_oth` , `cdb_ssb`
...
<table style="width: 100%;" border="0" >
    <tr>
        ...
        <td style="text-align: right; vertical-align: bottom;">
            <?php
                $query_Count_authPending = mysqli_query($conn , "select COUNT(*) FROM cdb_helpdesk AS c WHERE c.cmb_code = '5000' AND c.cat_code = '1014';");
...
<?php
 ...
 FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth` ,  `cdb_ssb`
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5001' AND
       `cdb_helpdesk`.`cat_code` = '1014' AND
       (`cdb_helpdesk`.`ssb_app_number` = '' || ( `cdb_helpdesk`.`ssb_app_number` <> '' AND `cdb_helpdesk`.`curr_stage` = '')) AND 
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        echo "<td style='width:90px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[24]."</span></td>";
       ...
       /* echo "<td style='width:40px;text-align: left;' title ='".$rec_det_detels[15].chr(10).$rec_det_detels[18].chr(10).$rec_det_detels[19].chr(10).$rec_det_detels[20]."'><span style='margin-left: 5px;'><img src='../../../img/".$rec_det_detels[14].".png'></span></td>";
        ...
        if ($rec_det_detels[23]>30){   // $rec_det_detels[17] == From Last verified time 3:22 PM 15/06/2017
			echo "<td style='width:70px;text-align: right;".$MyRow ."' bgcolor='#f99995' title='From Last Verified: ".$rec_det_detels[22]."'><span style='margin-right: 2px;'>".$rec_det_detels[23]."</span></td>";
		}
		elseif($rec_det_detels[23]>15){ // $rec_det_detels[17] == From Last verified time 3:22 PM 15/06/2017
			echo "<td style='width:70px;text-align: right;".$MyRow ."' bgcolor='#e8ff42' title='From Last Verified: ".$rec_det_detels[22]."'><span style='margin-right: 2px;'>".$rec_det_detels[23]."</span></td>";
		}else
			echo "<td style='width:70px;text-align: right;".$MyRow ."'  bgcolor='#d9ff87' title='From Last Verified: ".$rec_det_detels[22]."'><span style='margin-right: 2px;'>".$rec_det_detels[23]."</span></td>";
