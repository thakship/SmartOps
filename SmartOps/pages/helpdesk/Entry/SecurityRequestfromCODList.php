...
Page Name		: Security Request from COD List
Purpose			: To viwe Request list for Security Request from COD
Author			: Madushan Wikramaarachchi
Date & Time		: 15.26 P.M 08/01/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/034";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/SecurityRequestfromCODList.php?DispName=Security%20Request%20from%20COD%20List','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/SecurityRequestfromCODList.php?DispName=Security%20Request%20from%20COD%20List','conectpage');
}
...
function is_getScat_01(){
    ...
    if(cat1 == ""){
        ...
        mydata.onreadystatechange=function(){
            if(mydata.readyState==4){
                document.getElementById('diva22').innerHTML=mydata.responseText;       
            }
        }
        mydata.open("GET","ajax_SecurityRequestfromCODList.php"+"?getCatagorySetletment="+cat1,true);
        mydata.send();
    }
...
function getGried(){
    var cat1 = document.getElementById('sel_catagory').value;
    var scat1 = document.getElementById('sel_scat01').value;
    var txt_USERgROUP = document.getElementById('txt_USERgROUP').value;
    var USERBRANCHNAME = document.getElementById('USERBRANCHNAME').value;
    ...
    mydata.open("GET","ajax_SecurityRequestfromCODList.php"+"?cat1="+cat1+"&scat1="+scat1+"&getUSERgROUP="+txt_USERgROUP+"&getUSERBRANCHNAME="+USERBRANCHNAME+"&userID="+userID,true);
    mydata.send();
}
</script>
<script>
...
<table>
  <tr>
    ...
    <td>
         <select class="box_decaretion"  style="width: 200px;" name="sel_catagory" id="sel_catagory" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" onchange="is_getScat_01();">
            <option value="">--Select Sub Catagory --</option>
            <option value="1">Security Request (Branch)</option>
            <option value="2">Security Request (Internal)</option>
            <option value="3">CR to be Collected</option>
            <option value="4">Facility to be Closed</option>
            <option value="5">CR to be RMV Lodge</option>
         </select>
    </td>
    <td>
        <div id="diva22">
...
<br />
<input type="button" style="width: 100px;margin-left: 50px;" class="buttonManage" id="btnClose" name="btnClose" value="Gen" onclick="getGried()"/>
<br />
<span id="maneSpan">
...
<div style="display: none;">
    ...
    <input type="text" name="USERBRANCHNAME" id="USERBRANCHNAME" value="<?php echo  $_SESSION['USERBRANCHNAME']; ?>" />
