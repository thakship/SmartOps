...
Page Name		: kiosk Service Request
Purpose			: To Request for kiosk Service
Author			: Madushan Wikramaarachchi
Date & Time		: 10.21 A.M 31/08/2015
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/007";
...
function deleteRow(n){ // this fuction is Delete Row(s) in table
   ...
   function getAdditionalBox(){
    ...
    function getAdditionalBoxsub(){
        var sel_scat02 = document.getElementById("sel_scat03").value;
        if(sel_scat02 == "1014013805" || sel_scat02 == "1014013909" || sel_scat02 == "1014014009" || sel_scat02 == "1014014109" || sel_scat02 == "1014014209" || sel_scat02 == "1014014309" || sel_scat02 == "1014014404"){
            document.getElementById("additionalBOX1").style.display = "inherit";
        }else{
            document.getElementById("additionalBOX1").style.display = "none";
        }
 ...
 //********************************************************************************************************************************************/  
   function is_getScat_01(getTitle){ 
   //alert(getTitle);
    var cat1 = document.getElementById('sel_catagory').value;
    var cat2 = document.getElementById('sel_scat01').value;
    var cat3 = document.getElementById('sel_scat02').value;
   ...
   // alert(cat3);
    if(getTitle == 5){
        ...
        mydata.onreadystatechange=function(){
            if(mydata.readyState==4){
               // alert(mydata.responseText);
                ...
                (function() {
                    document.getElementById("sel_scat03").onchange = function() { 
                        //alert('hello'); 
                        getAdditionalBoxsub();
                        var getScat03 = document.getElementById('sel_scat03').value;
                       	var mydata1;
                		mydata1 = new XMLHttpRequest();
                		mydata1.onreadystatechange=function(){
                			if(mydata1.readyState==4){
                			    var getStr = mydata1.responseText;
                                arrGet = getStr.split("|");
                				//document.getElementById('lblInsQ').innerHTML = "Sheet Rate Int : " + arrGet[0] + " Min Int Rate :"+arrGet[1];
								document.getElementById('txt_Intrest_Rate').value = 0.00;
								document.getElementById('txt_Intrest_Rate').innerHTML = "0.00";
                                document.getElementById("btnSave").disabled = true;
                			}
                		}
                        
                		mydata1.open('GET','ajax_serviceRequset_02.php'+'?getScat03_Sub='+getScat03,true);
                		mydata1.send();
                        
                        
                        //document.getElementById('lblInsQ').innerHTML = getScat03;
        ...
        mydata.open("GET","ajax_serviceRequset_01.php"+"?txt1="+cat3+"&txt2="+getTitle,true);
        mydata.send();
...
function CheckLostFocus(evt){
    var charCode = (evt.which) ? evt.which : event.keyCode;
	//alert(charCode);
    if (charCode == 9){
		
	    EnteredInterestRate = Number(document.getElementById('txt_Intrest_Rate').value);
		CDBSheetIntRate = Number(arrGet[0]);
		CDBMinimumIntRate = Number(arrGet[1]);
		ZoneActualRate   = Number(document.getElementById('lblZoneActualRate').innerHTML);
		ZoneRequiredRate = Number(document.getElementById('lblZoneRequiredRate').innerHTML);
		
		CDBZoneRate = 0;
		//alert("EnteredInterestRate = " + EnteredInterestRate + " | CDBSheetIntRate = " + CDBSheetIntRate + " | CDBMinimumIntRate = " + CDBMinimumIntRate);
		if(EnteredInterestRate >= CDBMinimumIntRate){
			//alert("1");
			if(EnteredInterestRate >= CDBSheetIntRate){
				//alert("2");
				document.getElementById('lblInsQ').innerHTML = "";
				document.getElementById("btnSave").disabled = false;
			}else{
				//alert("3");
				if(ZoneActualRate < ZoneRequiredRate){
					//alert("4");
					document.getElementById('lblInsQ').innerHTML = "Interest rate Error." + " - Zone Acutual: " + ZoneActualRate  + " | Required: " + ZoneRequiredRate;
					document.getElementById("btnSave").disabled = true;
				}else{
					//alert("4.ELSE");
					document.getElementById('lblInsQ').innerHTML = "";
			...
			//alert("5");
			document.getElementById('lblInsQ').innerHTML = "Minimum interest rate Error";
			document.getElementById("btnSave").disabled = true;
		}
			
		//document.getElementById('lblInsQ').innerHTML = "Interest rate Error ENTD:" + EnteredInterestRate + " - SHEET : " + CDBSheetIntRate + " - MIN " + CDBMinimumIntRate;
	}
...
function onclickCheckLostFocus(){
...
<form action="UPLOAD_FILE.php" method="post" id="uploadForm" name="uploadForm" enctype="multipart/form-data"> 
<p class="topline">
<?php 
...
<table>
  <tr>
    ...
    <td>
         <input class="box_decaretion" type="text" style="width:200px;" name="sel_catagory" id="sel_catagory" value='1014' onkeypress="return disableEnterKey(event)" readonly="readonly"/>
    </td>
    <td>
        <input class="box_decaretion" type="text" style="width:200px;" name="sel_scat01" id="sel_scat01" value='101401' onkeypress="return disableEnterKey(event)" readonly="readonly"/>
    </td>
    <td>
        <!--<input class="box_decaretion" type="text" style="width:200px;" name="sel_scat02" id="sel_scat02" value='10140101' onkeypress="return disableEnterKey(event)" readonly="readonly"/>-->
    </td>
    <td>
...
<label class="linetop" style="margin-left: 160px; font-weight: bold;"><?php echo $_SESSION['user']." - ".$_SESSION['userID']." - ".$_SESSION['GSMNO'] ?> </label><br />
<label class="linetop" id="lblZone" style="margin-left: 160px; font-weight: bold;"><?php echo $_SESSION['CDBZONE']?> </label>
<label class="linetop" id="lblZoneActualRate" style="margin-left: 30px; font-weight: bold;"><?php echo $_SESSION['ZONELENDRATE']?> </label>
<label class="linetop" id="lblZoneRequiredRate" style="margin-left: 30px; font-weight: bold;"><?php echo $_SESSION['CDBWALR']?> </label><br /><br />

<table>
   <tr>
    <td style="width: 150px; text-align: right;padding-right: 5px;"><label class="linetop">Approval Type:</label></td>
    <td>
         <select class="box_decaretion"  style="width: 200px;" name="sel_MainFileType" id="sel_MainFileType" required="required" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)">
  ...
  <tr>
    <td style="width: 150px; text-align: right;padding-right: 5px;"><label class="linetop"> Sub File Type 2 :</label></td>
    <td>
       <select class="box_decaretion"  style="width: 200px;" name="sel_scat02" id="sel_scat02" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" title="5" onchange="getAdditionalBox();is_getScat_01(title);">
                <option value="">-- Sub File Type 2 --</option>
                    <?php
                       
                        $v_sql_cat_2 = "SELECT `scat_code_2` , `scat_discr_2` FROM `scat_02` WHERE `cat_code` = '1014' AND `scat_code_1` = '101401' and `scat_state_2` = '1';";
  ...
  <tr>
      <td style="width: 150px; text-align: right;padding-right: 5px;"><label class="linetop"> Sub File Type 3 :</label></td>
      <td>
        <div id="divz">
             <select class="box_decaretion"  style="width: 200px;" name="sel_scat03" id="sel_scat03" onkeypress="return disableEnterKey(event)"  required="required"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" disabled="disabled">
   ...
   <tr>
    ...
    <td>
        <input class="box_decaretion" type="text" style="width:150px;" maxlength="15" placeholder="0.00" name="txt_facility_amount" id="txt_facility_amount" value="" onkeypress="return isNumberKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" /> <label style="color: #CC0000;">*</label>
    </td>
  </tr>
   <tr>
    <td style="width: 150px; text-align: right;padding-right: 5px;"><label class="linetop">Interest Rate :</label></td>
    <td>
        <input class="box_decaretion" type="text" maxlength="5" style="width:150px;" maxlength="15" placeholder="0.00" name="txt_Intrest_Rate" id="txt_Intrest_Rate" value="" onkeypress="return isNumberKey(event)"  onkeydown="CheckLostFocus(event)" onfocus="hilightColoyr(this.id)" onkeyup="onclickCheckLostFocus()" onblur="colourLeave(this.id)" required="required" /> <label style="color: #CC0000;">*</label>
        <label id="lblInsQ" style="color: #9B2E28;"></label>
    </td>
  </tr>
</table>
...
<table>
  <tr>
    ...
    <td>
        <input class="box_decaretion" type="text"  style="width:400px;" name="txt_issue" placeholder="Client name and NIC " id="txt_issue" value="" onkeypress="return disableEnterKey(event)" onclick="onclickCheckLostFocus()"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" /> <label style="color: #CC0000;">*** NIC is mandatory</label>
    </td>
  </tr>
  <tr>
    <td style="width: 150px; text-align: right; vertical-align: top;padding-right: 5px;"><label class="linetop">Lending Information :</label></td>
    <td>
          <span id="textarea_id">
...
<div>
       <div>
            <?php echo "<img src='../../../user_img/".$_SESSION['user'].".jpg' style='margin-left : 20px;width: 100px; height: 100px;' />"; ?>
       </div>
       <div>
       <?php
			...
			$CUMULATIVE_COLRAT = 0;
            $DIFFYEARS = 0;
            $sql_emp_coll_mon = "select e.*,sp.*,IFNULL((SELECT es.COL_RAT_LAST_MON FROM emp_coll_mon_lstmon es WHERE es.MONEND = LAST_DAY(now() - interval 1 month ) AND es.HRIS = e.HRIS),0) AS CUMCOLL from emp_coll_mon e,erp_sys_param sp WHERE e.HRIS = '".$_SESSION['user']."' and sp.para_code = 15;";
            $query_emp_coll_mon = mysqli_query($conn,$sql_emp_coll_mon) or die(mysqli_error($conn));
            while($rec_emp_coll_mon = mysqli_fetch_assoc($query_emp_coll_mon)){
                $d1 = new DateTime(date("Y-m-d"));
                $d2 = new DateTime($rec_emp_coll_mon['OFF_JOIN_DATE']);
                $diff = $d2->diff($d1);
                //echo $diff->y." Y ".$diff->m." Mon";
                $DIFFYEARS   = $diff->y;
                $LIVE_COLRAT = floatval($rec_emp_coll_mon['COL_RAT_12_MON']);
				$CUMULATIVE_COLRAT = floatval($rec_emp_coll_mon['CUMCOLL']);
				$CVAL = max($LIVE_COLRAT,$CUMULATIVE_COLRAT);
                $KIOSKVAL = floatval($rec_emp_coll_mon['para_value']);
                $color = $CVAL<71?"#cc3300":($CVAL<81?"#ff6600":($CVAL<90?"#ffcc00":"#29a329"));
                echo "<label style='margin-left : 20px; color: ".$color."' title='12 Months : ".$LIVE_COLRAT."\nLast Month End : ".$CUMULATIVE_COLRAT."'>Collecion ratio : ".$CVAL."%</label><br/>";
                echo "<label style='margin-left : 20px;' title='Date Joined - ".$rec_emp_coll_mon['OFF_JOIN_DATE']."'>@ CDB: ".$diff->y." Y ".$diff->m." Mon"."</label><br/>";
            }
       ?>
       </div>
</div>
...
<table>
  <tr>
    ...
    <td>
       ...
       <label style="color: #C40000;">**Only Upload TIFF File</label>
    </td>
  </tr>
  <tr>
  <tr>
    ...
    <td>
       ...
       <label style="color: #C40000;">**Only Upload PDF File</label>
    </td>
  </tr>
  <tr>
     ...
     <tr>
        ...
        <td>
            <?php 
                ...
                if($DIFFYEARS > 0 ){
                    if($CVAL < $KIOSKVAL){
						if($_SESSION['usergroupNumber'] == "ug00020"){ //We are allowing 2nd Officer to upload files without checking the Collection Ratio
							true;
						}else{
							$EDB = "disabled='disabled'";
						}
                    } 
                } 
            ?>
            <input type="submit" style="width: 100px;" class="buttonManage" id="btnSave" <?php echo $EDB; ?> name="btnSave" value="Save" />
                ...
                if($DIFFYEARS > 0){
                    if($CVAL < $KIOSKVAL) {
						if($_SESSION['usergroupNumber'] == "ug00020"){ //We are allowing 2nd Officer to upload files without checking the Collection Ratio
							...
							echo "<BR><label style='color:#FF0000;'>***  File submission has been blocked due to not meeting of collection ratio requirement. ***</label>";
							echo "<script>document.getElementById('sel_MainFileType').disabled = true;document.getElementById('sel_scat02').disabled = true;document.getElementById('txt_Intrest_Rate').disabled = true;document.getElementById('txt_facility_amount').disabled = true;</script>";
						}
					}
                }  
            ?>
