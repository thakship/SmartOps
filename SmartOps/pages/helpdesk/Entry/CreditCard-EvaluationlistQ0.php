...
Page Name		: Credit Card Initial Screening
Purpose			: To viwe Credit Card Request for Credit Card Initial Screening
Author			: Madushan Wikramaarachchi
Date & Time		: 12:30 A.M 28/01/2019
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
session_start();
$_SESSION['page']="hel/e/069";
...
<head>
    ...
    <script type="text/javascript">
        function pageClose(){ //Page Close Function.......
            ...
            window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/CreditCard-EvaluationlistQ0.php?DispName=Credit%20Card%20Initial%20Screening','conectpage');
        }
        function pageRef(){
        ...
        function clientValidate(title,cat_cod){
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_creaditCard.php"+"?getHelpIDreqCardCenterQ0="+getHI+"&gettxt_helpdesk_closeCardCenterQ0="+helpClose,true);
            mydata.send();
        }
        ...
        function isEveluale(title){
            ...
            if(title == "A"){
                ...
                var v_srting =  "R|"+getsubHI+"|Are you sure you want to Approve this?";
                popup1(1,v_srting);
                //alert('O');
            }else if(title == "R"){
                //alert('Reject');
                ...
                var getsubHI = document.getElementById('txt_help_ID').value;
                var resTitle = title.split("|");
                var v_srting =  "R|"+getsubHI+"|Are you sure you want to Back to BOIC this?";
                popup3(1,v_srting);
            }else{
            ...
            if(x==1){
                ...
                mydataGried.onreadystatechange=function(){
                    if(mydataGried.readyState==4){
                        ...
                        document.getElementById('outer').style.visibility = "visible";
                        document.getElementById('conten').style.visibility = "visible";
                        //alert(document.getElementById('sel_scat02').value);
                        var sCode = document.getElementById('sel_scat02Code').value;
                        document.getElementById('sel_scat02').value = sCode;

                        document.getElementById('txt_Card_Amount').value = document.getElementById('txt_issuessb_facility_amount').value;
                ...
                mydataGried.open("GET","credit_card_call_ajax.php"+"?get_ApproveCE="+x+"&get_functionCE="+title,true);
                mydataGried.send();
                document.getElementById('outer').style.visibility = "visible";
                document.getElementById('conten').style.visibility = "visible";
        ...
        function popup3(x,title){
            ...
            if(x==1){
                ...
                mydataGried.open("GET","credit_card_call_ajax.php"+"?get_Back_AUTH="+x+"&get_function="+title,true);
                mydataGried.send();
                document.getElementById('outer').style.visibility = "visible";
                document.getElementById('conten').style.visibility = "visible";
...
$v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` ,
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5049' AND
       `cdb_helpdesk`.`cat_code` = '1038' AND
       date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
...
?>
        ...
        try{
            ...
            if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
                ...
                $sql_update = "UPDATE cdb_helpdesk AS a
                            SET a.ssb_type = 'Credit Evalution - Recommended to proceed',
                                a.cmb_code ='5050'
                            WHERE a.helpid = '".$_POST['txt_helpdeskCredit']."';";
                $query_update = mysqli_query($conn,$sql_update) or  die(mysqli_error($conn));

                $msg = "Credit Card Recommended to proceed by Credit Evalution - ".trim($_POST['txtCommentCredit']);
                noteTbl($conn, $_POST['txt_helpdeskCredit'], $msg, $_SESSION['user']);

                $sql_NEW = "INSERT INTO credit_card_init_rec (help_id,scat02,card_amount,credit_score,r_by)
                             VALUES('".$_POST['txt_helpdeskCredit']."','".$_POST['sel_scat02']."','".$_POST['txt_Card_Amount']."','".$_POST['txt_Credit_Score']."','".$_SESSION['user']."');";
                $query_NEW = mysqli_query($conn,$sql_NEW) or  die(mysqli_error($conn));
                // ----------------------------------------------------------------------------------------------------------------------//
                mysqli_commit($conn);
    ...
    if(isset($_POST['btn_Reject']) && $_POST['btn_Reject']=='Reject'){
        ...
        try{
            ...
            if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
                // echo "<script> alert('C');</script>";
                //$getmail = 0;
                $StrCallAnswered = "Credit Evalution - File Reject : ".trim($_POST['txtCommentCredit']);
                date_default_timezone_set('Asia/Colombo');
                $v_update_sql = "UPDATE `cdb_helpdesk` 
                                 SET `cmb_code`='5005', 
                                     ...
                                     `solved_on`= now() ,
                                     `ssb_type` = 'Credit Evalution - File Reject Q0'
                               WHERE `helpid` = '".$_POST['txt_helpdeskCredit']."';";
                $que_getSQL_Update = mysqli_query($conn,$v_update_sql) or die(mysqli_error($conn));

                $msg = "Credit Card Application Rejected by Credit Evalution : ".trim($_POST['txtCommentCredit']);
                noteTbl($conn, $_POST['txt_helpdeskCredit'], $msg, $_SESSION['user']);

                // Send Mail
                $rec_get_entry_branch = "";
                $sql_get_entry_branch = "select s.entry_branch from cdb_helpdesk AS s where s.helpid = '".$_POST['txt_helpdeskCredit']."';";
                $query_get_entry_branch = mysqli_query($conn,$sql_get_entry_branch) or die(mysqli_error($conn));
                while($rec_get_entry_branch = mysqli_fetch_array($query_get_entry_branch)){
                    $VbrnCode = $rec_get_entry_branch[0];
                }

                if($VbrnCode != ""){
                    $subject = "Credit Card Application Rejected by Credit Evalution : ".$_POST['txt_helpdeskCredit'];
                    SendEmailToBranch($conn,$VbrnCode,$subject,$msg);
    ...
    // btn_Back
    if(isset($_POST['btn_Back']) && $_POST['btn_Back']=='Back'){
        // echo "<script> alert('A');</script>";
        mysqli_autocommit($conn,FALSE);
        try{
            // Validation rule engine
            ...
            if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
                // echo "<script> alert('C');</script>";
                //$getmail = 0;
                $StrCallAnswered = "Credit Evalution - Back to Branch Level : ".trim($_POST['txtCommentCredit']);
                date_default_timezone_set('Asia/Colombo');
                $v_update_sql = "UPDATE `cdb_helpdesk` 
                                 SET `cmb_code`='5000', 
                                     `ssb_type` = 'Credit Evalution - Back to Branch Level'
                               WHERE `helpid` = '".$_POST['txt_helpdeskCredit']."';";
                $que_getSQL_Update = mysqli_query($conn,$v_update_sql) or die(mysqli_error($conn));

                $msg = "Credit Card Application Back to Branch Level by Credit Evalution : ".trim($_POST['txtCommentCredit']);
                noteTbl($conn, $_POST['txt_helpdeskCredit'], $msg, $_SESSION['user']);
                ...
                //echo $VbrnCode;
                if($VbrnCode != ""){
                    $subject = "Credit Card Application Back to Branch Level by Credit Evalution : ".$_POST['txt_helpdeskCredit'];
                    SendEmailToBranch($conn,$VbrnCode,$subject,$msg);
    ...
    function SendEmailToBranch($conn,$VbrnCode,$subject,$body){
        $toMailTo = "";
        $sql = "select u.email from branch b ,user u where b.branchNumber = '".$VbrnCode."' and u.userName = b.BOIC";
        $rs = mysqli_query($conn , $sql) or die(mysqli_error($conn));
        while($rs_tmp = mysqli_fetch_array($rs)){
            $toMailTo = $rs_tmp[0];
        }
        echo $toMailTo;
        if($toMailTo !=""){
            ...
            $inseet_mailBox = "INSERT INTO `mail_queue`( `jobIndateTime`, `sender`, `receivers_to`, `subject`, `body`, `sendDateTime`) 
                                                 VALUES (NOW(), 'SYSTEM', '".$toMailTo."', '".$subject."', '".$body."', '0000-00-00 00:00:00');";
            //echo $inseet_mailBox;
