...
Page Name		: Service Request Delete
Purpose			: To Close Request for Service
Author			: Madushan Wikramaarachchi
Date & Time		: 09.50 P.M 13/01/2015
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/003";
...
function isEditRequset(getTitle){
    ...
    mydata1.onreadystatechange=function(){
            ...
            mydata1.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?editHelpReq_ID="+editHelpID,true);
            mydata1.send();
}
...
function isAddiImgRequset(title){
    ...
    */
	window.open('./applicationPendingUpload.php?DispName=Application%20%20Pending%20Upload&helpID='+helpID+'&pendingUploadImgType='+aIType,'conectpage');
}
</script>
<script>
/* jquery.form.min.js */
...
<table border="1" id="myTable" cellpadding="0" cellspacing="0" style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
            <tr style="background-color: #BEBABA;">
                ...
                <td style="width:150px; text-align: left;"><span style="margin-left: 5px;">Category </span></td>
...
<?php
   ...
   /*$v_sql_select = "SELECT `helpid`, DATE(`enterDateTime`) AS getDate, `issue` 
                      ...
                      WHERE `cdb_helpdesk`.`cat_code` = '1014' and 
                            `cdb_helpdesk`.`enterBy` = '".$_SESSION['user']."' AND 
                            `cdb_helpdesk`.`cmb_code` IN ('5001','5000','5050')
                            UNION ALL 
                            SELECT `helpid`, DATE(`enterDateTime`) AS getDate,`issue` 
                            FROM `cdb_helpdesk` 
                            WHERE `cdb_helpdesk`.`cat_code` <> '1014' and 
                                  `cdb_helpdesk`.`cmb_code` IN ('5001','5000','5050') AND 
    ...
    $v_sql_select = "SELECT `cdb_helpdesk`.`helpid`, 
                               DATE(`cdb_helpdesk`.`enterDateTime`) AS getDate, 
                        		 `cdb_helpdesk`.`issue` ,
                        		 `cat_states`.`cat_discr`
                        FROM `cdb_helpdesk` , `cat_states` 
                        WHERE `cdb_helpdesk`.`cat_code` = `cat_states`.`cat_code`
                          AND `cdb_helpdesk`.`cat_code` = '1014' 
                          and `cdb_helpdesk`.`enterBy` = '".$_SESSION['user']."' 
                          AND `cdb_helpdesk`.`cmb_code` IN ('5001','5000','5050','5049') 
                        UNION ALL 
                        SELECT `cdb_helpdesk`.`helpid`, 
                               DATE(`cdb_helpdesk`.`enterDateTime`) AS getDate, 
                        		 `cdb_helpdesk`.`issue` ,
                        		  `cat_states`.`cat_discr`
                        FROM `cdb_helpdesk` , `cat_states` 
    ...
    while($res_Select = mysqli_fetch_assoc($v_que_select)){
        ...
        echo "<td style='width:150px; text-align: left;'><span  style='margin-left: 5px;'>".$res_Select['cat_discr']."</span></td>";
        ...
        echo "<td style='width:250px;'>
                ...
                <input type='button' class='buttonManage' id='btn_close' name='btn_close' value='Remove' title='txtclose".$index."' onclick='isDeleteRequset(title);'/>
