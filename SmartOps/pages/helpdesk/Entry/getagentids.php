...
if (strlen($param2) > 0) {
    ...
    if($couhelpDesk_veri != 0){
        while($rec_helpDesk_veri = mysqli_fetch_assoc($que_helpDesk_veri)){
                ...
                $SQL_select_mail = "select u.helpid , u.email , u.sendon from useriddelivery AS u where u.helpid = '".$param2."';";
                $QUERY_select_mail = mysqli_query($conn,$SQL_select_mail);
                $getSendEmail = "";
                while($RESALT_select_mail = mysqli_fetch_array($QUERY_select_mail)){
                    $getSendEmail .= $RESALT_select_mail[1]." ";   
                }                
                $textout2 .= $getHelpid."|".$getIssue."|".$getHelp_discr."|".$getSolution."|".$getDefPassword."|".$getattachment_name."|".$getSendEmail;
...
$idNIC = $_GET['idNIC'];
if (strlen($param4) > 0) {
    $textout4 = $param4."|".$idNIC;
   /* $sql_helpDesk_veri = "SELECT * FROM `cdb_helpdesk` WHERE `helpid` = '".$param4."';";
    $que_helpDesk_veri  = mysqli_query($conn , $sql_helpDesk_veri);
    $couhelpDesk_veri = mysqli_num_rows($que_helpDesk_veri);
    if($couhelpDesk_veri != 0){
        while($rec_helpDesk_veri = mysqli_fetch_assoc($que_helpDesk_veri)){
                ...
                $SQL_select_mail = "select u.helpid , u.email , u.sendon from useriddelivery AS u where u.helpid = '".$param4."';";
                $QUERY_select_mail = mysqli_query($conn,$SQL_select_mail);
                $getSendEmail = "";
                while($RESALT_select_mail = mysqli_fetch_array($QUERY_select_mail)){
                    $getSendEmail .= $RESALT_select_mail[1]." ";   
                }                
                $textout4 .= $getHelpid."|".$getIssue."|".$getHelp_discr."|".$getSolution."|".$getDefPassword."|".$getattachment_name."|".$getSendEmail;
