...
Page Name		: Service Request - Instant Card 
Purpose			: To Request for Service Request - Instant Card 
Author			: Madushan Wikramaarachchi
Date & Time		: 12.50 PM 2018-10-04
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/062";
...
<!--<h1 style="color: red;"> System Development in Process. Plz Wait!</h1>-->
<table>
  <tr style="display: none;">
    <td style="width: 100px; text-align: right;"><label class="linetop">Category :</label></td>
    <td>
         <input class="box_decaretion" type="text" readonly="readonly" name="sel_catagory" id="sel_catagory" value="1021" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
    <td>
        <input class="box_decaretion" type="text" readonly="readonly" name="sel_scat01" id="sel_scat01" value="102102" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
    <td>
        <input class="box_decaretion" type="text" readonly="readonly" name="sel_scat02" id="sel_scat02" value="10210208" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
    <!--<td>
...
<table id="get_issue">  
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Card Type :</label></td>
    <td>
       <select class="box_decaretion"  style="width: 200px;" name="sel_scat03" id="sel_scat03" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" >
              <?php
                ...
                $v_sql_abc = "select s.scat_code_3 , s.scat_discr_3 
                from scat_03 AS s 
                WHERE s.cat_code = '1021' 
                 AND s.scat_code_1 = '102102' 
                 AND s.scat_code_2 = '10210208'
                 and s.scat_state_3 = 1 
                 and s.isDisplay = 1";
                $que_getabc = mysqli_query($conn,$v_sql_abc);
                while($RES_getABC = mysqli_fetch_array($que_getabc)){
                    echo "<option value=".$RES_getABC[0].">".$RES_getABC[1]."</option>";
                }
            ?>
        </select>
    </td>
  ...
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Name and NIC :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:400px;" name="txt_issue" id="txt_issue" value="" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
         <label style="color: #CA0000;">*** Name and NIC are Mandatory </label>
    </td>
  </tr>
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Application sending Branch :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:50px;background-color: silver;" readonly="readonly" name="txt_Application_sending_Branch" id="txt_Application_sending_Branch" value="<?php echo $_SESSION['userBranch']; ?>" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
        <label style="color: #A40000;"><?php echo $_SESSION['userBranchName']; ?></label>
    </td>
  </tr>
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Home branch :</label></td>
    <td>
  ...
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Account Number :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:200px;" maxlength="20" name="txt_Account_number" id="txt_Account_number" value="" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
  </tr>
   <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Individual Client Code :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:100px;" maxlength="8" name="txt_Individual_Client_Code" id="txt_Individual_Client_Code" value="" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
  </tr>
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Card number :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:300px;" maxlength="30" name="txt_Card_number" id="txt_Card_number" value="" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
  </tr>
  <tr>
    <td style="width: 100px; text-align: right;"><label class="linetop">Mother&apos;s Maiden name :</label></td>
    <td>
        <input class="box_decaretion" type="text"  style="width:500px;" name="txt_Mother_Maiden_name" id="txt_Mother_Maiden_name" value="" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" required="required" />
    </td>
  </tr>
</table>
<table>
...
function isUniquc_Validator($conn,$scat_code_2,$issue){
    ...
    if(isset($_POST['btnSave']) && $_POST['btnSave']=='Save'){
        ...
        try{
            ...
            // trim($_POST['txt_facAmount'])
            $txt_facAmount = 0;
            
            $iss_Description = mysqli_real_escape_string($conn,trim($_POST['txt_issue']));
            if(trim($_SESSION['user'])!=""){
                if(trim($_POST['sel_catagory']) != "" && trim($_POST['sel_scat01']) != "" && trim($_POST['sel_scat02']) != "" && trim($_POST['txt_issue']) != "" && trim($iss_Description) != "" && trim($_POST['sel_States']) != "" && trim($_POST['sel_Urgency']) != "" && trim($_POST['sel_Priority'])){
            ...
            if(trim($_SESSION['user'])!=""){
                if(trim($_POST['sel_catagory']) != "" && trim($_POST['sel_scat01']) != "" && trim($_POST['sel_scat02']) != "" && trim($_POST['txt_issue']) != "" && trim($iss_Description) != "" && trim($_POST['sel_States']) != "" && trim($_POST['sel_Urgency']) != "" && trim($_POST['sel_Priority']) && $isValidator == 0 ){
                    $TableID = "";
                    $hetCheck = 0;
                    ...
                    if($getUploadStates == 2){
                        ...
                        if($getUploadStates == 3){
							...
							if ($usrDf =='01BRANCH'){
								...
								//echo $SQL_BRWISE_USER;
                                // echo "<script> alert('". $SQL_BRWISE_USER ."');</script>";
                            ...
                            //trim($iss_Description)
                            $txt_Description = mysqli_real_escape_string($conn,trim($iss_Description));
                            while($rec_Auth_Very = mysqli_fetch_array($query_Auth_Very)){
                                if($rec_Auth_Very[0] == 1){
                                    ...
                                    $v_getSQL_insert = "INSERT INTO `cdb_helpdesk`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`defPassword`,`ssb_facility_amount`,`init_enrty_by`,`init_enrty_on`,`branch_auth_by`,`branch_auth_on`) 
                                                                           VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '5000', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".$txt_issue."', '".$txt_Description."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','". $DefaultPassword ."','".$txt_facAmount."','".$_SESSION['user']."',NOW(),'','0000-00-00 00:00:00');";
									...
									$v_getSQL_insert_1 = "INSERT INTO `cdb_helpdesk_his`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_facility_amount`,`init_enrty_by`,`init_enrty_on`,`branch_auth_by`,`branch_auth_on`)
                                                            VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '5000', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".$txt_issue."', '".$txt_Description."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','".$txt_facAmount."','".$_SESSION['user']."',NOW(),'','0000-00-00 00:00:00');";
                                    ...
                                    $v_getSQL_insert = "INSERT INTO `cdb_helpdesk`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`defPassword`,`ssb_facility_amount`) 
                                                VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".$txt_issue."', '".$txt_Description."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','". $DefaultPassword ."','".$txt_facAmount."');";
                                    $que_getSQL_insert = mysqli_query($conn,$v_getSQL_insert) or die(mysqli_error($conn));
                                    $v_getSQL_insert_1 = "INSERT INTO `cdb_helpdesk_his`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_facility_amount`)
                                                 VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".$txt_issue."', '".$txt_Description."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','".$txt_facAmount."');";
                                    $que_getSQL_insert_1 = mysqli_query($conn,$v_getSQL_insert_1) or die(mysqli_error($conn));
                            ...
                            //echo "A<br/>";
                            // -------- Insert to Instance card Table ----------------------
                            $SQL_INS_CARD = "INSERT INTO card_header_ins(COLLECTING_BRANCH, HOME_BRANCH, ACCOUNT_NO_1, DEBIT_CARD_NUMBER, MOTHER_MAIDEN_NAME,help_ID,WITHDRAWAL_LIMIT,PURCHASING_LIMIT,cif)
                                                                 VALUES ('".trim($_POST['txt_Application_sending_Branch'])."', '".trim($_POST['sel_Home_Branch'])."', '".trim($_POST['txt_Account_number'])."', '".trim($_POST['txt_Card_number'])."', '".trim($_POST['txt_Mother_Maiden_name'])."','".$TableID."','100000.00','200000.00','".trim($_POST['txt_Individual_Client_Code'])."');";
                            //echo $SQL_INS_CARD . "<br/>";
                            $QUERY_INS_CARD = mysqli_query($conn,$SQL_INS_CARD) or die(mysqli_error($conn));
                            //------------------------------------------------------------------
...
while($res_cou_cycal = mysqli_fetch_array($query_cou_cycal)){
    if ( substr($res_cou_cycal[1],0,2) =='07'){                  
        $sql_sms = "INSERT INTO `CDB_SMS_INBOX`(`mobileno`, `message`, `idatetime`, `module`, `uevent`, `sent`) 
                    VALUES ('".$res_cou_cycal[1]."',CONCAT('PATPAT.LK Alert!".chr(10)."Ref :".$TableID.chr(10)."Client Contact Number :".trim($_POST['txt_issue']).chr(10)."".trim($iss_Description).chr(10)."', now()),now(),'HELPDESK','PATPAT.LK',0);";
...
while($res_cou_call = mysqli_fetch_array($query_cou_call)){
    if ( substr($res_cou_call[4],0,2) =='07'){                  
        $sql_sms = "INSERT INTO `CDB_SMS_INBOX`(`mobileno`, `message`, `idatetime`, `module`, `uevent`, `sent`) 
                    VALUES ('".$res_cou_call[4]."',CONCAT('Call Center Inquiry Alert!".chr(10)."Ref : ".$TableID.chr(10)."Client Contact Number :".trim($_POST['txt_issue']).chr(10)."".trim($iss_Description).chr(10)."', now()),now(),'HELPDESK','Call Center Inquiry',0);";
        $query_sms =  mysqli_query($conn,$sql_sms) or die(mysqli_error($conn)); 
