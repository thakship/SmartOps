<?php
    ...
    WHERE `cdb_helpdesk`.`cat_code` = '1014' AND `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
        ...
        and `cdb_helpdesk`.`helpid` NOT IN (SELECT c.helpid FROM callcenterinquiry AS c)";
 
    $v_que_det_detelsCNT = mysqli_query($conn,$v_sql_det_detelsCNT);
    $index = 0;	
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
        echo "<h3>Total record(s) - New Items  : ". $rec_det_detelsCNT[0]. "&nbsp;&nbsp;<font style='color:red; font-size:14px;'>[For the Day : ".$rec_det_detelsCNT[1]."]</font>&nbsp;&nbsp;<font style='color:blue; font-size:14px;'>[Last 30 Days : ".$rec_det_detelsCNT[2]." ]</font></h3>";
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
       `cdb_helpdesk`.`helpid` NOT IN (SELECT c.helpid FROM callcenterinquiry AS c ) 
