...
Page Name		: kiosk Service Request List - App create
Purpose			: To viwe comlited app request list 
Author			: Madushan Wikramaarachchi
Date & Time		: 09.36 A.M 16/02/2016
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/016";
...
<title>kiosk Service Request List - App create</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/kioskServiceRequestList_AppGenaret.php?DispName=Kiosk%20Service%20Request%20-%20App%20Created','conectpage');
}
function pageRef(){
  ...
  var key = event.keyCode || event.which;
   ...
   if(key == 120){ //F9
       ...
       var mydataNew;
		...
		mydataNew.open('GET','ajaxEntryNewGrid_AppCRF9.php'+'?n_userID='+userID+'&n_userg='+userg,true);
		mydataNew.send();
        ...
        if(key == 121){
		   ...
		   var mydataNew;
			...
			var userID = document.getElementById('txt_USERMY').value;
			var userg = document.getElementById('txt_USERgROUP').value;
			mydataNew.open('GET','ajaxEntryNewGrid_AppCRF10.php'+'?n_userID='+userID+'&n_userg='+userg,true);
			mydataNew.send();
        }else{
             if(key == 113){
			   var mydataNew;
				...
				var userID = document.getElementById('txt_USERMY').value;
				var userg = document.getElementById('txt_USERgROUP').value;
				mydataNew.open('GET','ajaxEntryNewGrid_AppCRF2.php'+'?n_userID='+userID+'&n_userg='+userg,true);
				mydataNew.send();
            }else {
				if(key == 114){
				   var mydataNew;
					...
					var userID = document.getElementById('txt_USERMY').value;
					var userg = document.getElementById('txt_USERgROUP').value;
					mydataNew.open('GET','ajaxEntryNewGrid_AppCRF3.php'+'?n_userID='+userID+'&n_userg='+userg,true);
					mydataNew.send();
				}else{
					if(key == 119){
					   var mydataNew;
						...
						var userID = document.getElementById('txt_USERMY').value;
						var userg = document.getElementById('txt_USERgROUP').value;
						mydataNew.open('GET','ajaxEntryNewGrid_AppCRF8.php'+'?n_userID='+userID+'&n_userg='+userg,true);
						mydataNew.send();
					}	
				}				
			}
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:70px;text-align: right;"><span style="margin-right: 5px;">LOS Assign User</span></td>
...
<?php
 ...
 // (if((select count(*) from cdb_ssb_his pv where pv.helpid = `cdb_helpdesk`.`helpid` and pv.P_V = 'P') = 0 ,(ROUND(time_to_sec((`cdb_helpdesk`.`appcrdate`, `cdb_helpdesk`.`enterDateTime`))) / 60) , 999))
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    //-------------------------------------
        ...
        */
		...
		echo "<td style='width:90px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[28]."</span></td>";
        ...
        echo "<td style='width:70px;text-align: right;".$MyRow ."' title = '".$asingBy."'><span style='margin-right: 2px;'>".$rec_det_detels[29]."</span></td>";
