...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax.php"+"?getHelpIDreq_SP_sub="+getHI+"&gettxt_helpdesk_close_SP_sub="+helpClose,true);
        mydata.send();
 ...
 <script>
  $(function() {
   ...
   function isSelectSRReport(){
        ...
        mydata.open("GET","ajax_scan_camplied.php"+"?getEntryDate="+date1+"&getuserGroup="+userGroup+"&geFacilityNumber="+FacilityNumber,true);
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`isScanned` = 1 AND
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
       ORDER BY `cdb_helpdesk`.`caloser_dateTime` desc, `cdb_helpdesk`.`helpid` limit 400;";
