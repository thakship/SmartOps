...
Page Name		: Service Request List - Verification Deposit
Purpose			: To viwe Request list for Service Verification Deposit
Author			: Madushan Wikramaarachchi
Date & Time		: 11 : 12 AM 2019-01- 23
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
session_start();
$_SESSION['page']="hel/e/067";
...
<head>
    ...
    <script type="text/javascript">
        function pageClose(){ //Page Close Function.......
            ...
            window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/serviceRequestList_BTOB.php?DispName=Service%20Request%20List%20-%20Verification%20Deposit%20BP','conectpage');
        }
        function pageRef(){
            window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/serviceRequestList_BTOB.php?DispName=Service%20Request%20List%20-%20Verification%20Deposit%20BP','conectpage');
        }
...
$where_2 = "`cdb_helpdesk`.`cat_code` not in ('1014' , '1024' , '1032' , '1028' , '1029' , '1038') AND `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND";
...
$v_sql_det_detelsCNT = "SELECT COUNT(1) 
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`scat_code_1` = '2021' AND
       ".$where_1."
...
//Send the message, check for errors
                ...
                if($getStateList == 1){
                    $disMkr = "";
                    $sql_issue_disMkr = "select ig.isDisMkr from cdb_cat_issue_gen AS ig WHERE ig.c_scat_code_2 = '".$scat_code_2_number."' AND ig.run_statas = 1  AND  ig.isEntryUser = 0 AND ig.isDisMkr = '".trim($_POST['selDesision'])."';";
                    ...
                    //echo "<script> alert('".$disMkr."');</script>";
                            if($disMkr == trim($_POST['selDesision'])){
                                ...
                                $sql_issue_cat = "select COUNT(*) from cdb_cat_issue_gen AS ci WHERE ci.c_scat_code_2 = '".$scat_code_2_number."' AND ci.run_statas = 1 AND  ci.isEntryUser = 0 AND ci.isDisMkr = '".trim($_POST['selDesision'])."';";
                                     ...
                                     //    echo "<script> alert('C');</script>";
                                        if($resalt_issue_cat[0] > 0){
                                         //   echo "<script> alert('D');</script>";
                                            genHelpdeskIssue($conn,trim($_POST['get_help_ID']),$scat_code_2_number,trim($_POST['selDesision']));
                                        }
                                    }else if($disMkr == ""){
                    ...
                    $disMkr1 = "";
                    $sql_issue_disMkr = "select ig.isDisMkr from cdb_cat_issue_gen AS ig WHERE ig.c_scat_code_2 = '".$scat_code_2_number."' AND ig.run_statas = 1  AND  ig.isEntryUser = 1 AND ig.isDisMkr = '".trim($_POST['selDesision'])."';";
                    $query_issue_disMkr = mysqli_query($conn,$sql_issue_disMkr) or die(mysqli_error($conn));
                    while($resalt_issue_disMkr = mysqli_fetch_array($query_issue_disMkr)){
                        $disMkr1 = $resalt_issue_disMkr[0];
                    }
                        //echo "<script> alert('".$disMkr1."');</script>";
                        if($disMkr1 == trim($_POST['selDesision'])){
                            //echo "<script> alert('H');</script>";
                            $sql_issue_catnew = "select COUNT(*) from cdb_cat_issue_gen AS ci WHERE ci.c_scat_code_2 = '".$scat_code_2_number."' AND ci.run_statas = 1  AND  ci.isEntryUser = 1 AND ci.isDisMkr = '".trim($_POST['selDesision'])."';";
                               ...
                               // echo "<script> alert('I');</script>";
                                ...
                                if($disMkr1 == trim($_POST['selDesision'])){
                                    ...
                                    if($resalt_issue_catnew[0] > 0){
                                        ...
                                        //echo "<script> alert('C');</script>";
                                        genHelpdeskNewIssue($conn,trim($_POST['get_help_ID']),$scat_code_2_number,$_SESSION['user'],$_SESSION['userBranch'],$_SESSION['userDepartment'],trim($_POST['selDesision']));
                                    }
                                }else if($disMkr1 == ""){
    ...
    function genHelpdeskIssue($conn,$helpdeskId,$scat02,$Desision){
        $isUniquc = 0;
        $isValidator = 0;
        ...
        $scat_code_2 = "";
        $scat_code_3 = "";
        $sql_issue_cat = "select ci.cat_code , ci.scat_code_1, ci.scat_code_2, ci.scat_code_3 
                           from cdb_cat_issue_gen AS ci WHERE ci.c_scat_code_2 = '".$scat02."' 
                           AND ci.run_statas = 1 
                           AND ci.isEntryUser = 0
                           AND ci.isDisMkr = '".$Desision."' ;";
        $query_issue_cat = mysqli_query($conn,$sql_issue_cat) or die(mysqli_error($conn));
        while($resalt_issue_cat = mysqli_fetch_array($query_issue_cat)){
...
<table border='1'>
    ...
    function genHelpdeskNewIssue($conn,$helpdeskId,$scat02,$userEnrty,$userBranch,$userDepartment,$Desision){
        $isUniquc = 0;
        $isValidator = 0;
        ...
        $scat_code_2 = "";
        $scat_code_3 = "";
        $sql_issue_cat = "select ci.cat_code , 
                                 ci.scat_code_1, 
                                 ci.scat_code_2, 
                                 ci.scat_code_3 
                          from cdb_cat_issue_gen AS ci 
                          WHERE ci.c_scat_code_2 = '".$scat02."' 
                          AND ci.run_statas = 1 
                          AND ci.isEntryUser = 1
                          AND ci.isDisMkr = '".$Desision."';";
        $query_issue_cat = mysqli_query($conn,$sql_issue_cat) or die(mysqli_error($conn));
        while($resalt_issue_cat = mysqli_fetch_array($query_issue_cat)){
