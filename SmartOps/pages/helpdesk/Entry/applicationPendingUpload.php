  ...
  <tr style="vertical-align: top;">
    ...
    <td>
   	     <select class="box_decaretion"  style="width: 200px;" name="sel_Purpose" id="sel_Purpose" onkeypress="return disableEnterKey(event)"  onfocus="hilightColoyr(this.id)" onblur="colourLeave(this.id)" onchange="getNewFiledScat02();">
                ...
                <option value="UCL Downpayment Receipt">UCL Downpayment Receipt</option>
                <option value="UCL Insurance Debit Note">UCL Insurance Debit Note</option>
         </select>
    </td>
  </tr>
</table>
...
<table  border="1" id="myTable" cellpadding="0" cellspacing="0" style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px; margin-left: 80px;">
    ...
    <?php
        $sql_select_tbl = "SELECT `up_index`, `file_type`, `help_id`, `ai_serial_number`, `file_name`, `file_path`, `ai_file_name`, `done_by`, `done_on`, `flag`,`removedon`, `remark` , `DocPurpose` FROM `pending_upload_file` WHERE `help_id` = '".$_REQUEST['helpID']."';"; 
                    ...
                    if($_FILES["fileAttachment"]["error"] > 0){
    				   ...
    				   $_SESSION['fileAttachment'] = $TableID.$_FILES["fileAttachment"]["name"];
                        if(file_exists("C:/wamp64/www/CDB/uploadHelpdesk/" .$_SESSION['fileAttachment'])){
                            ...
                            $sql_insert = "INSERT INTO `pending_upload_file`(`file_type`, `help_id`, `ai_serial_number`, `file_name`, `file_path`, `ai_file_name`, `done_by`, `done_on`, `flag` , `remark` ,`DocPurpose`) 
                                                                    VALUES ('".$_POST['txt_aIType']."','".$_POST['txt_Help_ID']."','".$batch_num."','".$_FILES["fileAttachment"]["name"]."','C:/wamp64/www/CDB/uploadHelpdesk/','".$_SESSION['fileAttachment']."','".$_SESSION['user']."',now(),0,'".$_POST['textRemark']."','".$_POST['sel_Purpose']."');";
                            ...
                            $NOTE = "Additional Images Uploaded";
                           $CreateNote = "ADDIMGUPLOAD";
                            //CreateNote($conn,trim($_POST['txt_Help_ID']),$NOTE,$_SESSION['user'],$CreateNote);
                            $SERIAL = 1;
    
                            /*Get the Max count of Note*/
                            $sql_select_note_count =  "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".trim($_POST['txt_Help_ID'])."';";
                            $query_select_note_count = mysqli_query($conn , $sql_select_note_count) or die(mysqli_error($ConnERP));
                            while($rec_select_note_count = mysqli_fetch_array($query_select_note_count)){
                                $SERIAL = $rec_select_note_count[0] + 1;    
                            }
                            
                            $sql_note_update = "INSERT INTO `cdb_help_note`(`helpid`, `note_code`, `note_discr`, `enterBy`, `enterDateTime`,`NoteTag`) VALUES (".trim($_POST['txt_Help_ID']).",'".$SERIAL."','".$NOTE."','".$_SESSION['user']."',now(),'".$CreateNote."');";
                            $query_note_update = mysqli_query($conn,$sql_note_update) or die(mysqli_error($ConnERP));
                            mysqli_commit($conn); 
