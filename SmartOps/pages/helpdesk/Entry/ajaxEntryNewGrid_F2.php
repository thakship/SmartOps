 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    $FromTables = "FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`";       
     ...
     WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
           ...
           `cdb_helpdesk`.`cat_code` = '1014' AND `scat_02`.scat_code_1 = '101401' AND `scat_02`.scat_code_2 = '10140115' AND
    ...
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
		// echo "<h3>Total record(s) : ". $rec_det_detelsCNT[4]. " </h3>";
		...
		//echo "<table border='1' cellpadding='0' cellspacing='0'><tr><td>Not Assigned</td><td>Initial Submission</td><td>Pending Notified</td><td>File Re-Submitted</td><td>Additional Images</td><td>CDPU Verified</td></tr><tr><td>1</td><td>0</td><td>3</td><td>2</td><td>2</td><td>0</td></tr></table>";
	}
...
<?php
 ...
 `cdb_helpdesk`.`ssb_app_number`,`cdb_helpdesk`.`facno`,ROUND(time_to_sec((TIMEDIFF(NOW(), `cdb_helpdesk`.lastactivityon))) / 60) ".
 $FromTables."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       `cdb_helpdesk`.`scat_code_2` = `scat_02`.`scat_code_2` AND
