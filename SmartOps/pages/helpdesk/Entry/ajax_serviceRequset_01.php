<?php
    ...
    if($v_Select_ID == 1){
        $v_sql_selection = "SELECT `scat_code_1`,`scat_discr_1` FROM `scat_01` WHERE `cat_code` = '".$v_catID."' AND `scat_state_1` = 1 AND `isDisplay` = 1;";
    ...
    if($v_Select_ID == 4){
        $v_sql_selection = "SELECT `scat_code_3`,`scat_discr_3`,`SheetRateInt`,`MinIntRate` FROM `scat_03` WHERE `scat_code_2`='".$v_catID."' AND `scat_state_3` = '1' AND `isDisplay` = 1;";
     ...
     if($v_Select_ID == 5){
        $v_sql_selection = "SELECT `scat_code_3`,`scat_discr_3`,`SheetRateInt`,`MinIntRate` FROM `scat_03` WHERE `scat_code_2`='".$v_catID."' AND `scat_state_3` = '1' AND `isDisplay` = 1;";
        $v_selctTag = "<select class='box_decaretion'  style='width: 200px;' name='sel_scat03' id='sel_scat03' onKeyPress='return disableEnterKey(event)'  onfocus='hilightColoyr(this.id)' onchange='is_getiss_scat_3();' onblur='colourLeave(this.id)'><option value=''>--Select Sub Catagory 3--</option>";
    }
?>
<?php
	//;
	//echo $_SERVER['HTTP_REFERER'];
	//http://cdberp:8080/cdb/pages/helpdesk/Entry/kioskServiceRequset.php?DispName=Kiosk%20Service%20Request
	
	if(substr($_SERVER['HTTP_REFERER'], -25)=="Kiosk%20Service%20Request"){
	}
