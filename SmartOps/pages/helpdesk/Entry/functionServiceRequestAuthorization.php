...
//---------------------------------------------------- Send Mail --------------------------------------------------
       ...
       while($rec_select_iss = mysqli_fetch_assoc($query_select_iss)){
		  //echo "1.".$rec_select_iss['cat_code'];
		  //echo "2.". ($rec_select_iss['cat_code'] == '9999'?"YES":"NO");
		   ...
		   if($rec_select_iss['cat_code'] == '9999'){
			   CreateNoteLoan($conn , $hid,$NOTE,$huser,$CreateNote);
		   }else{
...
function CreateNoteLoan($ConnERP , $HELP_ID,$NOTE,$USER,$CreateNote){
        date_default_timezone_set('Asia/Colombo');
        $SERIAL = 1;
    
        /*Get the Max count of Note*/
        $sql_select_note_count =  "SELECT COUNT(*) FROM `loan_cdb_help_note` WHERE `helpid` = '".$HELP_ID."';";
        $query_select_note_count = mysqli_query($ConnERP , $sql_select_note_count) or die(mysqli_error($ConnERP));
        while($rec_select_note_count = mysqli_fetch_array($query_select_note_count)){
            $SERIAL = $rec_select_note_count[0] + 1;    
        }
        
        $sql_note_update = "INSERT INTO `loan_cdb_help_note`(`helpid`, `note_code`, `note_discr`, `enterBy`, `enterDateTime`) VALUES ('".$HELP_ID."','".$SERIAL."','".$NOTE."','".$USER."',now());";
