...
Page Name		: Service Request Acknowledge - New 
Purpose			: do Service Request Acknowledgement - New
Author			: Madushan Wikramaarachchi
Date & Time		: 10:50 P.M 29/08/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/028";
...
<span id="maneSpan">
<?php
   $v_sql_select = "SELECT `helpid`,`issue`, DATE(`enterDateTime`) AS enDate, `solved_by`, DATE(`solved_on`) AS svDate , `entry_branch` AS entry_branch  , `enterBy` AS enterBy
                    FROM `cdb_helpdesk` 
                    WHERE  `cdb_helpdesk`.`cmb_code` = '5002' AND 
