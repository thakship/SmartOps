...
<span style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:11px; font-weight:bold"><b>F8</b> - File Verified , <b>F10</b> - QC Pending  
...
<?php
 ...
 ." ,IFNULL((select COUNT(*) from cdb_ssb_his ssbh where ssbh.helpid = `cdb_helpdesk`.helpid  and ssbh.P_V = 'P') ,0) ".$FromTables."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       `cdb_helpdesk`.`scat_code_2` = `scat_02`.`scat_code_2` AND
       `cdb_helpdesk`.`entry_branch` = `branch`.`branchNumber` AND
