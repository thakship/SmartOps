...
Page Name		: Credit Card - CDPU List
Purpose			: To viwe Credit Card Request for CDPU list
...
<title>Credit Card - CDPU List</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/credit_card_CDPU_list.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20CDPU','conectpage');
}
function pageRef(){
   ...
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/credit_card_CDPU_list.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20CDPU','conectpage');
    ...
    function clientValidate(title,cat_cod){
        ...
        var helpClose = document.getElementById('txt_helpdesk_close').value;
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_creaditCard.php"+"?getHelpIDreqcdpu="+getHI+"&gettxt_helpdesk_closeCDPU="+helpClose,true);
            mydata.send();  
    }
...
function isUpdateCliendtl(title){
    ...
    if(title == 8 && txt_dtl.length != 18){
        alert('Recovery Account Number is Incorect.');
    }else if(txt_dtl != ""){
        $.ajax({ 
    		type:'POST', 
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5001' AND
		...
		(`cdb_helpdesk`.`ssb_type` = 'Call Center Approvel' OR `cdb_helpdesk`.`ssb_type` = 'Call Center Approved'  OR `cdb_helpdesk`.`ssb_type` = 'Additional Images Uploaded' OR `cdb_helpdesk`.`ssb_type` ='File re-submitted') AND
    ...
    try{
        if(trim($_POST['get_help_ID']) != "" && trim($_POST['selCallFeedbackNO']) != "" && trim($_POST['txtarea_Solution']) != ""){
            ...
            date_default_timezone_set('Asia/Colombo');
                        ...
                        $DIS =  "CDPU  - File Reject. " . $StrCallAnswered." / ".$strCallFeedback;              
                        ...
                        while($rec_getbranchdtl = mysqli_fetch_array($query_getBrandtl)){
                            $subject = "Debit Card Alert - CDPU Inquiry !.";
                            $messageBodyIn = "
                                            <h4>CDPU Inquiry!</h4>
