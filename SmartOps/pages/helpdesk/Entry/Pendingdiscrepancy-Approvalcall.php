...
Page Name		: Pending discrepancy - Approval call 
 ...
 WHERE `cdb_helpdesk`.`cat_code` = '1014' AND `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
        ...
        and `cdb_helpdesk`.`helpid` IN (SELECT c.helpid
                                            	...
                                            	WHERE (c.callstatus = 1  AND c.call_feedback = 2 AND brn_reply_by = '') OR
                                                      (c.callstatus = 1  AND c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number') AND brn_reply_by = '')
                                        )";
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:100px;text-align: left;"><span style="margin-left: 5px;">Facility Number</span></td>
...
<?php
      ...
      $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
						   ...
						   (SELECT CallFeedback FROM callcenterinquiry AS c WHERE c.helpid =  `cdb_helpdesk`.`helpid` and c.callstatus = 1 AND (c.call_feedback = 2  OR (c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number'))) AND brn_reply_by = '' limit 1) as CallFeedback,
						   (SELECT concat('by :',GetUserName(enrtyby),char(10),'',remark) FROM callcenterinquiry AS c WHERE c.helpid =  `cdb_helpdesk`.`helpid` and c.callstatus = 1 AND (c.call_feedback = 2  OR (c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number'))) AND brn_reply_by = '' limit 1) as DescrpRemark										  
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    //-------------------------------------
       ...
       // echo "<td style='width:70px;text-align: right;".$MyRow ."' title = '".$asingBy."'><span style='margin-right: 2px;'>".$rec_det_detels[9]."</span></td>";
		 ...
		 echo "<td style='width:70px;text-align: right;".$MyRow ."'title ='".$rec_det_detels[30]."'><span style='margin-right: 2px;'>".$rec_det_detels[29]."</span></td>";
