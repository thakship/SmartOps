...
Page Name		: Pending discrepancy - Approval call (Branch)
Purpose			: To viwe Pending discrepancy - Approval call  Branch
Author			: Madushan Wikramaarachchi
Date & Time		: 10.00 AM 2018-03-06
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/041";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/Pendingdiscrepancy-Approvalcall-Branch.php?DispName=Pending%20discrepancy%20-%20Approval%20call%20%28Branch%29','conectpage');
}
function pageRef(){
...
function isSolution(title){
	...
	$.ajax({ 
		type:'POST', 
		data: {get_helpID_pro_disq : title , get_user_pro_disq : ksrl_user  , get_txtComment : txtComment}, 
		url: '../helpdesk_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function_callCenter.php', 
		success: function(val_retn) { 
		   //document.getElementById('maneSpan').innerHTML = val_retn;
		    alert(val_retn);
...
function popup(x,title){
    ...
    if(x==1){ 
			...
			mydataGried.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_callCenter.php"+"?get_Approve="+x+"&get_function="+title,true);
			mydataGried.send();
			document.getElementById('outer').style.visibility = "visible";
			document.getElementById('conten').style.visibility = "visible";
...
<?php
 ...
 WHERE `cdb_helpdesk`.`cat_code` = '1014' AND `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
       	    date(`cdb_helpdesk`.`appcrdate`) >= (CURRENT_DATE - INTERVAL 30 DAY)
            and `cdb_helpdesk`.callRecOn IS NOT NULL 
       and `cdb_helpdesk`.`helpid` IN (SELECT c.helpid
                                            	FROM callcenterinquiry AS c
                                            	WHERE (c.callstatus = 1 AND c.call_feedback = 2 AND brn_reply_by = '')
                                                   OR (c.callstatus = 1  AND c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number') AND brn_reply_by = '')
                                ) AND
...
<?php
      ...
      $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
                          ...
                          (SELECT COUNT(*) FROM callPriorityBranch AS cp WHERe cp.helpid = `cdb_helpdesk`.`helpid`) AS Count_P,
                          `cdb_helpdesk`.`cif` AS CIF,
                          (SELECT cq.inquiry_id FROM callcenterinquiry AS cq WHERE cq.callstatus = 1 AND (cq.call_feedback = 2  OR (cq.call_answered = 'NO' and cq.callFeedback in ('Invalid Number','No such person','No contact number'))) AND cq.helpid = `cdb_helpdesk`.`helpid`) AS CALLEN_ID
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`facno` <> '' AND `cdb_helpdesk`.`IsAppValid` = 4 AND
       `cdb_helpdesk`.`helpid` IN (SELECT c.helpid
                                            	FROM callcenterinquiry AS c
                                            	WHERE (c.callstatus = 1 AND c.call_feedback = 2 AND brn_reply_by = '')
                                                   OR (c.callstatus = 1  AND c.call_answered = 'NO' and c.callFeedback in ('Invalid Number','No such person','No contact number') AND brn_reply_by = '')) AND
...
//	echo $v_sql_det_detels;	
    ...
    //-------------------------------------
        ...
        */
        ...
        //<a class='isLinkPending' href='#' title='".$index."|".$res_Select['HEADER_ID']."' onclick='isPending(title);'>Pending</a>
        echo "<td style='width:50px;'><input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='".$index."|".$rec_det_detels[29]."' onclick='isSubmit(title);'/></td>";
