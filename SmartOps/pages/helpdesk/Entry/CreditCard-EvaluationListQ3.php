...
Page Name		: Credit Card - Evaluation List 
Purpose			: To viwe Credit Card Request for Evaluation List 
Author			: Madushan Wikramaarachchi
Date & Time		: 11.24 A.M 25/07/2018
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/060";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/CreditCard-EvaluationListQ3.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20Evaluation','conectpage');
}
function pageRef(){
   ...
   window.open('http://cdberp:8080/cdb/pages/helpdesk/Entry/CreditCard-EvaluationListQ3.php?DispName=Credit%20Card%20Service%20Request%20List%20-%20Evaluation','conectpage');
    ...
    function clientValidate(title,cat_cod){
        ...
        var helpClose = document.getElementById('txt_helpdesk_close').value;
            ...
            mydata.open("GET","../helpdesk_DEVELOPMENT/PHP_FUNCTION/helpdesk_Ajax_creaditCard.php"+"?getHelpIDreqCardCenterCC="+getHI+"&gettxt_helpdesk_closeCardCenterCC="+helpClose,true);
            mydata.send();  
    }
    ...
    try{
        ...
        if(trim($_POST['txt_helpdeskCredit']) != "" && trim($_POST['txtCommentCredit']) != ""){
           ...
           $StrCallAnswered = "Credit Evalution - File Approve";
            ...
            $msg = "Credit Card Application Approved by Credit Evalution - ".trim($_POST['txtCommentCredit']);
            noteTbl($conn, $_POST['txt_helpdeskCredit'], $msg, $_SESSION['user']);

            $sql_NEW = "INSERT INTO credit_card_chng (help_id,scat02,card_amount,credit_score)
                             VALUES('".$_POST['txt_helpdeskCredit']."','".$_POST['sel_scat02']."','".$_POST['txt_Card_Amount']."','".$_POST['txt_Credit_Score']."');";
