<?php
    ...
    ini_set('max_execution_time', 360000000000); //300 seconds = 5 minutes
    try{
		// Validation rule engine
        if(trim($_SESSION['user'])!=""){
            if(trim($_POST['sel_catagory']) != "" && trim($_POST['sel_scat01']) != "" && trim($_POST['sel_scat02']) != "" && trim($_POST['txt_issue']) != "" && trim($_POST['txt_Description']) != "" && trim($_POST['sel_States']) != "" && trim($_POST['sel_Urgency']) != "" && trim($_POST['sel_Priority'])!= "" && trim($_POST['txt_ClientMobileNo']) != "" && trim($_POST['txt_Client_Name'])){
							...
							if($getUploadStates == 3){
								...
								if($getUploadStatessub == 1){
									...
									if($getUploadStatessub == 2){
										echo "already exists. File Error. [UPLOAD_FILE.php >> Line 59] TableID >> ".$TableID;   
									}else{
										if($getUploadStatessub == 3){
                                            ...
                                            //trim($_POST['txt_Existing_Client_Code'])
                                            //trim($_POST['txt_Existing_Facility_No'])
                                            ...
                                            while($rec_Auth_Very = mysqli_fetch_array($query_Auth_Very)){
                                                if($rec_Auth_Very[0] == 1){
                                                    $v_getSQL_insert = "INSERT INTO `cdb_helpdesk`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_type`,`ssb_cycle`,`ssb_facility_amount`,`attachment_namesub`,`lastactivityon`,`AUX_01`,`AUX_02` ,`Linked_helpid`,`init_enrty_by`,`init_enrty_on`,`branch_auth_by`,`branch_auth_on`) 
                                                                                           VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '5000', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".trim($_POST['txt_issue'])."', '".trim($_POST['txt_Description'])."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','Initial Submission',0,'".trim($_POST['txt_facility_amount'])."','".$getUploadStatessub."',now(),'".trim($_POST['txt_Existing_Client_Code'])."','".trim($_POST['txt_Existing_Facility_No'])."','".trim($_POST['txtRequestSource'])."','".$_SESSION['user']."',NOW(),'','0000-00-00 00:00:00');";
                                                    ...
                                                    $v_getSQL_insert = "INSERT INTO `cdb_helpdesk`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_type`,`ssb_cycle`,`ssb_facility_amount`,`attachment_namesub`,`lastactivityon`,`AUX_01`,`AUX_02` ,`Linked_helpid`,`init_enrty_by`,`init_enrty_on`,`branch_auth_by`,`branch_auth_on`) 
                                                                                           VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".trim($_POST['txt_issue'])."', '".trim($_POST['txt_Description'])."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','Initial Submission',0,'".trim($_POST['txt_facility_amount'])."','".$getUploadStatessub."',now(),'".trim($_POST['txt_Existing_Client_Code'])."','".trim($_POST['txt_Existing_Facility_No'])."','".trim($_POST['txtRequestSource'])."','','0000-00-00 00:00:00','','0000-00-00 00:00:00');";
                                                    //	echo $v_getSQL_insert;
        											$que_getSQL_insert = mysqli_query($conn,$v_getSQL_insert) or die(mysqli_error($conn));
        											$v_getSQL_insert_1 = "INSERT INTO `cdb_helpdesk_his`(`helpid`, `cat_code`, `scat_code_1`, `scat_code_2`, `cmb_code`, `ur_code`, `pr_code`, `issue`, `help_discr`, `enterBy`, `enterDateTime`,`attachment_name`, `inner_brCode`, `inner_dept`, `inner_user`, `inner_remark`, `inner_get`,`entry_branch`,`entry_department`,`s_type`,`scat_code_3`,`asing_by`,`ipAddress`,`ssb_type`,`ssb_cycle`,`ssb_facility_amount`,`attachment_namesub`,`lastactivityon`,`AUX_01`,`AUX_02`,`Linked_helpid`,`init_enrty_by`,`init_enrty_on`,`branch_auth_by`,`branch_auth_on`)
                                                         VALUES ('".$TableID."', '".trim($_POST['sel_catagory'])."', '".trim($_POST['sel_scat01'])."', '".trim($_POST['sel_scat02'])."', '".trim($_POST['sel_States'])."', '".trim($_POST['sel_Urgency'])."', '".trim($_POST['sel_Priority'])."', '".trim($_POST['txt_issue'])."', '".trim($_POST['txt_Description'])."', '".$_SESSION['user']."', now(),'".$getUploadStates."','".trim($_POST['txt_Branch'])."', '".trim($_POST['txt_Department'])."', '".trim($_POST['txt_inner_User1'])."','".trim($_POST['inner_Remark'])."','".$hetCheck."','".$_SESSION['userBranch']."','".$_SESSION['userDepartment']."','".trim($_POST['sel_Source'])."','".trim($_POST['sel_scat03'])."','".$usrDf."','".$_SESSION['userIP']."','Initial Submission',0,'".trim($_POST['txt_facility_amount'])."','".$getUploadStatessub."',now(),'".trim($_POST['txt_Existing_Client_Code'])."','".trim($_POST['txt_Existing_Facility_No'])."','".trim($_POST['txtRequestSource'])."','','0000-00-00 00:00:00','','0000-00-00 00:00:00');";
                                        ...
                                        //trim($_POST['txt_ClientMobileNo'])
                                        // trim($_POST['txt_Client_Name'])
                                            $sql_insert = "INSERT INTO `cdb_ssb`(`helpid`, `applicant_info`, `clear_valuation`, `guarantor_1`, `guarantor_2`, `cr_copy_and_invoce`, `supplar_info` , `C_Moble`,`C_Client_name`) 
                                                                VALUES ('".$TableID."','0','0','0','0','0','0','".$_POST['txt_ClientMobileNo']."','".$_POST['txt_Client_Name']."');"; 
                                            ...
                                            if(trim($_POST['txtRequestSource']) != ""){
                                                $sol = "Lease Converted to Business <REF : ".$TableID.">";
                                                $getStateList = isServiceRequsetListInsert(trim($_POST['txtRequestSource']),"-",$sol,$_SESSION['user']);	
                                            }
