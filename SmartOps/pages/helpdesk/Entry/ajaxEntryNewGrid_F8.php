...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
			...
			<td style="width:120px;text-align: right;"><span style="margin-right: 5px;">Int Rate</span></td>
 ...
 $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` ,
                             (`cdb_helpdesk`.`enterDateTime`),
                              ...
                              `cdb_helpdesk`.`ssb_facility_amount` , 
                               CONCAT(`cdb_helpdesk`.`ssb_type`,' - ',(`cdb_helpdesk`.`lastactivityon`)) ,
                             ...
                             `gar_code_2`,
                             `gar_code_3`,
                             ROUND(time_to_sec((TIMEDIFF(NOW(), (if(HOUR(IFNULL((select max(p.done_on) from pending_upload_file p where p.help_id = cdb_helpdesk.helpid  and p.done_on > cdb_helpdesk.enterDateTime),cdb_helpdesk.enterDateTime)) >= 16, STR_TO_DATE(concat( IF(DAYNAME(date(IFNULL((select max(p.done_on) from pending_upload_file p where p.help_id = cdb_helpdesk.helpid and p.done_on > cdb_helpdesk.enterDateTime),cdb_helpdesk.enterDateTime)))='Friday',DATE_ADD(date(IFNULL((select max(p.done_on) from pending_upload_file p where p.help_id = cdb_helpdesk.helpid and p.done_on > cdb_helpdesk.enterDateTime),cdb_helpdesk.enterDateTime)) ,INTERVAL 3 DAY),DATE_ADD(date(IFNULL((select max(p.done_on) from pending_upload_file p where p.help_id = cdb_helpdesk.helpid and p.done_on > cdb_helpdesk.enterDateTime),cdb_helpdesk.enterDateTime)) ,INTERVAL 1 DAY))  ,' 08:30:00'), '%Y-%m-%d %H:%i:%s') , 
                             IFNULL((select max(p.done_on) from pending_upload_file p where p.help_id = cdb_helpdesk.helpid and p.done_on > cdb_helpdesk.enterDateTime),cdb_helpdesk.enterDateTime)))))) / 60),
                             `cdb_helpdesk`.`mainFileType`,(select scat_03.scat_discr_3 from scat_03 where scat_03.cat_code = cdb_helpdesk.cat_code and scat_03.scat_code_1 = cdb_helpdesk.scat_code_1 and scat_03.scat_code_2 = cdb_helpdesk.scat_code_2 and scat_03.scat_code_3 = cdb_helpdesk.scat_code_3),`cdb_helpdesk`.`Intrest_Rate`"
 .$FromTables."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
	   cdb_helpdesk.helpid = cdb_ssb.helpid AND
    ...
    $index = 0;
     ...
     while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        echo "<td style='width:100px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]."-".$rec_det_detels[34]."</span></td>";
        echo "<td style='width:120px;text-align: right;".$MyRow ."'><span style='margin-right: 2px;'>".number_format($rec_det_detels[11], 2)."</span></td>";
		echo "<td style='width:120px;text-align: right;".$MyRow ."'><span style='margin-right: 2px;'>".$rec_det_detels[35]."</span></td>";
		...
		*/
        ...
        echo "<td style='width:50px;'><input type='button' class='buttonManage' id='btn_Validate' name='btn_Validate' value='...' title='txta".$index."' onclick='clientValidatef8(title,".$rec_det_detels[10].");'/></td>";
        echo "</tr>";
    }
?>
</table>
