<?php
	...
	FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
							 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
								   ...
								   /*9:24 AM 23/04/201 (`cdb_helpdesk`.`ssb_type` = 'Additional Images Uploaded' and `cdb_helpdesk`.`assign_to` = 'SSOO') AND 8*/
								   /*9:24 AM 23/04/2018`cdb_helpdesk`.`taken_by` ='  - Pending' AND */
								   /*`cdb_helpdesk`.`helpid` IN (select l.helpid  from cdb_los_sendback l) AND*/
								   `cdb_helpdesk`.`assign_to` IN ('SSOO','SSBI','CO') AND 
								    exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = `cdb_helpdesk`.`helpid` and company = 'CDB') AND
    ...
    while($rec_det_detelsCNT = mysqli_fetch_array($v_que_det_detelsCNT)){
		echo "<h3>Sendback Applications in CDPU | Total record(s) : ". $rec_det_detelsCNT[0]. " </h3>";
    }
?>	
...
<?php	// Print the group wise pending info
	...
	$v_sql_det_detelsGR =  "SELECT `cdb_helpdesk`.`assign_to`,
							 ...
							 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
								   ...
								   `cdb_helpdesk`.`facno` = '' AND
								   `cdb_helpdesk`.`curr_stage` = 'LSERECMD' AND
								   /*9:24 AM 23/04/2018 (`cdb_helpdesk`.`ssb_type` = 'Additional Images Uploaded' and `cdb_helpdesk`.`assign_to` = 'SSOO') AND*/
								   /*`cdb_helpdesk`.`helpid` IN (select l.helpid  from cdb_los_sendback l) AND*/
								   `cdb_helpdesk`.`assign_to` IN ('SSOO','SSBI','CO') AND 
								   exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = `cdb_helpdesk`.`helpid` and company = 'CDB') AND
									date(`cdb_helpdesk`.`appcrdate`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            <td style="width:120px;text-align: right;"><span style="margin-right: 5px;">Category</span></td>
            <td style="width:60px;text-align: right;"><span style="margin-right: 5px;">Number of Items</span></td>
			<td style="width:50px;text-align: right;"><span style="margin-right: 5px;">Value</span></td>
		</tr>
<?php
$COUNT_CT = 0; 
$TOTAL_CT = 0; 
$v_sql_det_detelsGR_CT =  "select MIS.TYPE,SUM(MIS.NUM_FILES),SUM(MIS.ssb_facility_amount) FROM (
select 'Application Created' AS TYPE,count(*)  AS NUM_FILES,SUM(ch.ssb_facility_amount) as ssb_facility_amount
FROM `cdb_helpdesk` ch, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
							 WHERE ch.`ur_code` = `urgency_states`.`ur_code` AND
								   ch.`pr_code` = `priority_states`.`pr_code` AND
								   ch.`scat_code_2` = `scat_02`.`scat_code_2` AND
								   ch.`entry_branch` = `branch`.`branchNumber`AND
								   ch.`entry_department` = `deparment`.`deparmentNumber` AND
								   ch.`cat_code` = `cdb_help_user_oth`.`cat_code` AND 
								   ch.`cmb_code` = '5001' AND
								   ch.`cat_code` = '1014' AND
								   ch.`ssb_app_number` <> '' AND ch.`IsAppValid` <> 4 AND  ch.`IsAppValid` <> 5 AND
								   ch.`facno` = '' AND
								   ch.`curr_stage` = 'LSERECMD' AND
								   ch.`assign_to` IN ('SSOO','SSBI','CO') AND 
								    exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = ch.helpid and company = 'CDB') AND
									date(ch.`appcrdate`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
								   `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
								   and ch.ssb_type = 'Application Created'
								   and not exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = ch.helpid and cdb_los_sendback.enterDateTime > ch.lastactivityon)
union 
select 'Sendback Application' AS TYPE,count(*)  AS NUM_FILES,SUM(ch.ssb_facility_amount)
FROM `cdb_helpdesk` ch, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
							 WHERE ch.`ur_code` = `urgency_states`.`ur_code` AND
								   ch.`pr_code` = `priority_states`.`pr_code` AND
								   ch.`scat_code_2` = `scat_02`.`scat_code_2` AND
								   ...
								   `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
								   and ch.ssb_type = 'Application Created'
								   and exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = ch.helpid and cdb_los_sendback.enterDateTime > ch.lastactivityon)
union								   
select  trim(substr(ssb_type,1,19)) AS TYPE,count(*)  AS NUM_FILES,SUM(ch.ssb_facility_amount)
FROM `cdb_helpdesk` ch, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
							 WHERE ch.`ur_code` = `urgency_states`.`ur_code` AND
								   ch.`pr_code` = `priority_states`.`pr_code` AND
								   ch.`scat_code_2` = `scat_02`.`scat_code_2` AND
								   ...
								   `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
								   and ch.ssb_type <> 'Application Created'
								   and ch.ssb_type <> 'File Verified by CDPU'
								   group by substr(ssb_type,1,19)								   
union

select 'Application Created' AS TYPE,count(*)  AS NUM_FILES,SUM(ch.ssb_facility_amount)
FROM `cdb_helpdesk` ch, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
							 WHERE ch.`ur_code` = `urgency_states`.`ur_code` AND
								   ch.`pr_code` = `priority_states`.`pr_code` AND
								   ch.`scat_code_2` = `scat_02`.`scat_code_2` AND
...
GROUP BY MIS.TYPE";  
//echo $v_sql_det_detelsGR_CT;
	 $v_que_det_detelsGR_CT = mysqli_query($conn,$v_sql_det_detelsGR_CT);	   
	 while($rec_det_detelsGR_CT = mysqli_fetch_array($v_que_det_detelsGR_CT)){
         echo "<TR><td style='width:120px;text-align: right;'><span style='margin-right: 2px;'>".$rec_det_detelsGR_CT[0]."</span></td>";
		 echo "<td style='width:60px;text-align: right;'><span style='margin-right: 2px;'>".$rec_det_detelsGR_CT[1]."</span></td>";
		 echo "<td style='width:50px;text-align: right;'><span style='margin-right: 2px;'>".number_format($rec_det_detelsGR_CT[2],2)."</span></td></TR>";
		 $COUNT_CT = $COUNT_CT + $rec_det_detelsGR_CT[1];
		 $TOTAL_CT = $TOTAL_CT + $rec_det_detelsGR_CT[2];	 
	 } 
	echo "<TR><td style='width:120px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>Total</span></td>";
	echo "<td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>".$COUNT_CT."</span></td>";
	echo "<td style='width:70px;text-align: right;font-weight: bold;font-size: 12px;'><span style='margin-right: 2px;'>".number_format($TOTAL_CT,2)."</span></td></TR>";
?>
</table>
</div>
</div>
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="background:#FFFFFF;text-align:center; font-family:Calibri, Helvetica, sans-serif; font-size:11px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
			...
			<td style="width:40px;text-align: left;"><span style="margin-left: 5px;">Minutes Taken - After Addi. Imgs</span></td>
...
<?php
 ...
 $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
						   ...
						   `cdb_helpdesk`.`ssb_type`,
						   GetLastSendBack(`cdb_helpdesk`.`helpid`) as sbmsg ,
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`facno` = '' AND
	   `cdb_helpdesk`.`curr_stage` = 'LSERECMD' AND
	    /*cdb_helpdesk.helpid = '2018153640' and */
		/*9:24 AM 23/04/2018 (`cdb_helpdesk`.`ssb_type` = 'Additional Images Uploaded' and `cdb_helpdesk`.`assign_to` = 'SSOO') AND*/
		/*`cdb_helpdesk`.`helpid` IN (select l.helpid  from cdb_los_sendback l) AND*/
		`cdb_helpdesk`.`assign_to` IN ('SSOO','SSBI','CO') AND 
		 exists (select 1 from cdb_los_sendback where cdb_los_sendback.helpid = `cdb_helpdesk`.`helpid` and company = 'CDB') AND
	    date(`cdb_helpdesk`.`appcrdate`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."'
       ORDER BY 27 ;"; /*`cdb_helpdesk`.`ssb_type`, cdb_helpdesk.lastactivityon desc ,`cdb_helpdesk`.`appcrdate`*/
    ...
    $index = 0;
     ...
     while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
        ...
        */
		...
		if($rec_det_detels[28] == "0" && $rec_det_detels[12] == 'Application Created'){			
           $col = "#990000";
		   echo "<tr style='color: ".$col.";'>";
		   //$MyRow = "background-color:#ff9999;";
        }else{
			echo "<tr style='color: ".$col.";'>";
		}
		
        if($rec_det_detels[17] <= 15){
            echo "<td style='width:80px;text-align: right;".$MyRow .";color: ".$f_Col.";' title ='".$rec_det_detels[31]."'><div style='display: none;'><input style='width:60px;color:#999999;text-align: right;' type='text' name='txta".$index."' id='txta".$index."' value='".$rec_det_detels[0]."'  onKeyPress='return disableEnterKey(event)'/></div><span style='margin-left: 2px;'>".$rec_det_detels[0]."</span></td>";
        }else{
            echo "<td style='width:80px;text-align: right;".$MyRow .";color: ".$f_Col.";' title ='".$rec_det_detels[31]."'><div style='display: none;'><input style='width:60px;color:#999999;text-align: right;' type='text' name='txta".$index."' id='txta".$index."' value='".$rec_det_detels[0]."'  onKeyPress='return disableEnterKey(event)'/></div><img src='../../../img/exp.png' height='15' width='5'><span style='margin-left: 2px;'>".$rec_det_detels[0]."</span></td>";
        }
        ...
        echo "<td style='width:90px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[32]."</span></td>";
        ...
        echo "<td style='width:70px;text-align: right;".$MyRow ."';color:".$f_Col."title = '".$asingBy."'><span style='margin-right: 2px;'>".$rec_det_detels[9]."</span></td>";
