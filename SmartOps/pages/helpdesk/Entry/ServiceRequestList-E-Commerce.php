...
Page Name		: Service Request List - E - Commerce 
Purpose			: To viwe Request list for Service - E - Commerce  Inquiry
Author			: Madushan Wikramaarachchi
Date & Time		: 10.13 A.M 2018-09-19
--------------------------- ---------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="hel/e/061";
...
<title>Service Request List - E - Commerce</title>
...
<?php
    ...
    $where_2 = "`cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND";
    $sql_sp = "SELECT COUNT(*) FROM cdb_help_user_oth_user_sp AS u WHERE u.user_id = '". $_SESSION['user']."' AND u.cat_code = 1024;";
    ...
    $v_sql_det_detelsCNT = "SELECT COUNT(1) FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`scat_01`,`cat_states` ".$tbl."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`scat_code_1` NOT IN ('102401','102402','102403','102404','102405') AND
       ".$where_2."
