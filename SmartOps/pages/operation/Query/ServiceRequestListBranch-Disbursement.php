...
Page Name		: Service Request List Branch - Disbursement  
Purpose			: To viwe Request List for Disbursement for branch 
Author			: Madushan Wikramaarachchi
Date & Time		: 12.53 P.M 25/06/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/q/001";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Query/ServiceRequestListBranch-Disbursement.php?DispName=Branch%20QC','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Query/ServiceRequestListBranch-Disbursement.php?DispName=Branch%20QC','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    if(cat_cod != '1014' ){
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreqBRanch="+getHI+"&gettxt_helpdesk_closeBRanch="+helpClose,true);
        mydata.send();
    }
...
function is_add_row(){
 ...
 function popup(x){
            ...
            var txt_userBranch = document.getElementById('txt_userBranch').value;
			if(x==1){
			var mydataGried;
			...
			mydataGried.open("GET","ajax_Gried.php"+"?sr_gried="+x+"&userBranch="+txt_userBranch,true);
			mydataGried.send();
			/*document.getElementById('outer').style.visibility = "visible";
			document.getElementById('conten').style.visibility = "visible";*/
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
		...
		date(`cdb_helpdesk`.`COD_START_DATE`) >= (CURRENT_DATE - INTERVAL 30 DAY) AND 
         `cdb_helpdesk`.COD_STATUS = '001' AND `cdb_helpdesk`.COD_DO_PRINT_ON = '0000-00-00 00:00:00'AND
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="table-layout:fixed;width:100%;background:#FFFFFF;text-align:center; font-family:Calibri,Candara,Segoe,Segoe UI,Optima,Arial,sans-serif;; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:90px;text-align: left;"><span style="margin-left: 5px;">SSB Type</span></td>
...
<?php
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`COD_START_DATE`, `cdb_helpdesk`.`issue`,`scat_02`.`scat_discr_2`, 
								   ...
								   `cdb_helpdesk`.`SUP_ACCNO`,`cdb_helpdesk`.`SUP_ACCSTS`
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
        ...
        echo "<td style='width:90px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[12]."</span></td>";
...
<div style="display: none;">
    ...
    <input type="text" name="txt_userBranch" id="txt_userBranch" value="<?php echo  $_SESSION['userBranch']; ?>" />
</div>
