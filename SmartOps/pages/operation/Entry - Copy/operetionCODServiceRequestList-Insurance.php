...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND
		 ...
		 and `cdb_helpdesk`.QC_INS_ON IS NULL
		 and `cdb_helpdesk`.QC_INS_ON_DATA IS NOT NULL
       ORDER BY `cdb_helpdesk`.`helpid`;";
...
<?php
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`COD_START_DATE`, `cdb_helpdesk`.`issue`,`scat_02`.`scat_discr_2`, 
								   ...
								   `cdb_helpdesk`.`COD_CHKLIST_ON`,ROUND(time_to_sec((TIMEDIFF(NOW(), `cdb_helpdesk`.QC_INS_ON_DATA))) / 60),`cdb_helpdesk`.`pr_code`,
								   `cdb_helpdesk`.`QC_CDPU_ON` , `cdb_helpdesk`.`QC_SECDOCS_ON`,`cdb_helpdesk`.`QC_INS_ON`,`cdb_helpdesk`.QC_INS_ON_DATA,
