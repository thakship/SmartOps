...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/operetionCODServiceRequestList-SECURITY.php?DispName=Security%20Doc%20QC','conectpage');
}
function pageRef(){
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/operetionCODServiceRequestList-SECURITY.php?DispName=Security%20Doc%20QC','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    if(cat_cod != '1014' ){
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreqsecu="+getHI+"&gettxt_helpdesk_closesecu="+helpClose,true);
        mydata.send();
    }
  ...
  var key = event.keyCode || event.which;
   ...
   if(key == 120){ // F9
       ...
       var mydataNew;
        ...
        var userID = document.getElementById('txt_USERMY').value;
        var userg = document.getElementById('txt_USERgROUP').value;
		mydataNew.open('GET','ajaxEntryNewGrid_F2_SECURITY.php'+'?n_userID='+userID+'&n_userg='+userg,true);
		mydataNew.send();
       ...
       var mydataNew;
        ...
        var userID = document.getElementById('txt_USERMY').value;
        var userg = document.getElementById('txt_USERgROUP').value;
		mydataNew.open('GET','ajaxEntryNewGrid_F10_SECURITY.php'+'?n_userID='+userID+'&n_userg='+userg,true);
		mydataNew.send();
       ...
       var mydataNew;
        ...
        var userID = document.getElementById('txt_USERMY').value;
        var userg = document.getElementById('txt_USERgROUP').value;
		mydataNew.open('GET','ajaxEntryNewGrid_F8_SECURITY.php'+'?n_userID='+userID+'&n_userg='+userg,true);
		mydataNew.send();
    }
}
</script>
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND
		 ...
		 and `cdb_helpdesk`.QC_SECDOCS_ON IS NULL
		 and `cdb_helpdesk`.QC_SECDOCS_DATA IS NOT NULL
...
<?php
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`COD_START_DATE`, `cdb_helpdesk`.`issue`,`scat_02`.`scat_discr_2`, 
								   ...
								   `cdb_helpdesk`.`COD_CHKLIST_ON`,ROUND(time_to_sec((TIMEDIFF(NOW(), `cdb_helpdesk`.QC_SECDOCS_DATA))) / 60),`cdb_helpdesk`.`pr_code`,
								   `cdb_helpdesk`.`QC_CDPU_ON` , `cdb_helpdesk`.`QC_SECDOCS_ON`,`cdb_helpdesk`.`QC_INS_ON` , `cdb_helpdesk`.QC_SECDOCS_DATA,
								    (select count(*) from pending_upload_file ad where ad.help_id = `cdb_helpdesk`.helpid and ad.DocPurpose = 'Security Doc') AS NUMUPLOADS,
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
	   ...
	   and `cdb_helpdesk`.QC_SECDOCS_ON IS NULL
	   and `cdb_helpdesk`.QC_SECDOCS_DATA IS NOT NULL
       ORDER BY `cdb_helpdesk`.QC_SECDOCS_DATA desc,`cdb_helpdesk`.`COD_START_DATE`;";
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        echo "<td style='width:90px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[31]."</span></td>";
		...
		*/
		$col = $rec_det_detels[30] > 1?"#D7BDE2":"#FDFEFE";
        echo "<td style='width:25px;text-align: left;background-color:".$col."' title='".$rec_det_detels[30]." upload.'><span style='margin-left: 5px;'>".$rec_det_detels[29]."</span></td>";
