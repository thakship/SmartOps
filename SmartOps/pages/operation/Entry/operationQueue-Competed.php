...
Page Name		: Operation Queue - Competed 
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/operationQueue-Competed.php?DispName=Operation%20Queue%20-%20Competed','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Entry/operationQueue-Competed.php?DispName=Operation%20Queue%20-%20Competed','conectpage');
}
...
function isNotifyPending(){
    //alert("p");
    var pendingHelpid = document.getElementById('txt_help_ID').value;
    var pendingEnrtyBy = document.getElementById('txt_USERMY').value;
    var pendingComent = document.getElementById('txt_pn').value;
   ...
   // alert(assuser);
    ...
    if (r==true){
		...
		$.ajax({ 
			type:'POST', 
			data: {getPendingHelpid : pendingHelpid  , getPendingEnrtyBy : pendingEnrtyBy , getPendingComent : pendingComent}, 
			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
			success: function(val_retn) { 
			   //document.getElementById('maneSpan').innerHTML = val_retn;
			   alert(val_retn);
...
function permentRelease(){
    var helpid = document.getElementById('txt_help_ID').value;
    var entryBy = document.getElementById('txt_USERMY').value;
    
   // alert(assuser);
    var r = confirm('Confirm to Payment Release?')
    if (r==true){
        $.ajax({ 
    		type:'POST', 
    		data: {getPermentReleaseHelpID :helpid, getPermentReleaseEntryBy : entryBy}, 
    		url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    		success: function(val_retn) { 
    		   alert(val_retn);
               if(val_retn == "OK"){
                    alert("Payment Release Updated");
                    pageClose();
               }else{
                    alert("Payment Release Not Updated. - Error");
               }
    		}               
    	});
    }
  ...
  var key = event.keyCode || event.which;
   ...
   if(key == 120){ //f9
       ...
       var mydataNew;
		...
		mydataNew.open('GET','ajax_OperationQueueCompeted_F.php'+'?comf9_userID='+userID+'&comf9_userg='+userg,true);
		mydataNew.send();
    } 
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND
        ...
        /*  (`cdb_helpdesk`.doc_verifity = 1 OR
       `cdb_helpdesk`.scat_code_3 IN ('1014013906',
'1014014006',
'1014014106',
'1014014206',
'1014014306')
...
<?php
/*
  $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`facno`,`cdb_helpdesk`.doc_verifity_on								   
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
       `cdb_helpdesk`.`cat_code` = '1014' AND
	   `cdb_helpdesk`.`disbdate` = '0000-00-00' AND
       `cdb_help_user_oth`.`user_group` = '".$_SESSION['usergroupNumber']."' AND
       `cdb_helpdesk`.COD_STATUS = '001'  AND
       `cdb_helpdesk`.doc_verifity = 1 AND
       `cdb_helpdesk`.PAYMENT_RELEASE_ON = '0000-00-00 00:00:00'
       ORDER BY `cdb_helpdesk`.`doc_verifity_on` desc;";
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
		echo $rec_det_detels[0] . " | ". $rec_det_detels[1] . " | ". $rec_det_detels[2]."<BR>";
	}
	*/
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`COD_START_DATE`, `cdb_helpdesk`.`issue`,`scat_02`.`scat_discr_2`, 
								   ...
								   `cdb_helpdesk`.`COD_CHKLIST_ON`,`cdb_helpdesk`.`disbdate`,`cdb_helpdesk`.`COD_STATUS` ,`cdb_helpdesk`.`ssb_type` ,  `cdb_helpdesk`.`COD_LAST_EVENT_DT`
 FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       `cdb_helpdesk`.`scat_code_2` = `scat_02`.`scat_code_2` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        while($rec_add_img_ok = mysqli_fetch_array($QUERY_add_img_ok)){
            ...
            $colectNote = substr($rec_add_img_ok[0],0,24);
        ...
        $row_count_upload_doc = mysqli_num_rows($query_pendin_upload_doc);
         if($row_count_upload_doc == 0){
            //while($rec_pendin_upload_doc = mysqli_fetch_array($query_pendin_upload_doc)){
                ...
                if(substr($colectNote,0,24)  == "Operation Notify Pending"){
                   // $qwer = $rec_pendin_upload_doc[0];
            ...
            while($rec_pendin_upload_doc = mysqli_fetch_array($query_pendin_upload_doc)){
                if(substr($colectNote,0,24)  == "Operation Notify Pending" && $rec_pendin_upload_doc[0] < $rec_det_detels[27]){
                   // $qwer = $rec_pendin_upload_doc[0];
                    $qwer = "background-color: #ff3300;";
                }else{
