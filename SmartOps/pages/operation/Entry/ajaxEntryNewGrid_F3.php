 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    $FromTables = "FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`";       
     ...
     WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
           ...
           `cdb_helpdesk`.`cat_code` = '1014' AND `scat_02`.scat_code_1 = '101401' AND `scat_02`.scat_code_2 = '10140106' AND
...
<?php
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`entry_department` = `deparment`.`deparmentNumber` AND ".$vsqlAux_3." `cdb_helpdesk`.`cmb_code` = '5001' AND
       `cdb_helpdesk`.`cat_code` = '1014' AND `scat_02`.scat_code_1 = '101401' AND  `scat_02`.scat_code_2 = '10140106' AND 
       `cdb_helpdesk`.`ssb_app_number` = '' AND  date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 90 DAY)  AND ". $vsqlAux_1.$vsqlAux_2.$vsqlAux_4." ORDER BY `cdb_helpdesk`.`asing_by` , `cdb_helpdesk`.`helpid` ;";
