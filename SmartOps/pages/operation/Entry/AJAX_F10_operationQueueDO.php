...
if(isset($_REQUEST['f10_userID']) && isset($_REQUEST['f10_userg'])){
    getF2_Table($conn,$_REQUEST['f10_userID'],$_REQUEST['f10_userg']);
}
?>
...
function getF2_Table($conn,$f2_userID,$f2_userg){
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
        ...
        `cdb_helpdesk`.`COD_FILE_PROCUSER` IS NOT NULL AND
          `cdb_helpdesk`.`doc_verifity` = 0 AND
