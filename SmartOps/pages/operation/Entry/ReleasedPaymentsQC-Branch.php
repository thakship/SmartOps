...
Page Name		: Released Payments QC -Branch
Purpose			: To viwe Released Payments for Disbursement  - Branch
Author			: Madushan Wikramaarachchi
Date & Time		: 01.30 P.M 03/08/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/e/009";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/ReleasedPaymentsQC-Branch.php?DispName=Released%20Payments%20QC%20-%20Branch','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Entry/ReleasedPaymentsQC-Branch.php?DispName=Released%20Payments%20QC%20-%20Branch','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreqBRRP="+getHI+"&gettxt_helpdesk_closeBRRP="+helpClose,true);
        mydata.send();
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
		...
		date(`cdb_helpdesk`.`disbdate`) >= (CURRENT_DATE - INTERVAL 15 DAY) AND 
         ...
         `cdb_helpdesk`.PAYMENT_COMPLETED_ON = '0000-00-00 00:00:00' AND
