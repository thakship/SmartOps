...
Page Name		: Released Payments QC Report
Purpose			: To viwe Released Payments for Disbursement  - Pement reles
Author			: Madushan Wikramaarachchi
Date & Time		: 03.45 P.M 2017-10-12
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/e/010";
...
function clientValidate(title,cat_cod){
    ...
    if(cat_cod != '1014' ){
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreq1plq="+getHI+"&gettxt_helpdesk_close1plq="+helpClose,true);
        mydata.send();
    }
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        $sql_row_co = "SELECT COUNT(*) 
                         FROM cdb_helpdesk AS c
                         WHERE c.PAYMENT_COMPLETED_ON != '0000-00-00 00:00:00' AND
                               c.helpid = '".$rec_det_detels[0]."';";
        //echo $sql_row_co;
        
       $query_row_co = mysqli_query($conn,$sql_row_co);
       while($rec_row_co = mysqli_fetch_array($query_row_co)){
            if ($rec_row_co[0] == 0){
