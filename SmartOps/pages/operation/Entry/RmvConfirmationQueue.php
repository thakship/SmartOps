...
Page Name		: RMV Confirmation Queue
Purpose			: To viwe Request List for RMV Confirmation Queue
Author			: Madushan Wikramaarachchi
Date & Time		: 04.04 P.M 27/03/2019
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/e/012";
...
<title>RMV Confirmation Queue</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/RmvConfirmationQueue.php?DispName=RMV%20Confirmation%20Queue','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Entry/RmvConfirmationQueue.php?DispName=RMV%20Confirmation%20Queue','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreq1rmvlq="+getHI+"&gettxt_helpdesk_close1rmvlq="+helpClose+"&pageref=RMVConfirmation",true);
        mydata.send();
    ...
    var r = confirm('Confirm to Update ?')
            if (r==true){
    			...
    			$.ajax({ 
    				...
    				type:'POST', 
    				data: {get_assuserrmv : assuser  , get_aass_ksrl_helpidrmv : ksrl_helpid , pagerefrmv : 'RMVConfirmation'}, 
    				url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				   // alert(val_retn);
    ...
    if (r==true){
       // alert('function ok');
		$.ajax({ 
			...
			type:'POST', 
		//	data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVConfirmation' , getrmv_officer : rmv_officer}, 
        data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVConfirmation' ,getrmv_officer : rmv_officer, getrmv_vehicle : rmv_vehicle , getrmv_Cr_Stat_name : rmv_Cr_Stat_name ,getrmv_Cr_Stat_code : rmv_Cr_Stat_code ,RMV_PAMT : txt_RMVPayment , Licensing_Authority : txt_Licensing_Authority}, 
			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
			success: function(val_retn) { 
			   //document.getElementById('maneSpan').innerHTML = val_retn;
			    //alert(val_retn);
...
<table border="1" cellpadding="0" cellspacing="0" id="myTable"  style="table-layout:fixed;width:100%;background:#FFFFFF;text-align:center; font-family:Calibri,Candara,Segoe,Segoe UI,Optima,Arial,sans-serif;; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:60px;text-align: right;"><span style="margin-right: 5px;">RMV Assigned User</span></td>
...
<?php
 ...
 WHERE  `cdb_helpdesk`.`helpid` =  `rmv_header`.`helpid` AND
     ...
     /*  `cdb_helpdesk`.COD_STATUS = '001' AND `cdb_helpdesk`.COD_DO_PRINT_ON = '0000-00-00 00:00:00' */
       ...
       and `rmv_header`.`RmvConfirmedBy` IS NULL
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
        ...
        */
        ...
        echo "<td style='width:70px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[3]." - ".$rec_det_detels[36]."</span></td>";
		...
		echo "<td style='width:90px;text-align: left;".$MyRow ."' title='".con_min_days($rec_det_detels[24])."'><span style='margin-left: 2px;'>".$rec_det_detels[27]."</span></td>";
        /*Rizvi*/           
