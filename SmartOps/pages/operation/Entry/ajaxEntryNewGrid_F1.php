 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    $FromTables = "FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `scat_02` , `branch`, `deparment`,`cdb_help_user_oth`";       
     ...
     WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
           ...
           `cdb_helpdesk`.`ssb_app_number` = ''  AND  date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 90 DAY)  AND ". $vsqlAux_1.$vsqlAux_2.$vsqlAux_4." ORDER BY `cdb_helpdesk`.`helpid`;";       
...
<?php
 ...
 `cdb_helpdesk`.`facno`,(if(`cdb_helpdesk`.`ssb_type` like 'Pending Notified%' , 0, ROUND(time_to_sec((TIMEDIFF(NOW(), `cdb_helpdesk`.lastactivityon))) / 60)))".
 $FromTables."
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       `cdb_helpdesk`.`pr_code` = `priority_states`.`pr_code` AND
       `cdb_helpdesk`.`scat_code_2` = `scat_02`.`scat_code_2` AND
       ...
       `cdb_helpdesk`.`entry_department` = `deparment`.`deparmentNumber` AND ".$vsqlAux_3." `cdb_helpdesk`.`cmb_code` = '5001' AND
       `cdb_helpdesk`.`cat_code` = '1014' AND `scat_02`.scat_code_1 = '101401' AND 
       `cdb_helpdesk`.`ssb_app_number` = '' AND  date(`cdb_helpdesk`.`enterDateTime`) >= (CURRENT_DATE - INTERVAL 90 DAY)  
	   AND `cdb_helpdesk`.`helpid` NOT IN (select cdb_ssb_his.helpid from cdb_ssb_his where cdb_ssb_his.P_V = 'V')
	   AND ". $vsqlAux_1.$vsqlAux_2.$vsqlAux_4." ORDER BY 10,18 desc;";
