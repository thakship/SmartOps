...
Page Name		: Released Payments QC
Purpose			: To viwe Released Payments for Disbursement  
Author			: Madushan Wikramaarachchi
Date & Time		: 03.50 P.M 02/08/2017
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/e/008";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('cdberp:8080/cdb/pages/operation/Entry/ReleasedPaymentsQC.php?DispName=Released%20Payments%20QC','conectpage');
}
function pageRef(){
   window.open('cdberp:8080/cdb/pages/operation/Entry/ReleasedPaymentsQC.php?DispName=Released%20Payments%20QC','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreqRP="+getHI+"&gettxt_helpdesk_closeRP="+helpClose,true);
        mydata.send();
    ...
    var r = confirm('Confirm to Update ?')
            if (r==true){
    			...
    			$.ajax({ 
    				...
    				url: '../Operation_DEVELOPMENT/PHP_FUNCTION/php_function_PAYMENT.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				    alert(val_retn);
                        if(val_retn == 'OK'){
...
function is_sel_BacktoCOD(){
     var assuser = document.getElementById('txt_inner_User1').value;
     var ksrl_helpid = document.getElementById('txt_help_ID').value;
     var backtocod_dis = document.getElementById('sel_BacktoCOD').value; 
    
   // alert(assuser);
   if(backtocod_dis == ""){
        alert('Missing Back to COD Comment');
   }else{
        var r = confirm('Confirm to Back to COD ?')
        if (r==true){
            //alert('function ok');
    		$.ajax({ 
    		 
    			type:'POST', 
    			data: {get_backtocodassuser : assuser  , get_backtocod_helpid : ksrl_helpid , getbacktocod_dis : backtocod_dis}, 
    			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/php_function_PAYMENT.php', 
    			success: function(val_retn) { 
    			   //document.getElementById('maneSpan').innerHTML = val_retn;
    			    alert(val_retn);
...
function isPaymentCompleted(){
    ...
    var paymentMode = document.getElementById('selPaymentMode').value;
   // alert(assuser);
   if(paymentMode != ""){
         var r = confirm('Confirm to Payment Completed?')
        if (r==true){
            $.ajax({ 
        		type:'POST', 
        		data: {getPaymentCompletedHelpID :helpid, getPaymentCompletedEntryBy : entryBy , getpaymentMode : paymentMode}, 
        		url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
        		success: function(val_retn) { 
        		   alert(val_retn);
                   //document.getElementById('maneSpan').innerHTML= val_retn;
                   if(val_retn == "OK"){
                        alert("Payment Completed Updated");
                   }else{
                        alert("PaymentCompleted Not Updated. - Error");
                   }
        		}               
        	});
        }
   }else{
        alert("Missing Payment Mode.");
   }
...
<?php
    $v_sql_det_detelsCNT = "SELECT COUNT(1) FROM `cdb_helpdesk`, `urgency_states`, `priority_states`, `cmb_states`, `branch`, `deparment`
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
       ...
       `cdb_helpdesk`.`cmb_code` = '5002' AND
		...
		date(`cdb_helpdesk`.`disbdate`) >= (CURRENT_DATE - INTERVAL 15 DAY) AND 
         ...
         `cdb_helpdesk`.PAYMENT_COMPLETED_ON = '0000-00-00 00:00:00'
...
<?php
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , `cdb_helpdesk`.`COD_START_DATE`, `cdb_helpdesk`.`issue`,`scat_02`.`scat_discr_2`, 
								   ...
								   `cdb_helpdesk`.`COD_CHKLIST_ON`,`cdb_helpdesk`.`disbdate`,`cdb_helpdesk`.`COD_STATUS`
 ...
 WHERE `cdb_helpdesk`.`ur_code` = `urgency_states`.`ur_code` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       $sql_count_note = "SELECT COUNT(*) FROM `cdb_help_note` WHERE `helpid` = '".$rec_det_detels[0]."';";
        ...
        echo "<td style='width:190px;text-align: left;".$colPH ."'><span style='margin-left: 2px;'>".$rec_det_detels[12]."</span></td>";
