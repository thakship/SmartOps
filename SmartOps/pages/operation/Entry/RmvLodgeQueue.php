...
Page Name		: RMV Lodge Queue  
Purpose			: To viwe RMV Lodge Queue  
Author			: Nilanka Chameera
Date & Time		: 10.00 A.M 26/03/2019
------------------------------------------------------------------------------------------------------------------------->
<!-- Common Included -->
<?php
	session_start();
	$_SESSION['page']="ope/e/011";
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/RmvLodgeQueue.php?DispName=RMV%20Lodge%20-%20Queue','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Entry/RmvLodgeQueue.php?DispName=RMV%20Lodge%20-%20Queue','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreq1rmvlq="+getHI+"&gettxt_helpdesk_close1rmvlq="+helpClose+"&pageref=RMVLodge",true);
        mydata.send();
   ...
   // alert(assuser);
    var r = confirm('Confirm to Update ?')
            if (r==true){
    			...
    			$.ajax({ 
    			 	type:'POST', 
    				data: {get_assuserrmv : assuser  , get_aass_ksrl_helpidrmv : ksrl_helpid , pagerefrmv : 'RMVLodge'}, 
    				url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				   // alert(val_retn);
...
function isRMVlodge(){
  //  alert('RMV');
    var ksrl_helpid = document.getElementById('txt_help_ID').value;
    var ksrl_user = document.getElementById('txt_USERMY').value;
    var rmv_officer = document.getElementById('sel_Rmv_officer').value;
     var txt_RMVPayment = document.getElementById('txt_RMVPayment').value;
     var txt_Licensing_Authority = "";
     var txt_RmvDebitedAmountfrom = document.getElementById('txt_RmvDebitedAmountfrom').value;
     //alert(Number(txt_RmvDebitedAmountfrom));
     //alert(Number(txt_RMVPayment));
     
    
    var rmv_vehicle = "" ;
    var rmv_Cr_Stat_name = "" ;
    var rmv_Cr_Stat_code = "" ;
    
    
    if(txt_RmvDebitedAmountfrom< txt_RMVPayment){
        alert('exceed debited amount');
    }else{
        var r = confirm('Confirm to Update ?')
        if (r==true){
          //  alert('function ok');
    		$.ajax({ 
    			...
    			type:'POST', 
    		//	data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVLodge' , getrmv_officer : rmv_officer}, 
                data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVLodge' ,getrmv_officer : rmv_officer, getrmv_vehicle : rmv_vehicle , getrmv_Cr_Stat_name : rmv_Cr_Stat_name ,getrmv_Cr_Stat_code : rmv_Cr_Stat_code ,RMV_PAMT : txt_RMVPayment , Licensing_Authority : txt_Licensing_Authority}, 
    			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    			success: function(val_retn) { 
    			  // document.getElementById('maneSpan').innerHTML = val_retn;
    			    //alert(val_retn);
        ...
        if (r==true){
          //  alert('function ok');
    		$.ajax({ 
    			...
    			type:'POST', 
    		//	data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVLodge' , getrmv_officer : rmv_officer}, 
                data: {getCDPUQCUserrmvlodgeext : ksrl_user  , getCDPUQCHelpIDrmvlodgeext : ksrl_helpid , getPageTitleext : 'RMVLodge' ,getrmv_officerext : rmv_officer, getrmv_vehicleext : rmv_vehicle , getrmv_Cr_Stat_nameext : rmv_Cr_Stat_name ,getrmv_Cr_Stat_codeext : rmv_Cr_Stat_code ,RMV_PAMText : txt_RMVPayment , Licensing_Authorityext : txt_Licensing_Authority}, 
    			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    			success: function(val_retn) { 
    			  // document.getElementById('maneSpan').innerHTML = val_retn;
    			    //alert(val_retn);
...
function isRMVExtractCashInHand(){
    var ksrl_helpid = document.getElementById('txt_help_ID').value;
    var ksrl_user = document.getElementById('txt_USERMY').value;
    var rmv_officer = document.getElementById('sel_Rmv_officer').value;
     var txt_RMVPayment = document.getElementById('txt_RMVPayment').value;
...
// use for text field enter only numbers
...
<table border="1"  cellpadding="0" cellspacing="0" id="myTable"  style="table-layout:fixed;width:100%;background:#FFFFFF;text-align:center; font-family:Calibri,Candara,Segoe,Segoe UI,Optima,Arial,sans-serif;; font-size:12px;">
        <tr style="background-color: #BEBABA; font-size: 11; font-weight: bold;">
            ...
            <td style="width:60px;text-align: right;"><span style="margin-right: 5px;">RMV Assign User</span></td>
...
<?php
       ...
       $v_sql_det_detels = "SELECT `cdb_helpdesk`.`helpid` , 
								   ...
								   `rmv_header`.`RmvLogAssignBy`, 
								   `rmv_header`.`RmvLogAssignOn`
 ...
 WHERE  `cdb_helpdesk`.`helpid` =  `rmv_header`.`helpid` AND
       ...
       /* `cdb_helpdesk`.COD_STATUS = '001' */ 
        `rmv_header`.`RmvLogBy` is null AND
         `rmv_header`.`RMV_Type` not in ('AUTO FINANCE','NON AUTO','NA')
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
        ...
        if( $rec_det_detels[25]=='7002'){ // || $rec_det_detels[25]=="7002" $rec_det_detels[8]=='Highest' ||
         ...
         $MyRow = "";
	   /*
 	   $sql_selectrmv_header    = "SELECT `rmv_header`.`RmvLogAssignBy`, `rmv_header`.`RmvLogAssignOn` 
                                     from `rmv_header` 
                                    WHERE `rmv_header`.`helpid` = '".$rec_det_detels[0]."';";
       //echo $sql_selectrmv_header;
        ...
        while($rec_selectrmv_header = mysqli_fetch_array($query_selectrmv_header)){
            $RMV_ASS_USER = $rec_selectrmv_header[0];
		...
		*/
		...
		$RMV_ASS_USER = $rec_det_detels[38];
		if ($_SESSION['user'] == $rec_det_detels[38]){
			$MyRow = "background-color:lightblue;";
		}
		
		
        $asingBy = getUserNameGenaral_Mob_Eml($RMV_ASS_USER,$conn);
        $col = "";
        ...
        */
        ...
        echo "<td style='width:60px;text-align: right;".$qwer."' title = '".$asingBy."'><span style='margin-right: 2px;'>".$RMV_ASS_USER."</span></td>";
		...
		echo "<td style='width:90px;text-align: left;".$MyRow ."' title='".con_min_days($rec_det_detels[26])."'><span style='margin-left: 2px;'>".$rec_det_detels[26]."</span></td>";
        /*Rizvi*/           
