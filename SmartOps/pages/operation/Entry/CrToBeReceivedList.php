...
<title>CR To Be Received List</title>
...
function pageClose(){ //Page Close Function.......
    ...
    window.open('http://cdberp:8080/cdb/pages/operation/Entry/CrToBeReceivedList.php?DispName=CR%20To%20Be%20Received%20List','conectpage');
}
function pageRef(){
   window.open('http://cdberp:8080/cdb/pages/operation/Entry/CrToBeReceivedList.php?DispName=CR%20To%20Be%20Received%20List','conectpage');
}
...
function clientValidate(title,cat_cod){
    ...
    var helpClose = document.getElementById('txt_helpdesk_close').value;
        ...
        mydata.open("GET","../Operation_DEVELOPMENT/PHP_FUNCTION/operation_Ajax.php"+"?getHelpIDreq1rmvlq="+getHI+"&gettxt_helpdesk_close1rmvlq="+helpClose+"&pageref=RMVToBeReceived",true);
        mydata.send();
    ...
    var r = confirm('Confirm to Update ?')
            if (r==true){
    			...
    			$.ajax({ 
    				...
    				type:'POST', 
    				data: {get_assuserrmv : assuser  , get_aass_ksrl_helpidrmv : ksrl_helpid , pagerefrmv : 'RMVToBeReceived'}, 
    				url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
    				success: function(val_retn) { 
    				   //document.getElementById('maneSpan').innerHTML = val_retn;
    				   // alert(val_retn);
...
function isRMVToBeReceived(){
    //alert('RMV');
    var ksrl_helpid = document.getElementById('txt_help_ID').value;
    var ksrl_user = document.getElementById('txt_USERMY').value;
    var rmv_officer = "";
      var txt_RMVPayment = "";
     var txt_Licensing_Authority = document.getElementById('txt_Licensing_Authority').value;
    var rmv_vehicle = document.getElementById('txt_Vehical').value;
    var rmv_Cr_Stat_name = document.getElementById('sel_cr_crstat_name').value;
    var rmv_Cr_Stat_code = document.getElementById('sel_rmv_cr_place').value;
    ...
    if (r==true){
       // alert('function ok');
		$.ajax({ 
			...
			type:'POST', 
			data: {getCDPUQCUserrmvlodge : ksrl_user  , getCDPUQCHelpIDrmvlodge : ksrl_helpid , getPageTitle : 'RMVToBeReceived' ,getrmv_officer : rmv_officer, getrmv_vehicle : rmv_vehicle , getrmv_Cr_Stat_name : rmv_Cr_Stat_name ,getrmv_Cr_Stat_code : rmv_Cr_Stat_code ,RMV_PAMT : txt_RMVPayment , Licensing_Authority : txt_Licensing_Authority }, 
			url: '../Operation_DEVELOPMENT/PHP_FUNCTION/kiosk_php_function.php', 
			success: function(val_retn) { 
			   //document.getElementById('maneSpan').innerHTML = val_retn;
			    //alert(val_retn);
 ...
 WHERE  `cdb_helpdesk`.`helpid` =  `rmv_header`.`helpid` AND
    ...
    while($rec_det_detels = mysqli_fetch_array($v_que_det_detels)){
       ...
       echo "<td style='width:120px;text-align: left;".$MyRow ."'><span style='margin-left: 2px;'>".$rec_det_detels[26]."</span></td>";
		...
		echo "<td style='width:90px;text-align: left;".$MyRow ."' title='".con_min_days($rec_det_detels[27])."'><span style='margin-left: 2px;'>".$rec_det_detels[27]."</span></td>";
        /*Rizvi*/           
