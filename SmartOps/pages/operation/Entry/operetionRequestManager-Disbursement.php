...
<table border="1" id="myTable" cellpadding="0" cellspacing="0" style="background:#FFFFFF;text-align:center; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
            <tr style="background-color: #BEBABA;">
                ...
                <td style="width:200px;text-align: left;"><span  style="margin-left: 5px;">Last Statas</span></td>
                <td style="width:200px;text-align: left;"><span  style="margin-left: 5px;">Last Statas ON</span></td>
                <td style="width:100px;text-align: left;"><span  style="margin-left: 5px;">Min. Taken - After Last Act. </span></td>
                <td style="width:100px;text-align: left;"><span  style="margin-left: 5px;">Assigned User </span></td>                
...
<?php
    ...
    /*$v_sql_select = "SELECT `helpid`, DATE(`enterDateTime`) AS getDate,`issue` 
      ...
      $v_sql_select = "SELECT * 
                         FROM (SELECT `helpid`, DATE(`enterDateTime`) AS getDate,`issue` ,  `COD_LAST_EVENT`, `COD_STATUS` ,
                             ROUND(time_to_sec((TIMEDIFF(now(),`COD_LAST_EVENT_DT`))) / 60) AS ADII_UP , COD_LAST_EVENT_DT ,ssb_type ,asing_by
                                                   FROM `cdb_helpdesk` 
                                                   WHERE `cdb_helpdesk`.`cat_code` = '1014' AND 
                                                         `cdb_helpdesk`.`cmb_code` = '5002' AND 
                                                         `cdb_helpdesk`.COD_STATUS IN ('001','002' , '003') AND
                                                         `cdb_helpdesk`.PAYMENT_RELEASE_ON = '0000-00-00 00:00:00'  AND
                                                         `COD_LAST_EVENT` <> 'Facility Cancelled' AND
					                 			        (`cdb_helpdesk`.`entry_branch` = '".$_SESSION['userBranch']."')  AND  
					               			          `cdb_helpdesk`.`entry_department` = '".$_SESSION['userDepartment']."' AND
                                                          date(`cdb_helpdesk`.`COD_START_DATE`) >= (CURRENT_DATE - INTERVAL 90 DAY)
                                              UNION ALL
	                  				           SELECT `helpid`, DATE(`enterDateTime`) AS getDate,`issue` ,  `COD_LAST_EVENT` , `COD_STATUS`,
                                                     ROUND(time_to_sec((TIMEDIFF(now(),`COD_LAST_EVENT_DT`))) /60) AS ADII_UP ,COD_LAST_EVENT_DT ,ssb_type ,asing_by
                                                   FROM `cdb_helpdesk` 
                                                   WHERE `cdb_helpdesk`.`cat_code` = '1014' AND 
                                                         `cdb_helpdesk`.`cmb_code` = '5002' AND 
   ...
   // echo $v_sql_select;
    ...
    while($res_Select = mysqli_fetch_assoc($v_que_select)){
        ...
        $add_img_Col = "#FFFFFF";
        if($res_Select['ADII_UP'] > 1440){
            $add_img_Col = "#ff9999";
       }
       $asingBy = getUserNameGenaral_Mob_Eml($res_Select['asing_by'],$conn);
       $sql_add_img_ok = "SELECT n.note_discr
                                FROM  cdb_help_note AS n 
                                WHERE n.helpid = '".$res_Select['helpid']."'  
        ...
        while($rec_add_img_ok = mysqli_fetch_array($QUERY_add_img_ok)){
            $qqq = $rec_add_img_ok[0];
            if((substr($rec_add_img_ok[0],0,24) == 'Operation Notify Pending' || substr($rec_add_img_ok[0],0,20) == 'COD Pending Notified') && $res_Select['ssb_type'] == 'Additional Images Uploaded'){ 
                $col = "#BB11F3;"; //|| 
            }
        }
        ...
        $sql_pendin_upload_doc = "SELECT pu.done_on ,  pu.DocPurpose
                                    ...
                                    WHERE pu.help_id = '".$res_Select['helpid']."' AND
        ...
        $row_count_upload_doc = mysqli_num_rows($query_pendin_upload_doc);
         if($row_count_upload_doc == 0){
            //while($rec_pendin_upload_doc = mysqli_fetch_array($query_pendin_upload_doc)){
                ...
                if(substr($res_Select['COD_LAST_EVENT'],0,16)  == "Pending Notified"){
                   // $qwer = $rec_pendin_upload_doc[0];
            ...
            while($rec_pendin_upload_doc = mysqli_fetch_array($query_pendin_upload_doc)){
                if(substr($res_Select['COD_LAST_EVENT'],0,16)  == "Pending Notified" && $rec_pendin_upload_doc[0] < $res_Select['COD_LAST_EVENT_DT']){
                   // $qwer = $rec_pendin_upload_doc[0];
                    $qwer = "background-color: #ff3300;";
                }else{
        ...
        echo "<tr style='background-color: ".$col.";' title='".$qqq."'>";
        ...
        echo "<td style='width:200px; text-align: left;' title = '".$res_Select['COD_LAST_EVENT_DT']."'><span  style='margin-left: 5px;'>".$res_Select['COD_LAST_EVENT']."</span></td>";
        echo "<td style='width:200px; text-align: left;'><span  style='margin-left: 5px;'>".$res_Select['COD_LAST_EVENT_DT']."</span></td>";
        echo "<td style='width:100px; text-align: left;background-color: ".$add_img_Col.";'><span  style='margin-left: 5px;'>".$res_Select['ADII_UP']."</span></td>";
        echo "<td style='width:100px; text-align: left;".$qwer."' title='".$asingBy."'><span  style='margin-left: 5px;'>".$res_Select['asing_by']."</span></td>";  // asing_by       
        echo "<td style='width:250px;'>";
                if($res_Select['COD_STATUS'] == '002' || $res_Select['COD_STATUS'] == '003'){
                    echo "<input type='button'   class='buttonManage' id='btn_Edit' name='btn_Edit' value='Edit' disabled='disabled' title='txtclose".$index."' onclick='isEditRequset(title);'/>";
                }else{
                     echo "<input type='button'  class='buttonManage' id='btn_Edit' name='btn_Edit' value='Edit'  title='txtclose".$index."' onclick='isEditRequset(title);' />";
                }
                echo "<input type='button' class='buttonManage' id='btn_uplad' name='btn_uplad' value='Additional Img' title='txtclose".$index."' onclick='isAddiImgRequset(title);'/>";
           echo "</td>";
        echo "</tr>";
    }
?>
